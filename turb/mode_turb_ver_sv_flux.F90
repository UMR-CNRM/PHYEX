...
IF (TURBN%LHARAT) THEN
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZKEFF(JIJ,JK+IKB-(1) ) =  PLM(JIJ,JK) * SQRT(PTKEM(JIJ,JK))
    ENDDO
  ENDDO
ELSE
  DO JK=1,D%NKT 
...
! Preparation of the arguments for TRIDIAG
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE   
        ZA(JIJ,JK+IKB-(1) ) = -PTSTEP * ZKEFF(JIJ,JK) * ZWORK1(JIJ,JK) &
        / PDZZ(JIJ,JK)**2
      ENDDO
    ENDDO
    ELSE
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZA(JIJ,JK+IKB-(1) ) = -PTSTEP*ZCSV*PPSI_SV(JIJ,JK,JSV) *   &
        ZKEFF(JIJ,JK) * ZWORK1(JIJ,JK) / PDZZ(JIJ,JK)**2
      ENDDO
    ENDDO
    ENDIF
...
! is taken into account in the vertical part
  IF (TURBN%CTURBDIM=='3DIM') THEN
    DO JIJ=IIJB,IIJE 
      ZSOURCE(JIJ,IKB) = (TURBN%XIMPL*PSFSVP(JIJ,JSV) + PEXPL*PSFSVM(JIJ,JSV)) / &
      PDZZ(JIJ,IKB) * PDIRCOSZW(JIJ)                    &
...
!  Compute the equivalent tendency for the JSV scalar variable
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      PRSVS(JIJ,JK+IKB-(1) ,JSV)= PRSVS(JIJ,JK,JSV)+    &
      PRHODJ(JIJ,JK)*(ZRES(JIJ,JK)-PSVM(JIJ,JK,JSV))/PTSTEP
    ENDDO
  ENDDO
!
  IF ( (TURBN%LTURB_FLX .AND. TPFILE%LOPENED) .OR. OLES_CALL ) THEN
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK1(JIJ,JK) = PLM(JIJ,JK)*SQRT(PTKEM(JIJ,JK))
        ZWORK2(JIJ,JK) = TURBN%XIMPL*ZRES(JIJ,JK) + PEXPL*PSVM(JIJ,JK,JSV)
      ENDDO
    ENDDO
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZFLXZ(JIJ,JK+IKB-(1) ) = -ZCSV * PPSI_SV(JIJ,JK,JSV) * ZWORK3(JIJ,JK) & 
        / PDZZ(JIJ,JK) * &
        ZWORK4(JIJ,JK)
      ENDDO
    ENDDO
    ...
    IF (TURBN%CTURBDIM=='3DIM') THEN
      DO JIJ=IIJB,IIJE 
        ZFLXZ(JIJ,IKB) = (TURBN%XIMPL*PSFSVP(JIJ,JSV) + PEXPL*PSFSVM(JIJ,JSV))  &
        * PDIRCOSZW(JIJ)  
      ENDDO
    ELSE
      DO JIJ=IIJB,IIJE 
    ...
    DO JK=IKTB+1,IKTE-1
      DO JIJ=IIJB,IIJE 
        PWSV(JIJ,JK,JSV)=0.5*(ZFLXZ(JIJ,JK)+ZFLXZ(JIJ,JK+D%NKL))
      ENDDO
    END DO
    DO JIJ=IIJB,IIJE 
      PWSV(JIJ,IKB,JSV)=0.5*(ZFLXZ(JIJ,IKB)+ZFLXZ(JIJ,IKB+D%NKL))
      PWSV(JIJ,IKE,JSV)=PWSV(JIJ,IKE-D%NKL,JSV)
    ENDDO
 END IF
  !
  ...
  IF (OLES_CALL) THEN
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK2(JIJ,JK) = -ZCSVP*SQRT(PTKEM(JIJ,JK))/PLM(JIJ,JK)*ZWORK1(JIJ,JK)
      ENDDO
    ENDDO
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK1(JIJ,JK) = PWM(JIJ,JK)*ZFLXZ(JIJ,JK)
      ENDDO
    ENDDO
