...
DO JIJ=IIJB,IIJE 
  PETHETA(JIJ,D%NKA) = 2.*PETHETA(JIJ,IKB) - PETHETA(JIJ,IKB+D%NKL)
  PEMOIST(JIJ,D%NKA) = 2.*PEMOIST(JIJ,IKB) - PEMOIST(JIJ,IKB+D%NKL)
ENDDO
!
!---------------------------------------------------------------------------
IF (.NOT. OHARAT) THEN
...
IF (OOCEAN) THEN
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        PREDTH1(JIJ,JK)= CSTURB%XCTV*PBLL_O_E(JIJ,JK)*ZWORK1(JIJ,JK)
      ENDDO
    ENDDO
  PREDR1(:,:) = 0.
ELSE
  IF (KRR /= 0) THEN                ! moist case
    CALL GZ_M_W_PHY(D,PRM(:,:,1),PDZZ,ZWORK2)
      DO JK=1,D%NKT 
        DO JIJ=IIJB,IIJE 
          PREDTH1(JIJ,JK)= CSTURB%XCTV*PBLL_O_E(JIJ,JK) * PETHETA(JIJ,JK) &
          * ZWORK1(JIJ,JK)
          PREDR1(JIJ,JK) = CSTURB%XCTV*PBLL_O_E(JIJ,JK) * PEMOIST(JIJ,JK) &
          * ZWORK2(JIJ,JK)
        ENDDO
      ENDDO
  ELSE                              ! dry case
...
DO JSV=1,KSV
  CALL GZ_M_W_PHY(D,PSVM(:,:,JSV),PDZZ,ZWORK1)
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        PREDS1(JIJ,JK,JSV)=CSTURB%XCTV*PBLL_O_E(JIJ,JK)*ZWORK1(JIJ,JK)
      ENDDO
    ENDDO
END DO
!
...
!
  ...
  IF (KRR /= 0) THEN                 ! moist 3D case
    ...
    !
      DO JK=1,D%NKT 
        DO JIJ=IIJB,IIJE 
          PRED2TH3(JIJ,JK)= PREDTH1(JIJ,JK)**2+(CSTURB%XCTV*PBLL_O_E(JIJ,JK) &
          *PETHETA(JIJ,JK) )**2 * ZWORK2(JIJ,JK)
!
          PRED2R3(JIJ,JK)= PREDR1(JIJ,JK)**2 + (CSTURB%XCTV*PBLL_O_E(JIJ,JK) &
          * PEMOIST(JIJ,JK))**2 * ZWORK3(JIJ,JK)
!
          PRED2THR3(JIJ,JK)= PREDR1(JIJ,JK) * PREDTH1(JIJ,JK) +  CSTURB%XCTV**2 &
          * PBLL_O_E(JIJ,JK)**2   &
...
!
  ELSE                 ! dry 3D case in a 2D model
      DO JK=1,D%NKT 
        DO JIJ=IIJB,IIJE 
          PRED2TH3(JIJ,JK) = PREDTH1(JIJ,JK)**2 +  CSTURB%XCTV**2 & 
...
!
    IF (OOCEAN) THEN
        DO JK=1,D%NKT 
          DO JIJ=IIJB,IIJE 
            ZWORK1(JIJ,JK) = (CST%XG *CST%XALPHAOC * PLM(JIJ,JK) * PLEPS(JIJ,JK) &
            / PTKEM(JIJ,JK))**2
          ENDDO
        ENDDO
      CALL MZM_PHY(D,ZWORK1,ZWORK2)  
      IF (KRR /= 0) THEN
          DO JK=1,D%NKT 
            DO JIJ=IIJB,IIJE 
              ZW1(JIJ,JK) = ZWORK2(JIJ,JK) * PETHETA(JIJ,JK)
            ENDDO
          ENDDO
      ELSE
        ZW1 = ZWORK2
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PM3_WTH_WTH2(JIJ,JK) = CSTURB%XCSHF*PBLL_O_E(JIJ,JK)&
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ...
    (0.5*CSTURB%XCSHF*PBLL_O_E(JIJ,JK)*PETHETA(JIJ,JK)*0.5/CSTURB%XCTD/PD(JIJ,JK) &
    - PM3_WTH_WTH2(JIJ,JK)/PD(JIJ,JK)&
    *(1.5+PREDTH1(JIJ,JK)+PREDR1(JIJ,JK))  )&
    * PBLL_O_E(JIJ,JK) * PETHETA(JIJ,JK) * CSTURB%XCTV
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PM3_WTH_W2TH(JIJ,JK) = CSTURB%XCSHF*PKEFF(JIJ,JK)*1.5/ZWORK1(JIJ,JK) &
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ...
    - CSTURB%XCSHF*PKEFF(JIJ,JK)*1.5/ZWORK1(JIJ,JK)/(1.+PREDTH1(JIJ,JK))**2 &
    * CSTURB%XCTV*PBLL_O_E(JIJ,JK)*PETHETA(JIJ,JK)  &
    * (1. - 0.5*PREDR1(JIJ,JK)*(1.+PREDR1(JIJ,JK))/PD(JIJ,JK)* &
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PM3_WTH_W2R(JIJ,JK) = &
    - CSTURB%XCSHF*PKEFF(JIJ,JK)*0.75*CSTURB%XCTV*PBLL_O_E(JIJ,JK) &
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PM3_WTH_WR2(JIJ,JK) = - CSTURB%XCSHF*PKEFF(JIJ,JK)& 
    *0.25*PBLL_O_E(JIJ,JK)*CSTURB%XCTV*PEMOIST(JIJ,JK)**2 &
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PD_M3_WTH_WR2_O_DDTDZ(JIJ,JK) = - CSTURB%XCSHF*PKEFF(JIJ,JK)& 
    *0.25*PBLL_O_E(JIJ,JK)*CSTURB%XCTV*PEMOIST(JIJ,JK)**2 &
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PM3_WTH_WTHR(JIJ,JK) = &
    CSTURB%XCSHF*PKEFF(JIJ,JK)*PEMOIST(JIJ,JK)*ZWORK2(JIJ,JK) &
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ...
    /PD(JIJ,JK)*CSTURB%XCTV*PBLL_O_E(JIJ,JK)*PETHETA(JIJ,JK)
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ...
    * 1.5*PLM(JIJ,JK)*PLEPS(JIJ,JK)/PTKE(JIJ,JK)*CSTURB%XCTV
  ENDDO
ENDDO
!
...
!        / (1.+PREDTH1)**2, D%NKA, D%NKU, D%NKL)
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ...
      /PTKE(JIJ,JK)*CSTURB%XCTV * ZWORK2(JIJ,JK)
    ENDDO
  ENDDO
ELSE
  DO JK=1,D%NKT 
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ...
    *0.5/CSTURB%XCTD/PSQRT_TKE(JIJ,JK)*CSTURB%XCTV * ZWORK2(JIJ,JK)
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PM3_TH2_W2R(JIJ,JK) = 0.75*CSTURB%XCTV**2*ZWORK2(JIJ,JK) &
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PD_M3_TH2_W2R_O_DDTDZ(JIJ,JK) = 0.75*CSTURB%XCTV**2*PLM(JIJ,JK) *PLEPS(JIJ,JK) &
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PM3_TH2_WR2(JIJ,JK) = 0.25*CSTURB%XCTV**2*ZWORK2(JIJ,JK)&
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PD_M3_TH2_WR2_O_DDTDZ(JIJ,JK) = 0.25*CSTURB%XCTV**2*PLEPS(JIJ,JK) & 
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PM3_TH2_WTHR(JIJ,JK) = - 0.5*CSTURB%XCTV*PLEPS(JIJ,JK) & 
    / PSQRT_TKE(JIJ,JK)/CSTURB%XCTD * ZWORK2(JIJ,JK)
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PD_M3_TH2_WTHR_O_DDTDZ(JIJ,JK) = - 0.5*CSTURB%XCTV*PLEPS(JIJ,JK) & 
    / PSQRT_TKE(JIJ,JK)/CSTURB%XCTD * ZWORK2(JIJ,JK) 
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ...
    / CSTURB%XCTD * CSTURB%XCTV * ZWORK2(JIJ,JK)
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ...
    / PSQRT_TKE(JIJ,JK)/CSTURB%XCTD*CSTURB%XCTV * ZWORK2(JIJ,JK)
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ...
    /PSQRT_TKE(JIJ,JK)/CSTURB%XCTD*CSTURB%XCTV**2 * ZWORK2(JIJ,JK) 
  ENDDO
ENDDO
!
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ...
    / PTKE(JIJ,JK) * CSTURB%XCTV**2 * ZWORK1(JIJ,JK)
  ENDDO
ENDDO
!
