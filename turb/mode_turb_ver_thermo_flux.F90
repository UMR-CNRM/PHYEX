...
ELSE
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZF(JIJ,JK) = -CSTURB%XCSHF*PPHI3(JIJ,JK)*ZKEFF(JIJ,JK)& 
      *ZWORK1(JIJ,JK)/PDZZ(JIJ,JK)
      ZDFDDTDZ(JIJ,JK) = -CSTURB%XCSHF*ZKEFF(JIJ,JK)*ZWORK2(JIJ,JK)
    ENDDO
  ENDDO
END IF
!
...
IF (OOCEAN) THEN    ! ocean model in coupled case
  DO JIJ=IIJB,IIJE  
    ...
    *0.5* ( 1. + PRHODJ(JIJ,D%NKU)/PRHODJ(JIJ,IKE) )
  ENDDO 
ELSE ! atmosp bottom
  ...
  IF (TURBN%CTURBDIM=='3DIM') THEN
    DO JIJ=IIJB,IIJE  
      ...
      * PDIRCOSZW(JIJ)                       &
      * 0.5 * (1. + PRHODJ(JIJ,D%NKA) / PRHODJ(JIJ,IKB))
    ENDDO 
  ELSE
    DO JIJ=IIJB,IIJE  
      ZF(JIJ,IKB) = ( TURBN%XIMPL*PSFTHP(JIJ) + PEXPL*PSFTHM(JIJ) )   &
...
IF (TURBN%LHGRAD) THEN
 DO JK=1,D%NKU
!  DO JIJ=IIJB,IIJE 
!  ZALT(JIJ,JK) = PZZ(JIJ,JK)-XZS(JIJ) !TODO TO BE ADDED AS ARGS OF TURB
!  ENDDO
 END DO
...
DO JK=IKTB+1,IKTE-1
  DO JIJ=IIJB,IIJE 
    PWTH(JIJ,JK)=0.5*(ZFLXZ(JIJ,JK)+ZFLXZ(JIJ,JK+D%NKL))
  ENDDO
END DO
!
DO JIJ=IIJB,IIJE 
  PWTH(JIJ,IKB)=0.5*(ZFLXZ(JIJ,IKB)+ZFLXZ(JIJ,IKB+D%NKL)) 
ENDDO
!
IF (OOCEAN) THEN
  DO JIJ=IIJB,IIJE 
    PWTH(JIJ,IKE)=0.5*(ZFLXZ(JIJ,IKE)+ZFLXZ(JIJ,IKE+D%NKL))
    PWTH(JIJ,D%NKA)=0.
    PWTH(JIJ,D%NKU)=PWTH(JIJ,IKE)! not used
  ENDDO
ELSE
  DO JIJ=IIJB,IIJE 
    PWTH(JIJ,D%NKA)=0.5*(ZFLXZ(JIJ,D%NKA)+ZFLXZ(JIJ,D%NKA+D%NKL))
    PWTH(JIJ,IKE)=PWTH(JIJ,IKE-D%NKL)
  ENDDO
END IF
!
...
IF (OOCEAN) THEN
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      PTP(JIJ,JK)= CST%XG*CST%XALPHAOC * ZWORK1(JIJ,JK)
    ENDDO
  ENDDO
ELSE
  IF (KRR /= 0) THEN
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        PTP(JIJ,JK)  =  PBETA(JIJ,JK) * ZWORK2(JIJ,JK)
      ENDDO
    ENDDO
    DO JIJ=IIJB,IIJE 
      PTP(JIJ,IKB)=  PBETA(JIJ,IKB) * PETHETA(JIJ,IKB) *   &
      0.5 * ( ZFLXZ(JIJ,IKB) + ZFLXZ(JIJ,IKB+D%NKL) )
    ENDDO
  ELSE
    CALL MZF_PHY(D,ZFLXZ,ZWORK1)
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        PTP(JIJ,JK)=  PBETA(JIJ,JK) * ZWORK1(JIJ,JK)
      ENDDO
    ENDDO
  END IF
END IF
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PWTHV(JIJ,JK) = ZWORK1(JIJ,JK) * ZFLXZ(JIJ,JK)
  ENDDO
ENDDO
DO JIJ=IIJB,IIJE 
...
IF (OLES_CALL) THEN
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK2(JIJ,JK) = PWM(JIJ,JK)*ZFLXZ(JIJ,JK)
    ENDDO
  ENDDO
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK3(JIJ,JK) = ZWORK2(JIJ,JK) * ZWORK1(JIJ,JK)
    ENDDO
  ENDDO
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK2(JIJ,JK) = PDTH_DZ(JIJ,JK)*ZFLXZ(JIJ,JK)
    ENDDO
  ENDDO
  CALL MZF_PHY(D,ZWORK2,ZWORK3)
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK3(JIJ,JK) = ZWORK2(JIJ,JK) * ZFLXZ(JIJ,JK)
    ENDDO
  ENDDO
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK2(JIJ,JK) = -CSTURB%XCTP*PSQRT_TKE(JIJ,JK)/PLM(JIJ,JK) &
      *ZWORK1(JIJ,JK)
    ENDDO
  ENDDO
  ...
  IF (KRR>=1) THEN
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK2(JIJ,JK) = PDR_DZ(JIJ,JK)*ZFLXZ(JIJ,JK)
      ENDDO
    ENDDO
    CALL MZF_PHY(D,ZWORK2,ZWORK3)
  ...
  ENDDO
  DO JIJ=IIJB,IIJE 
    ZA(JIJ,IKB) = CSTURB%XCSHF*PPHI3(JIJ,IKB)*ZKEFF(JIJ,IKB)
  ENDDO
...
IF (KRR /= 0) THEN
 ...
 ELSE
  CALL D_PSI3DRDZ_O_DDRDZ(D,CSTURB,PPSI3,PREDR1,PREDTH1,PRED2R3,PRED2THR3,TURBN%CTURBDIM,GUSERV,ZWORK2)
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE     
        ZF(JIJ,JK) = -CSTURB%XCSHF*PPSI3(JIJ,JK)*ZKEFF(JIJ,JK)& 
        *ZWORK1(JIJ,JK)/PDZZ(JIJ,JK)
        ZDFDDRDZ(JIJ,JK) = -CSTURB%XCSHF*ZKEFF(JIJ,JK)*ZWORK2(JIJ,JK)
      ENDDO
    ENDDO    
 ENDIF
  !
...
!   END DO
  ...
  DO JK=IKTB+1,IKTE-1
    DO JIJ=IIJB,IIJE 
      PWRC(JIJ,JK)=0.5*(ZFLXZ(JIJ,JK)+ZFLXZ(JIJ,JK+D%NKL))
    ENDDO
  END DO
  DO JIJ=IIJB,IIJE 
    PWRC(JIJ,IKB)=0.5*(ZFLXZ(JIJ,IKB)+ZFLXZ(JIJ,IKB+D%NKL))
  ENDDO
  !
  IF (OOCEAN) THEN
    DO JIJ=IIJB,IIJE           
      PWRC(JIJ,IKE)=0.5*(ZFLXZ(JIJ,IKE)+ZFLXZ(JIJ,IKE+D%NKL))
      PWRC(JIJ,D%NKA)=0.
      PWRC(JIJ,IKE+1)=ZFLXZ(JIJ,IKE+1)
    ENDDO    
  ELSE
    DO JIJ=IIJB,IIJE 
      PWRC(JIJ,D%NKA)=0.5*(ZFLXZ(JIJ,D%NKA)+ZFLXZ(JIJ,D%NKA+D%NKL))
      PWRC(JIJ,IKE)=PWRC(JIJ,IKE-D%NKL)
    ENDDO    
  ENDIF
  !
  IF ( TURBN%LTURB_FLX .AND. TPFILE%LOPENED ) THEN
  ...
  IF (OOCEAN) THEN
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZA(JIJ,JK)=  -CST%XG*CST%XBETAOC  * ZWORK1(JIJ,JK)
      ENDDO
    ENDDO
  ELSE
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZA(JIJ,JK)   =  PBETA(JIJ,JK) * ZWORK2(JIJ,JK)
      ENDDO
    ENDDO
    DO JIJ=IIJB,IIJE 
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      PWTHV(JIJ,JK)=PWTHV(JIJ,JK) + ZWORK1(JIJ,JK) * ZFLXZ(JIJ,JK)
    ENDDO
  ENDDO
  DO JIJ=IIJB,IIJE 
 ...
 ELSE
  ...
  CALL MZM_PHY(D,ZWORK1,ZWORK3)
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZA(JIJ,JK)   = ZWORK2(JIJ,JK)/ PDZZ(JIJ,JK) * &
        (-PPHI3(JIJ,JK)*ZWORK3(JIJ,JK)) * CSTURB%XCSHF 
      ENDDO
    ENDDO
 ENDIF
  DO JIJ=IIJB,IIJE 
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK1(JIJ,JK) = PAMOIST(JIJ,JK) * 2.* PSRCM(JIJ,JK)
      ZWORK2(JIJ,JK) = PATHETA(JIJ,JK) * 2.* PSRCM(JIJ,JK)
    ENDDO
  ENDDO
  CALL MZM_PHY(D,ZWORK1,ZWORK3)
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZFLXZ(JIJ,JK) = ZWORK3(JIJ,JK)* ZFLXZ(JIJ,JK) &
      + ZWORK4(JIJ,JK)* ZA(JIJ,JK)
    ENDDO
  ENDDO
  DO JIJ=IIJB,IIJE 
    ZFLXZ(JIJ,D%NKA) = ZFLXZ(JIJ,IKB)
