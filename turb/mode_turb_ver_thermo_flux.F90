...
USE MODD_TURB_n,           ONLY: TURB_t, NCTP, NCED, NTURB_S
!
...
REAL, DIMENSION(D%NIJT,NTURB_S), INTENT(IN)          ::  PTURB_SPP ! SPP variables
!
!
...
ELSE
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF(JIJ, JK) = -PTURB_SPP(JIJ,NCED)*PPHI3(JIJ, JK)*ZKEFF(JIJ, JK)& 
                                    *ZWORK1(JIJ, JK)/PDZZ(JIJ, JK)
      ZDFDDTDZ(JIJ, JK) = -PTURB_SPP(JIJ,NCED)*ZKEFF(JIJ, JK)*ZWORK2(JIJ, JK)
    END DO
  END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZWKLES(JIJ, JK) = -PTURB_SPP(JIJ,NCTP)*PSQRT_TKE(JIJ, JK)/PLM(JIJ, JK)*ZMZF2D_WORK1(JIJ, JK)  
  END DO
END DO
...
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Rt_SBG_UaThl )
  ...
  END DO
  DO JIJ=IIJB, IIJE
    ZA(JIJ, IKB) = PTURB_SPP(JIJ,NCED)*PPHI3(JIJ, IKB)*ZKEFF(JIJ, IKB)
  END DO
...
IF (KRR /= 0) THEN
 ...
 ELSE
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF(JIJ, JK) = -PTURB_SPP(JIJ,NCED)*PPSI3(JIJ, JK)*ZKEFF(JIJ, JK)& 
                                    *ZWORK1(JIJ, JK)/PDZZ(JIJ, JK)
      ZDFDDRDZ(JIJ, JK) = -PTURB_SPP(JIJ,NCED)*ZKEFF(JIJ, JK)*ZWORK2(JIJ, JK)
    END DO
  END DO
...
! yet modify to use length scale and tke at half levels from vdfexcuhl
 ...
 ELSE
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZA(JIJ, JK)   = ZWORK2(JIJ, JK)/ PDZZ(JIJ, JK) * &
                                      (-PPHI3(JIJ, JK)*ZWORK3(JIJ, JK)) * PTURB_SPP(JIJ,NCED)
    END DO
  END DO
