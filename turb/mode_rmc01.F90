...
IMPLICIT NONE
CONTAINS
SUBROUTINE RMC01(D,CST,CSTURB,TURBN,PZZ,PDXX,PDYY,PDZZ,PDIRCOSZW,PSBL_DEPTH,PLMO,PTURB_SPP,PLK,PLEPS)
...
USE MODD_TURB_n, ONLY: TURB_t, NCSHF, NCMFS, NTURB_S
!
...
REAL, DIMENSION(MERGE(D%NIJT,0,TURBN%LRMC01)), INTENT(IN) :: PSBL_DEPTH! SBL depth
...
DO JK=1,IKT
  DO JIJ=IIJB, IIJE
    ZL(JIJ, JK) =  CST%XKARMAN/SQRT(CSTURB%XALPSBL)/PTURB_SPP(JIJ,NCMFS)                                      &
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZL(JIJ, JK) = ZL(JIJ, JK) * (CSTURB%XALPSBL**(3./2.)*CST%XKARMAN*PTURB_SPP(JIJ,NCSHF)) &
            / (CST%XKARMAN/SQRT(CSTURB%XALPSBL)/PTURB_SPP(JIJ,NCMFS))
  END DO
END DO
!
DO JK=1, IKT
