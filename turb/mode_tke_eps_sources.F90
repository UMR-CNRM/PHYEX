...
ZA(:,:) = - PTSTEP * CSTURB%XCET * ZMZM2D_WORK1(:, :) * ZMZM2D_WORK2(:, :) / PDZZ(:,:)**2
...
!$mnh_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
ZSHUGRADWK1_2D(:, :) = TURBN%XIMPL * ZRES(:,:) + PEXPL * PTKEM(:,:)
!$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
...
!$mnh_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
ZSHUGRADWK1_2D(:, :) = ZMZM2D_WORK1(:, :) * ZFLX(:,:) / PDZZ(:,:)
!$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
