...
ELSE
  ...
  DO JIJ=IIJB,IIJE 
    PDP(JIJ,IKB) = PDP(JIJ,IKB) * (1. + PDZZ(JIJ,IKB+D%NKL)/PDZZ(JIJ,IKB))
  ENDDO
END IF
!
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ZFLX(JIJ,JK) = CSTURB%XCED * SQRT(PTKEM(JIJ,JK)) / PLEPS(JIJ,JK)
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ZA(JIJ,JK)     = - PTSTEP * CSTURB%XCET * ZMWORK1(JIJ,JK) & 
    * ZMWORK2(JIJ,JK) / PDZZ(JIJ,JK)**2
  ENDDO
ENDDO
!
...
IF(HPROGRAM/='MESONH') THEN
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      GTKENEG(JIJ,JK) =  ZRES(JIJ,JK) <= CSTURB%XTKEMIN
      IF ( GTKENEG(JIJ,JK) ) THEN
        ZRES(JIJ,JK) = CSTURB%XTKEMIN
      ENDIF
    ENDDO
  ENDDO
END IF
...
!
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZDWORK1(JIJ,JK) = TURBN%XIMPL * ZRES(JIJ,JK) + PEXPL * PTKEM(JIJ,JK)
    ENDDO
  ENDDO
  CALL DZM_PHY(D,ZDWORK1,ZDWORK2)
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZFLX(JIJ,JK)  = - CSTURB%XCET * ZMWORK1(JIJ,JK) &
      * ZDWORK2(JIJ,JK) / PDZZ(JIJ,JK)
    ENDDO
  ENDDO
!
...
!
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZMWORK2(JIJ,JK) = ZMWORK1(JIJ,JK) * ZFLX(JIJ,JK) &
      / PDZZ(JIJ,JK)
    ENDDO
  ENDDO
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZTR(JIJ,JK)= ZTR(JIJ,JK) - ZDWORK1(JIJ,JK) &
      /PRHODJ(JIJ,JK)
    ENDDO
  ENDDO
!
...
IF (BUCONF%LBUDGET_TKE) THEN
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZMWORK1(JIJ,JK) = -CSTURB%XCED * SQRT(PTKEM(JIJ,JK))/PLEPS(JIJ,JK) * &
...
#ifdef REPRO48
    IF (BUCONF%LBUDGET_TKE) THEN
    PRTKES(JIJ,JK) = PRTKES(JIJ,JK) + PDP(JIJ,JK) * PRHODJ(JIJ,JK)
    PRTKES(JIJ,JK) = PRTKES(JIJ,JK) + PTP(JIJ,JK) * PRHODJ(JIJ,JK)
    PRTKES(JIJ,JK) = PRTKES(JIJ,JK) - CSTURB%XCED * SQRT(PTKEM(JIJ,JK)) / PLEPS(JIJ,JK) * &
    (PEXPL*PTKEM(JIJ,JK) + TURBN%XIMPL*ZRES(JIJ,JK)) * PRHODJ(JIJ,JK)
  END IF
#else
  PRTKES(JIJ,JK) = PRTKES(JIJ,JK) + PRHODJ(JIJ,JK) * &
  ( PDP(JIJ,JK) + PTP(JIJ,JK)                           &
  - CSTURB%XCED * SQRT(PTKEM(JIJ,JK)) / PLEPS(JIJ,JK) &
  * ( PEXPL*PTKEM(JIJ,JK) + TURBN%XIMPL*ZRES(JIJ,JK) ) )
#endif
!
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PDISS(JIJ,JK) =  -CSTURB%XCED * (PTKEM(JIJ,JK)**1.5) / PLEPS(JIJ,JK)
  ENDDO
ENDDO
END IF
!
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    PEDR(JIJ,JK) = CSTURB%XCED * (PTKEM(JIJ,JK)**1.5) / PLEPS(JIJ,JK)
  ENDDO
ENDDO
END IF
!
