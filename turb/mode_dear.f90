...
!$mnh_end_expand_array ( JIJ=IIJB:IIJE )
    ...
    IF (OOCEAN) THEN
!$mnh_expand_array ( JIJ=IIJB:IIJE )
      PWORK2D(:) = CST%XG*(CST%XALPHAOC*ZDTHLDZ(:, IKB) - CST%XBETAOC*ZDRTDZ(:, IKB))
!$mnh_end_expand_array ( JIJ=IIJB:IIJE )
    ELSE
!$mnh_expand_array ( JIJ=IIJB:IIJE )
      PWORK2D(:) = CST%XG / PTHVREF(:, IKB)*(ZETHETA(:, IKB)*ZDTHLDZ(:, IKB) + &
                           ZEMOIST(:,IKB)*ZDRTDZ(:, IKB))
!$mnh_end_expand_array ( JIJ=IIJB:IIJE )
    END IF
!$mnh_expand_where ( JIJ=IIJB:IIJE )
    WHERE (PWORK2D(:) > 0.)
      PLM(:, IKB) =  &
      & MAX(CST%XMNH_EPSILON, MIN(PLM(:, IKB), 0.76*SQRT(PTKET(:, IKB) / PWORK2D(:))))
    END WHERE
