...
SUBROUTINE TURB_VER_THERMO_CORR(D,CST,CSTURB,TURBN,NEBN,TLES,       &
                      ...
                      PTHLP,PRP,MFMOIST,PTURB_SPP,PSIGS             )
!     ###############################################################
!
!
...
USE MODE_PRANDTL, ONLY: D_M3_R2_W2R_O_DDRDZ, D_M3_R2_W2TH_O_DDRDZ, &
                        D_M3_R2_WR2_O_DDRDZ, D_M3_R2_WTH2_O_DDRDZ, D_M3_R2_WTHR_O_DDRDZ, &
                        D_M3_TH2_W2R_O_DDTDZ, D_M3_TH2_W2TH_O_DDTDZ, D_M3_TH2_WR2_O_DDTDZ, &
                        D_M3_TH2_WTH2_O_DDTDZ, D_M3_TH2_WTHR_O_DDTDZ, D_M3_THR_W2R_O_DDRDZ, &
                        D_M3_THR_W2R_O_DDTDZ, D_M3_THR_W2TH_O_DDRDZ, D_M3_THR_W2TH_O_DDTDZ, &
                        D_M3_THR_WR2_O_DDRDZ, D_M3_THR_WR2_O_DDTDZ, D_M3_THR_WTH2_O_DDRDZ, &
                        D_M3_THR_WTH2_O_DDTDZ, D_M3_THR_WTHR_O_DDRDZ, D_M3_THR_WTHR_O_DDTDZ, &
                        D_PHI3DRDZ_O_DDRDZ, D_PHI3DTDZ_O_DDTDZ, D_PSI3DRDZ2_O_DDRDZ, &
                        D_PSI3DRDZ_O_DDRDZ, D_PSI3DTDZ_O_DDTDZ, &
                        M3_R2_W2R, M3_R2_W2TH, M3_R2_WR2, M3_R2_WTH2, M3_R2_WTHR, M3_TH2_W2R, &
                        M3_TH2_W2TH, M3_TH2_WR2, M3_TH2_WTH2, M3_TH2_WTHR, M3_THR_W2R, M3_THR_W2TH, &
                        M3_THR_WR2, M3_THR_WTH2, M3_THR_WTHR
!
...
!
REAL, DIMENSION(D%NIJT) :: ZCHT1, ZCHT2
REAL :: ZTIME1, ZTIME2
!
...
!
  ...
  IF (GFTH2) THEN
    ...
    CALL D_M3_TH2_WTH2_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,&
     & PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,ZWORK2,PTURB_SPP)
    !
...
IF (KRR/=0) THEN
    ...
    IF (GFR2) THEN
      CALL M3_TH2_WR2(D,CSTURB,TURBN,PD,PLEPS,PSQRT_TKE,PBLL_O_E,&
       & PEMOIST,PDTH_DZ,ZWORK1,PTURB_SPP)
      CALL D_M3_TH2_WR2_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,&
       & PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTH_DZ,ZWORK2,PTURB_SPP)
      !
    ...
    CALL MZF_PHY(D,PFWR,ZWORK2)
      CALL D_M3_TH2_W2R_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,&
       & PLM,PLEPS,PTKEM,PBLL_O_E,PEMOIST,PDTH_DZ,ZWORK3,PTURB_SPP)
      !
    ...
    IF (GFTHR) THEN
      CALL M3_TH2_WTHR(D,CSTURB,TURBN,PREDR1,PD,PLEPS,PSQRT_TKE,&
       & PBLL_O_E,PEMOIST,PDTH_DZ,ZWORK1,PTURB_SPP)
       CALL D_M3_TH2_WTHR_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,&
       & PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTH_DZ,ZWORK2,PTURB_SPP)
      !
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_W_SBG_Thl2 )
    ZWKLES(:,:) = -2.*CSTURB%XCTD*PSQRT_TKE(:,:)*ZFLXZ(:,:)/PLEPS(:,:)
    CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_DISS_Thl2 )
    ZWKLES(:,:) = PETHETA(:,:)*ZFLXZ(:,:)
    CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_ThlThv )
    ZWKLES(:,:) = -CSTURB%XA3*PBETA(:,:)*PETHETA(:,:)*ZFLXZ(:,:)
...
ENDIF
    ...
    IF (GFTH2) THEN
      CALL M3_THR_WTH2(D,CSTURB,TURBN,PREDR1,PD,PLEPS,PSQRT_TKE,&
       & PBLL_O_E,PETHETA,PDR_DZ,ZWORK1,PTURB_SPP)
      CALL D_M3_THR_WTH2_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,&
       & PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDR_DZ,ZWORK2,PTURB_SPP)
      CALL D_M3_THR_WTH2_O_DDRDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,&
       & PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,ZWORK3,PTURB_SPP)
      !
    ...
    IF (GFWTH) THEN
      ...
      CALL M3_THR_W2TH(D,CSTURB,TURBN,PREDR1,PD,PLM,PLEPS,PTKEM,&
       & PDR_DZ,ZWORK2,PTURB_SPP)
      CALL D_M3_THR_W2TH_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,&
       & PD,PLM,PLEPS,PTKEM,PBLL_O_E,PDR_DZ,PETHETA,ZWORK3,PTURB_SPP)
      CALL D_M3_THR_W2TH_O_DDRDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,&
       & PD,PLM,PLEPS,PTKEM,ZWORK4,PTURB_SPP)
      !
    ...
    IF (GFR2) THEN
      ...
      CALL D_M3_THR_WR2_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,&
       & PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,ZWORK2,PTURB_SPP)
      CALL D_M3_THR_WR2_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,&
       & PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTH_DZ,ZWORK3,PTURB_SPP)
      !
    ...
    IF (GFTHR) THEN
      ...
      & PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,ZWORK3,PTURB_SPP)
      !
...
ENDIF
    ...
    IF (NEBN%LSTATNW) THEN
      ...
      DO JIJ=IIJB, IIJE
        ZFLXZ(JIJ, IKB) = (ZCHT1(JIJ) + ZCHT2(JIJ)) * ZFLXZ(JIJ, IKB)
      END DO
    ...
    ELSE
      ...
      DO JIJ=IIJB, IIJE
        ZFLXZ(JIJ, IKB) =                                                            & 
        (ZCHT1(JIJ) * PPHI3(JIJ, IKB+IKL) + ZCHT2(JIJ) * PPSI3(JIJ, IKB+IKL))   &
        *( PEXPL *                                                                              &
           ( ZCOEFF(JIJ, IKB+2*IKL)*PTHLM(JIJ, IKB+2*IKL)             &
            +ZCOEFF(JIJ, IKB+IKL)*PTHLM(JIJ, IKB+IKL)             & 
            +ZCOEFF(JIJ, IKB)*PTHLM(JIJ, IKB))                &
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_W_SBG_ThlRt )
      ZWKLES(:,:) = -2.*CSTURB%XCTD*PSQRT_TKE(:,:)*ZFLXZ(:,:)/PLEPS(:,:)
      CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_DISS_ThlRt )
      ZWKLES(:,:) = PETHETA(:,:)*ZFLXZ(:,:)
      CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_RtThv )
      ZWKLES(:,:) = -CSTURB%XA3*PBETA(:,:)*PETHETA(:,:)*ZFLXZ(:,:)
      CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_RtPz, .TRUE. )
      ZWKLES(:,:) = PEMOIST(:,:)*ZFLXZ(:,:)
      CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_ThlThv , .TRUE. )
      ZWKLES(:,:) = -CSTURB%XA3*PBETA(:,:)*PEMOIST(:,:)*ZFLXZ(:,:)
      CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_ThlPz, .TRUE. )
      !
      CALL SECOND_MNH(ZTIME2)
      TLES%XTIME_LES = TLES%XTIME_LES + ZTIME2 - ZTIME1
...
ENDIF
    ...
    IF (GFR2) THEN
      ...
      & PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,ZWORK2,PTURB_SPP)
      !
    ...
    IF (GFWR) THEN
      ...
      & PLM,PLEPS,PTKEM,ZWORK2,PTURB_SPP)
      CALL D_M3_R2_W2R_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,&
      & PD,PLM,PLEPS,PTKEM,GUSERV,ZWORK3,PTURB_SPP)
      !
    ...
    IF (KRR/=0) THEN
      ...
      IF (GFTH2) THEN
        ...
        CALL D_M3_R2_WTH2_O_DDRDZ(D,CSTURB,TURBN,PREDR1,&
          & PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDR_DZ,ZWORK2,PTURB_SPP)
        !
      ...
      CALL MZF_PHY(D,PFWTH,ZWORK1)
        CALL M3_R2_W2TH(D,CSTURB,TURBN,PD,PLM,PLEPS,PTKEM,&
          & PBLL_O_E,PETHETA,PDR_DZ,ZWORK2,PTURB_SPP)
        CALL D_M3_R2_W2TH_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,&
          & PD,PLM,PLEPS,PTKEM,PBLL_O_E,PETHETA,PDR_DZ,ZWORK3,PTURB_SPP)
        !
      ...
      IF (GFTHR) THEN
        CALL M3_R2_WTHR(D,CSTURB,TURBN,PREDTH1,PD,PLEPS,&
          & PSQRT_TKE,PBLL_O_E,PETHETA,PDR_DZ,ZWORK1,PTURB_SPP)
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_W_SBG_Rt2 )
      ZWKLES(:,:) = PEMOIST(:,:)*ZFLXZ(:,:)
      CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_RtThv , .TRUE. )
      ZWKLES(:,:) = -CSTURB%XA3*PBETA(:,:)*PEMOIST(:,:)*ZFLXZ(:,:)
      CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_RtPz, .TRUE. )
      ZWKLES(:,:) = -2.*CSTURB%XCTD*PSQRT_TKE(:,:)*ZFLXZ(:,:)/PLEPS(:,:)
      CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_DISS_Rt2 )
      !
      CALL SECOND_MNH(ZTIME2)
      TLES%XTIME_LES = TLES%XTIME_LES + ZTIME2 - ZTIME1
    END IF
