...
!  ground
DO JIJ=IIJB,IIJE 
  ZCOEFF(JIJ,IKB+2*D%NKL)= - PDZZ(JIJ,IKB+D%NKL) /      &
  ( (PDZZ(JIJ,IKB+2*D%NKL)+PDZZ(JIJ,IKB+D%NKL)) * PDZZ(JIJ,IKB+2*D%NKL) )
  ZCOEFF(JIJ,IKB+D%NKL)=   (PDZZ(JIJ,IKB+2*D%NKL)+PDZZ(JIJ,IKB+D%NKL)) /      &
  ( PDZZ(JIJ,IKB+D%NKL) * PDZZ(JIJ,IKB+2*D%NKL) )
  ZCOEFF(JIJ,IKB)= - (PDZZ(JIJ,IKB+2*D%NKL)+2.*PDZZ(JIJ,IKB+D%NKL)) /      &
  ( (PDZZ(JIJ,IKB+2*D%NKL)+PDZZ(JIJ,IKB+D%NKL)) * PDZZ(JIJ,IKB+D%NKL) )
ENDDO
!
!
IF (TURBN%LHARAT) THEN
  ...
  DO JIJ=IIJB,IIJE 
    PLMF(JIJ,D%NKT)=0.001
    PLEPSF(JIJ,D%NKT)=0.001
  ENDDO
...
!
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZF(JIJ,JK) = CSTURB%XCTV*PLM(JIJ,JK)*PLEPS(JIJ,JK)& 
      * ZWORK2(JIJ,JK)
    ENDDO
  ENDDO
  ENDIF
...
DO JK=1,D%NKT 
  DO JIJ=IIJB,IIJE 
    ZWORK3(JIJ,JK) = ZWORK2(JIJ,JK) / PDZZ(JIJ,JK)
  ENDDO
ENDDO
...
ENDDO
  ...
  DO JIJ=IIJB,IIJE 
    ...
    * PLEPSF(JIJ,IKB)                                         &
    *( PEXPL *                                                  &
    ( ZCOEFF(JIJ,IKB+2*D%NKL)*PTHLM(JIJ,IKB+2*D%NKL)             &
    +ZCOEFF(JIJ,IKB+D%NKL)*PTHLM(JIJ,IKB+D%NKL)             & 
    +ZCOEFF(JIJ,IKB)*PTHLM(JIJ,IKB)   )**2          &
    +TURBN%XIMPL *                                                  &
    ( ZCOEFF(JIJ,IKB+2*D%NKL)*PTHLP(JIJ,IKB+2*D%NKL)             &
    +ZCOEFF(JIJ,IKB+D%NKL)*PTHLP(JIJ,IKB+D%NKL)             &
    +ZCOEFF(JIJ,IKB)*PTHLP(JIJ,IKB)   )**2          &
    ) 
  ENDDO
    IF (TURBN%LSTATNW) THEN
    DO JIJ=IIJB,IIJE 
      ZFLXZ(JIJ,IKB) = CSTURB%XCTV * ZFLXZ(JIJ,IKB)
    ENDDO
     END IF
  ELSE
  DO JIJ=IIJB,IIJE 
    ZFLXZ(JIJ,IKB) = CSTURB%XCTV * PPHI3(JIJ,IKB+D%NKL) * PLM(JIJ,IKB)   &
    * PLEPS(JIJ,IKB)                                         &
    *( PEXPL *                                                  &
    ( ZCOEFF(JIJ,IKB+2*D%NKL)*PTHLM(JIJ,IKB+2*D%NKL)             &
    +ZCOEFF(JIJ,IKB+D%NKL)*PTHLM(JIJ,IKB+D%NKL)             & 
...
!
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK2(JIJ,JK) = ZWORK1(JIJ,JK) * ZFLXZ(JIJ,JK)
    ENDDO
  ENDDO
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK1(JIJ,JK) = -2.*CSTURB%XCTD*PSQRT_TKE(JIJ,JK)*ZFLXZ(JIJ,JK) &
      / PLEPS(JIJ,JK)
    ENDDO
  ENDDO
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK1(JIJ,JK) = PETHETA(JIJ,JK)*ZFLXZ(JIJ,JK)
    ENDDO
  ENDDO
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK1(JIJ,JK) = -CSTURB%XA3*PBETA(JIJ,JK)*PETHETA(JIJ,JK) &
      * ZFLXZ(JIJ,JK)
    ENDDO
  ENDDO
...
!
  ...
  ELSE
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK1(JIJ,JK) = 0.5*(PPHI3(JIJ,JK)+PPSI3(JIJ,JK))& 
        *PDTH_DZ(JIJ,JK)*PDR_DZ(JIJ,JK)
      ENDDO
    ENDDO
    CALL MZF_PHY(D,ZWORK1,ZWORK2)
    DO JK=1,D%NKT 
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK1(JIJ,JK) = PTHLP(JIJ,JK) - PTHLM(JIJ,JK)
      ZWORK2(JIJ,JK) = PRP(JIJ,JK) - PRM(JIJ,JK,1)
    ENDDO
  ENDDO
  ...
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
      ZWORK1(JIJ,JK) = ZWORK3(JIJ,JK) / PDZZ(JIJ,JK)
      ZWORK2(JIJ,JK) = ZWORK4(JIJ,JK) / PDZZ(JIJ,JK)
    ENDDO
  ENDDO
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE   
        ZWORK5(JIJ,JK) = 2. *PDR_DZ(JIJ,JK)  *ZWORK3(JIJ,JK) &
        / PDZZ(JIJ,JK)               &
        + 2. *PDTH_DZ(JIJ,JK) *ZWORK4(JIJ,JK) / PDZZ(JIJ,JK)
      ENDDO
    ENDDO
      !
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ...
        * ZWORK5(JIJ,JK)                                       &
        + TURBN%XIMPL * ZDFDDTDZ(JIJ,JK) * ZWORK7(JIJ,JK) &
        + TURBN%XIMPL * ZDFDDRDZ(JIJ,JK) * ZWORK8(JIJ,JK)
      ENDDO
    ENDDO
      IF (TURBN%LSTATNW) THEN    
      DO JK=1,D%NKT 
        DO JIJ=IIJB,IIJE 
          ZFLXZ(JIJ,JK)   = CSTURB%XCTV * ZFLXZ(JIJ,JK)
        ENDDO
      ENDDO
      END IF
    ELSE
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK5(JIJ,JK) = (ZWKPHIPSI1(JIJ,JK)+ZWKPHIPSI2(JIJ,JK))& 
        *PDR_DZ(JIJ,JK)*ZWORK3(JIJ,JK)/PDZZ(JIJ,JK) &
        + (ZWKPHIPSI3(JIJ,JK) + ZWKPHIPSI4(JIJ,JK)) & 
        *PDTH_DZ(JIJ,JK)*ZWORK4(JIJ,JK)/PDZZ(JIJ,JK)
      ENDDO
    ENDDO
      CALL MZF_PHY(D,ZWORK5,ZWORK6)      
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZFLXZ(JIJ,JK)   = ZF(JIJ,JK)                          &
        + TURBN%XIMPL * CSTURB%XCTV*PLM(JIJ,JK)*PLEPS(JIJ,JK)*0.5 &
        * ZWORK6(JIJ,JK)                                                   &
        + TURBN%XIMPL * ZDFDDTDZ(JIJ,JK) * ZWORK7(JIJ,JK)         &
        + TURBN%XIMPL * ZDFDDRDZ(JIJ,JK) * ZWORK8(JIJ,JK)
      ENDDO
    ...
    DO JIJ=IIJB,IIJE 
      ...
      *( ZCOEFF(JIJ,IKB+2*D%NKL)*PRM(JIJ,IKB+2*D%NKL,1)             &
      +ZCOEFF(JIJ,IKB+D%NKL)*PRM(JIJ,IKB+D%NKL,1)             & 
      +ZCOEFF(JIJ,IKB)*PRM(JIJ,IKB,1))                &
      +TURBN%XIMPL *                                                                              &
      ( ZCOEFF(JIJ,IKB+2*D%NKL)*PTHLP(JIJ,IKB+2*D%NKL)             &
      +ZCOEFF(JIJ,IKB+D%NKL)*PTHLP(JIJ,IKB+D%NKL)             &
      +ZCOEFF(JIJ,IKB)*PTHLP(JIJ,IKB))                &
      *( ZCOEFF(JIJ,IKB+2*D%NKL)*PRP(JIJ,IKB+2*D%NKL)             &
      +ZCOEFF(JIJ,IKB+D%NKL)*PRP(JIJ,IKB+D%NKL)             & 
      +ZCOEFF(JIJ,IKB)*PRP(JIJ,IKB))                &
      )
    ENDDO
    IF (TURBN%LSTATNW) THEN
      DO JIJ=IIJB,IIJE 
        ZFLXZ(JIJ,IKB) = (CSTURB%XCHT1 + CSTURB%XCHT2) * ZFLXZ(JIJ,IKB)
      ENDDO
    END IF
    ELSE 
    DO JIJ=IIJB,IIJE 
      ZFLXZ(JIJ,IKB) =                                                            & 
      (CSTURB%XCHT1 * PPHI3(JIJ,IKB+D%NKL) + CSTURB%XCHT2 * PPSI3(JIJ,IKB+D%NKL))   &
      *( PEXPL *                                                                              &
      ( ZCOEFF(JIJ,IKB+2*D%NKL)*PTHLM(JIJ,IKB+2*D%NKL)             &
      +ZCOEFF(JIJ,IKB+D%NKL)*PTHLM(JIJ,IKB+D%NKL)             & 
      +ZCOEFF(JIJ,IKB)*PTHLM(JIJ,IKB))                &
...
IF (OLES_CALL) THEN
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK1(JIJ,JK) = PEMOIST(JIJ,JK)*ZFLXZ(JIJ,JK)
      ENDDO
    ENDDO
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK1(JIJ,JK) = -CSTURB%XA3*PBETA(JIJ,JK)*PEMOIST(JIJ,JK) &
        * ZFLXZ(JIJ,JK)
      ENDDO
    ENDDO
      CALL LES_MEAN_SUBGRID_PHY(D,ZWORK1, X_LES_SUBGRID_ThlPz, .TRUE. )
...
IF (TURBN%LHARAT) THEN
  ...
  IF (TURBN%LSTATNW) THEN
      DO JK=1,D%NKT 
        DO JIJ=IIJB,IIJE 
          ZF(JIJ,JK) = CSTURB%XCTV * ZF(JIJ,JK)
        ENDDO
      ENDDO
  END IF
ELSE
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK1(JIJ,JK) = PPSI3(JIJ,JK)*PDR_DZ(JIJ,JK)**2
      ENDDO
    ENDDO
  CALL MZF_PHY(D,ZWORK1,ZWORK2)
    DO JK=1,D%NKT 
...
ENDIF
  ...
  IF (TURBN%LHARAT) THEN
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK5(JIJ,JK) = ZWORK2(JIJ,JK) / PDZZ(JIJ,JK)
        ZWORK3(JIJ,JK) = 2.*PDR_DZ(JIJ,JK)* ZWORK5(JIJ,JK)
      ENDDO
    ENDDO
    CALL MZF_PHY(D,ZWORK3,ZWORK4)
    CALL MZF_PHY(D,ZWORK5,ZWORK6)
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ...
        * ZWORK4(JIJ,JK) &
        + TURBN%XIMPL * ZDFDDRDZ(JIJ,JK) * ZWORK6(JIJ,JK)
      ENDDO
    ENDDO
    IF (TURBN%LSTATNW) THEN
      DO JK=1,D%NKT 
  ...
  ELSE
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK1(JIJ,JK) = ZWKPHIPSI1(JIJ,JK)*ZWORK2(JIJ,JK) &
        / PDZZ(JIJ,JK)
      ENDDO
    ENDDO
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZWORK4(JIJ,JK) = ZWORK2(JIJ,JK) / PDZZ(JIJ,JK)
      ENDDO
    ENDDO
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZFLXZ(JIJ,JK) = ZF(JIJ,JK)                             &
        + TURBN%XIMPL * CSTURB%XCTV*PLM(JIJ,JK) *PLEPS(JIJ,JK) &
        * ZWORK3(JIJ,JK) &
        + TURBN%XIMPL * ZDFDDRDZ(JIJ,JK) * ZWORK5(JIJ,JK)
      ENDDO
    ENDDO  
  ENDIF
    !
  ...
  IF (TURBN%LHARAT) THEN
    ...
    IF (TURBN%LSTATNW) THEN
      DO JIJ=IIJB,IIJE 
        ZFLXZ(JIJ,IKB) = CSTURB%XCHV * ZFLXZ(JIJ,IKB)
      ENDDO
    END IF 
  ELSE
    DO JIJ=IIJB,IIJE 
      ZFLXZ(JIJ,IKB) = CSTURB%XCHV * PPSI3(JIJ,IKB+D%NKL) * PLM(JIJ,IKB)   &
      * PLEPS(JIJ,IKB)                                        &
      *( PEXPL *                                                  &
      ( ZCOEFF(JIJ,IKB+2*D%NKL)*PRM(JIJ,IKB+2*D%NKL,1)             &
      +ZCOEFF(JIJ,IKB+D%NKL)*PRM(JIJ,IKB+D%NKL,1)             & 
...
!
  ...
  DO JIJ=IIJB,IIJE 
    PSIGS(JIJ,D%NKA) = PSIGS(JIJ,IKB)
    PSIGS(JIJ,D%NKU) = PSIGS(JIJ,IKE)
  ENDDO
  DO JK=1,D%NKT 
    DO JIJ=IIJB,IIJE 
#ifdef REPRO48
      PSIGS(JIJ,JK) =  MAX (PSIGS(JIJ,JK) , 0.)
      PSIGS(JIJ,JK) =  SQRT(PSIGS(JIJ,JK))
#else
      PSIGS(JIJ,JK) =  SQRT( MAX (PSIGS(JIJ,JK) , 1.E-12) )
#endif
    ENDDO
  ENDDO
  END IF
