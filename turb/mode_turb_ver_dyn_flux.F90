...
IF (OOCEAN) THEN  ! Ocean model
  ...
  DO JIJ=IIJB,IIJE   
    ...
    *0.5 * ( 1. + ZWORK1(JIJ,D%NKU) / ZWORK1(JIJ,IKE)) 
  ENDDO
  !
...
!
  ...
  DO JIJ=IIJB,IIJE 
    ...
    ) * 0.5 * ( 1. + ZWORK1(JIJ,D%NKA) / ZWORK1(JIJ,IKB) )
  ENDDO
!
...
ELSE
  ...
  DO JIJ=IIJB,IIJE 
    ...
    ) / 0.5 / ( 1. + ZWORK2(JIJ,D%NKA)/ ZWORK2(JIJ,IKB) )
  !
...
IF (OOCEAN) THEN
  ...
  DO JIJ=IIJB,IIJE   
    ZWORK2(JIJ,IKE) = ZFLXZ(JIJ,IKE) * (PUM(JIJ,IKE)-PUM(JIJ,IKE-D%NKL))  &
    / ZWORK1(JIJ,IKE-D%NKL)
  ENDDO                 
  CALL MXF_PHY(D,ZWORK2,ZWORK3)
  DO JIJ=IIJB,IIJE   
...
ELSE ! Atmosphere
  ...
  DO JIJ=IIJB,IIJE   
    ZWORK2(JIJ,IKB) = ZFLXZ(JIJ,IKB+D%NKL) * (PUM(JIJ,IKB+D%NKL)-PUM(JIJ,IKB))  &
    / ZWORK1(JIJ,IKB+D%NKL)
  ENDDO                   
  CALL MXF_PHY(D,ZWORK2,ZWORK3)
  DO JIJ=IIJB,IIJE 
...
IF(TURBN%CTURBDIM=='3DIM') THEN
  ...
  DO JIJ=IIJB,IIJE 
    ZFLXZ(JIJ,D%NKA) = 2 * ZFLXZ(JIJ,IKB) - ZFLXZ(JIJ,IKB+D%NKL) ! extrapolation 
  ENDDO
 IF (OOCEAN) THEN
    DO JIJ=IIJB,IIJE 
      ZFLXZ(JIJ,D%NKU) = 2 * ZFLXZ(JIJ,IKE) - ZFLXZ(JIJ,IKE-D%NKL) ! extrapolation
    ENDDO
 END IF
  !
  CALL MXM_PHY(D,PRHODJ,ZWORK1)
  ...
  IF (OOCEAN) THEN
    ...
    DO JIJ=IIJB,IIJE 
      ...
      / (0.5*(PDXX(JIJ,IKE-D%NKL)+PDXX(JIJ,IKE)))
    ENDDO
  ...
  ELSE !Atmosphere
    ...
    !
    DO JIJ=IIJB,IIJE 
      ZWORK21D(JIJ) = (PWM(JIJ,IKB+2*D%NKL)-PWM(JIJ,IKB+D%NKL)) &
      / (PDZZ(JIJ,IKB+2*D%NKL)+PDZZ(JIJ,IKB+D%NKL))       &
      + (PWM(JIJ,IKB+D%NKL)-PWM(JIJ,IKB))                       &
      / (PDZZ(JIJ,IKB+D%NKL)+PDZZ(JIJ,IKB)) 
    ENDDO
    !
    CALL MXM2D_PHY(D,ZWORK21D,ZWORK51D)
    DO JIJ=IIJB,IIJE 
      ZWORK31D(JIJ) = - ZFLXZ(JIJ,IKB+D%NKL) &
      * ( ZWORK1(JIJ,IKB+D%NKL) - ZWORK51D(JIJ) &
      *   PDZX(JIJ,IKB+D%NKL) ) &
      / (0.5*(PDXX(JIJ,IKB+D%NKL)+PDXX(JIJ,IKB)))
    ENDDO
    CALL MXF2D_PHY(D,ZWORK31D,ZWORK41D)
...
IF (OOCEAN) THEN
  ...
  DO JIJ=IIJB,IIJE 
    ZWORK2(JIJ,IKE) = ZFLXZ(JIJ,IKE) * (PVM(JIJ,IKE)-PVM(JIJ,IKE-D%NKL))  &
    / ZWORK1(JIJ,IKE-D%NKL)
  ENDDO
  CALL MYF_PHY(D,ZWORK2,ZWORK3)
  DO JIJ=IIJB,IIJE 
...
ELSE ! Atmosphere
  ! evaluate the dynamic production at w(IKB+D%NKL) in PDP(IKB)
  DO JIJ=IIJB,IIJE 
    ZWORK2(JIJ,IKB) = ZFLXZ(JIJ,IKB+D%NKL) * (PVM(JIJ,IKB+D%NKL)-PVM(JIJ,IKB))  &
    / ZWORK1(JIJ,IKB+D%NKL)
  ENDDO
  CALL MYF_PHY(D,ZWORK2,ZWORK3)
  DO JIJ=IIJB,IIJE 
...
IF(TURBN%CTURBDIM=='3DIM') THEN
  ...
  IF (OOCEAN) THEN
    DO JIJ=IIJB,IIJE 
      ZFLXZ(JIJ,IKE+D%NKL) = 2 * ZFLXZ(JIJ,IKE) - ZFLXZ(JIJ,IKE-D%NKL) ! extrapolation 
    ENDDO
  ELSE
    DO JIJ=IIJB,IIJE 
      ZFLXZ(JIJ,D%NKA) = 2 * ZFLXZ(JIJ,IKB) - ZFLXZ(JIJ,IKB+D%NKL) ! extrapolation
  ...
  IF (.NOT. O2D) THEN
    ...
    IF (OOCEAN) THEN
      ...
      DO JIJ=IIJB,IIJE 
        ZWORK31D(JIJ) = - ZFLXZ(JIJ,IKE) *  ZWORK1(JIJ,IKE) &
        / (0.5*(PDYY(JIJ,IKE-D%NKL)+PDYY(JIJ,IKE)))
      ENDDO
    ...
    ELSE ! Atmosphere
      ...
      DO JIJ=IIJB,IIJE 
        ...
        * ( ZWORK1(JIJ,IKB+D%NKL) - ZWORK51D(JIJ) &
        *   PDZY(JIJ,IKB+D%NKL) ) &
        / (0.5*(PDYY(JIJ,IKB+D%NKL)+PDYY(JIJ,IKB)))
      ENDDO
      CALL MYF2D_PHY(D,ZWORK31D,ZWORK41D)
      ZA(IIJB:IIJE,IKB) = ZWORK41D(IIJB:IIJE)
    !
