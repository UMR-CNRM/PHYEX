...
DO JSV=1,KSV
  ...
  IF (OLES_CALL) THEN
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZFLXZ(JIJ,JK) =  PPSI_SV(JIJ,JK,JSV)*ZWORK1(JIJ,JK)**2
        ZFLXZ(JIJ,JK) = ZCSV / ZCSVD * PLM(JIJ,JK) * PLEPS(JIJ,JK) * ZWORK2(JIJ,JK)
        ZWORK1(JIJ,JK) = -2.*ZCSVD*SQRT(PTKEM(JIJ,JK))*ZFLXZ(JIJ,JK)/PLEPS(JIJ,JK)
        ZWORK2(JIJ,JK) = ZWORK3(JIJ,JK)*ZFLXZ(JIJ,JK)
      ENDDO
    ENDDO
  ...
  IF (OLES_CALL) THEN
    ...
    DO JK=1,D%NKT 
      DO JIJ=IIJB,IIJE 
        ZFLXZ(JIJ,JK)= ( CSTURB%XCSHF * PPHI3(JIJ,JK) + ZCSV * PPSI_SV(JIJ,JK,JSV) ) &
    ...
    IF (KRR>=1) THEN
      ...
      DO JK=1,D%NKT 
        DO JIJ=IIJB,IIJE 
          ZFLXZ(JIJ,JK)= PLM(JIJ,JK) * PLEPS(JIJ,JK) / (2.*ZCQSVD) * ZWORK3(JIJ,JK)
          ZWORK1(JIJ,JK) = ZA(JIJ,JK)*ZFLXZ(JIJ,JK)
          ZWORK2(JIJ,JK) = -CST%XG/PTHVREF(JIJ,JK)/3.*ZA(JIJ,JK)*ZFLXZ(JIJ,JK)
        ENDDO
      ENDDO
