...
!
ZWKLES(:,:) = -2.*ZCSVD*SQRT(PTKEM)*ZFLXZ/PLEPS
    CALL LES_MEAN_SUBGRID_PHY(D,TLES, ZWKLES, TLES%X_LES_SUBGRID_DISS_Sv2(:,:,:,JSV) )
...
!
CALL LES_MEAN_SUBGRID_PHY(D,TLES, ZWKLES, TLES%X_LES_RES_W_SBG_Sv2(:,:,:,JSV) )
  END IF
  !
  ! covariance ThvSv
  !
  IF (TLES%LLES_CALL) THEN
    ...
    CALL GZ_M_W_PHY(D, PTHLM,PDZZ, ZGZ_M_W2D_WORK1)
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZFLXZ(JIJ, JK)= PLM(JIJ, JK) * PLEPS(JIJ, JK) / (2.*ZCTSVD) * ZMZF2D_WORK1(JIJ, JK)    
  END DO
END DO

!
ZWKLES(:,:) = ZA*ZFLXZ
    CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_SvThv(:,:,:,JSV) ) 
    ZWKLES(:,:) = -CST%XG/PTHVREF/3.*ZA*ZFLXZ
