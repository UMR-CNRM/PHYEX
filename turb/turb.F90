...
!
  ...
  ENDDO
   ...
   DO JK=IKTB,IKTE
    DO JIJ=IIJB,IIJE 
      ZLM(JIJ,JK) = ( 0.5*(PZZ(JIJ,JK)+PZZ(JIJ,JK+D%NKL)) - &
      & PZZ(JIJ,D%NKA+JPVEXT_TURB*D%NKL) ) * PDIRCOSZW(JIJ)
...
#ifdef REPRO48
  (1.E-60 + ZUSLOPE(JIJ)**2 + ZVSLOPE(JIJ)**2 ) )
#else
...
ELSE
  DO JIJ=IIJB,IIJE         
    ZTAU11M(JIJ) =2./3.*(  (1.+ (PZZ(JIJ,IKB+D%NKL)-PZZ(JIJ,IKB))  &
    /(PDZZ(JIJ,IKB+D%NKL)+PDZZ(JIJ,IKB))  &
    )   *PTKET(JIJ,IKB)                   &
    -0.5  *PTKET(JIJ,IKB+D%NKL)                 &
    )
  ENDDO
END IF
...
IF (ODZ) THEN
  ...
  DO JK = IKTB,IKTE ! 1D turbulence scheme
    DO JIJ=IIJB,IIJE 
      PLM(JIJ,JK) = PZZ(JIJ,JK+D%NKL) - PZZ(JIJ,JK)
    ENDDO
  END DO
  DO JIJ=IIJB,IIJE 
    PLM(JIJ,D%NKU) = PLM(JIJ,IKE)
    PLM(JIJ,D%NKA) = PZZ(JIJ,IKB) - PZZ(JIJ,D%NKA)
  ENDDO
...
DO JK=IKTB,IKTE 
  DO JIJ=IIJB,IIJE 
    PLM(JIJ,JK) = PZZ(JIJ,JK+D%NKL+IKTB-(IKTB) ) - PZZ(JIJ,JK)
  ENDDO
ENDDO
DO JIJ=IIJB,IIJE 
  PLM(JIJ,D%NKU) = PLM(JIJ,IKE)
...
DO JIJ=IIJB,IIJE 
  ZDTHLDZ(JIJ,IKB)=(PTHLT(JIJ,IKB+D%NKL)-PTHLT(JIJ,IKB))/PDZZ(JIJ,IKB+D%NKL)
ENDDO
! For dry simulations
IF (KRR>0) THEN
  DO JIJ=IIJB,IIJE 
    ZDRTDZ(JIJ,IKB)=(PRT(JIJ,IKB+D%NKL,1)-PRT(JIJ,IKB,1))/PDZZ(JIJ,IKB+D%NKL)
  ENDDO
ELSE
  ZDRTDZ(:,IKB)=0
ENDIF
...
DO JIJ=IIJB,IIJE 
  PLM(JIJ,D%NKA) = PLM(JIJ,IKB)
  PLM(JIJ,IKE) = PLM(JIJ,IKE-D%NKL)
  PLM(JIJ,D%NKU) = PLM(JIJ,D%NKU-D%NKL)
ENDDO
!
