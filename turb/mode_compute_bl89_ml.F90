...
DO JI=D%NIJB,D%NIJE 
  ZDELTVPT(JI,D%NKB)=PDZZ2D(JI,D%NKB)*ZDELTVPT(JI,D%NKB+D%NKL)/PDZZ2D(JI,D%NKB+D%NKL)
  ZHLVPT(JI,D%NKB)=PVPT(JI,D%NKB)-ZDELTVPT(JI,D%NKB)*0.5
ENDDO
!
!
!
...
IF (OUPORDN.EQV..TRUE.) THEN 
  DO JI=D%NIJB,D%NIJE 
    ZINTE(JI)=PTKEM_DEP(JI)
  ENDDO
 PLWORK=0.
 ZTESTM=1.
 IF(OFLUX)THEN
    DO JI=D%NIJB,D%NIJE 
      ZVPT_DEP(JI)=ZHLVPT(JI,KK) ! departure point is on flux level
    ENDDO
 ...
 ELSE
    DO JI=D%NIJB,D%NIJE 
      ZVPT_DEP(JI)=PVPT(JI,KK) ! departure point is on mass level
    ENDDO
 ENDIF
