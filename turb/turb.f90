...
!$acc kernels
  ...
  ! DVDX=PHGRADGOG(:,:,3)
  ! DVDY=PHGRADGOG(:,:,4)
  PDP(:,:)=PDP(:,:)+TURBN%XSMAG**2*PDXX(:,:)*PDYY(:,:)* &
                   &(PHGRADGOG(:,:,1)*PHGRADGOG(:,:,1)           &
                   &+PHGRADGOG(:,:,4)*PHGRADGOG(:,:,4)           &
                   &+0.5*(PHGRADGOG(:,:,2)+PHGRADGOG(:,:,3))     &
                   &*(PHGRADGOG(:,:,2)+PHGRADGOG(:,:,3)))**(3./2.)
  !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
!$acc end kernels
ENDIF
