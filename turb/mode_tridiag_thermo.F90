...
DO JIJ=IIJB,IIJE 
  ...
  - ZMZM_RHODJ(JIJ,IKB+D%NKL) * PF(JIJ,IKB+D%NKL)/PDZZ(JIJ,IKB+D%NKL)    &
  + ZMZM_RHODJ(JIJ,IKB) * PF(JIJ,IKB)/PDZZ(JIJ,IKB)          &
  + ZRHODJ_DFDDTDZ_O_DZ2(JIJ,IKB+D%NKL) * PIMPL * PVARM(JIJ,IKB+D%NKL) &
  - ZRHODJ_DFDDTDZ_O_DZ2(JIJ,IKB+D%NKL) * PIMPL * PVARM(JIJ,IKB)
ENDDO
!
DO JK=IKTB+1,IKTE-1
  DO JIJ=IIJB,IIJE 
    ZY(JIJ,JK) = PRHODJ(JIJ,JK)*PVARM(JIJ,JK)/PTSTEP                 &
    - ZMZM_RHODJ(JIJ,JK+D%NKL) * PF(JIJ,JK+D%NKL)/PDZZ(JIJ,JK+D%NKL)     &
    + ZMZM_RHODJ(JIJ,JK) * PF(JIJ,JK)/PDZZ(JIJ,JK)           &
    + ZRHODJ_DFDDTDZ_O_DZ2(JIJ,JK+D%NKL) * PIMPL * PVARM(JIJ,JK+D%NKL) &
    - ZRHODJ_DFDDTDZ_O_DZ2(JIJ,JK+D%NKL) * PIMPL * PVARM(JIJ,JK)   &
    - ZRHODJ_DFDDTDZ_O_DZ2(JIJ,JK) * PIMPL * PVARM(JIJ,JK)   &
    + ZRHODJ_DFDDTDZ_O_DZ2(JIJ,JK) * PIMPL * PVARM(JIJ,JK-D%NKL)
  ENDDO
END DO
! 
DO JIJ=IIJB,IIJE 
  ZY(JIJ,IKE) = PRHODJ(JIJ,IKE)*PVARM(JIJ,IKE)/PTSTEP               &
  - ZMZM_RHODJ(JIJ,IKE+D%NKL) * PF(JIJ,IKE+D%NKL)/PDZZ(JIJ,IKE+D%NKL) &
  + ZMZM_RHODJ(JIJ,IKE) * PF(JIJ,IKE)/PDZZ(JIJ,IKE)       &
  - ZRHODJ_DFDDTDZ_O_DZ2(JIJ,IKE) * PIMPL * PVARM(JIJ,IKE)   &
  + ZRHODJ_DFDDTDZ_O_DZ2(JIJ,IKE) * PIMPL * PVARM(JIJ,IKE-D%NKL)
ENDDO
!
!
...
!
  DO JIJ=IIJB,IIJE 
    ...
    - ZRHODJ_DFDDTDZ_O_DZ2(JIJ,IKB+D%NKL) * PIMPL
    ZC(JIJ,IKB) =   ZRHODJ_DFDDTDZ_O_DZ2(JIJ,IKB+D%NKL) * PIMPL
  ENDDO
!
  DO JK=IKTB+1,IKTE-1
    DO JIJ=IIJB,IIJE 
      ...
      - ZRHODJ_DFDDTDZ_O_DZ2(JIJ,JK+D%NKL) * PIMPL &
      - ZRHODJ_DFDDTDZ_O_DZ2(JIJ,JK) * PIMPL
      ZC(JIJ,JK) =   ZRHODJ_DFDDTDZ_O_DZ2(JIJ,JK+D%NKL) * PIMPL
    ENDDO
  END DO
!
  DO JIJ=IIJB,IIJE 
...
!
  ...
  DO JK = IKB+D%NKL,IKE-D%NKL,D%NKL
    DO JIJ=IIJB,IIJE 
      ZGAM(JIJ,JK) = ZC(JIJ,JK-D%NKL) / ZBET(JIJ)  
                                                    ! gam(k) = c(k-1) / bet
      ZBET(JIJ)    = ZB(JIJ,JK) - ZA(JIJ,JK) * ZGAM(JIJ,JK)
                                                    ! bet = b(k) - a(k)* gam(k)  
      PVARP(JIJ,JK)= ( ZY(JIJ,JK) - ZA(JIJ,JK) * PVARP(JIJ,JK-D%NKL) ) &
      / ZBET(JIJ)
  ...
  DO JIJ=IIJB,IIJE 
    ZGAM(JIJ,IKE) = ZC(JIJ,IKE-D%NKL) / ZBET(JIJ) 
                                                    ! gam(k) = c(k-1) / bet
    ZBET(JIJ)     = ZB(JIJ,IKE) - ZA(JIJ,IKE) * ZGAM(JIJ,IKE)
                                                    ! bet = b(k) - a(k)* gam(k)  
    PVARP(JIJ,IKE)= ( ZY(JIJ,IKE) - ZA(JIJ,IKE) * PVARP(JIJ,IKE-D%NKL) ) &
    / ZBET(JIJ)
                                       ! res(k) = (y(k) -a(k)*res(k-1))/ bet 
  ENDDO
!
