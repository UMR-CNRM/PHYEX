...
IF (OENTR_DETR) THEN
  ...
  IF (PARAMMF%LGZ) THEN
    ...
    ENDIF
    !$mnh_expand_array(JIJ=IIJB:IIJE)
    ZSURF(:)=TANH(PARAMMF%XGZ*SQRT(PDX*PDY)/ZLUPSURF(:))
    !$mnh_end_expand_array(JIJ=IIJB:IIJE)
  ELSE
    ZSURF(:)=1.
  ENDIF
  ...
  WHERE (ZWTHVSURF(:)>0.)
    PEMF(:,IKB) = PARAMMF%XCMF * ZSURF(:) * ZRHO_F(:,IKB) *  &
            ((ZG_O_THVREF(:,IKB))*ZWTHVSURF(:)*ZLUPSURF(:))**(1./3.)
...
DO JK=IKB,IKE-IKL,IKL
  ...
  IF(PARAMMF%LRELAX_ALPHA_MF) THEN
    !$mnh_expand_array(JIJ=IIJB:IIJE)
    ZRELAX_ALPHA_COEFF(:)=1./(1.-PFRAC_UP(:,JK))
    !$mnh_end_expand_array(JIJ=IIJB:IIJE)
  ELSE
  ...
  IF(PARAMMF%LMIXUV) THEN
    IF(JK/=IKB) THEN
      !$mnh_expand_where(JIJ=IIJB:IIJE)
      WHERE(GTEST(:))
        PU_UP(:,JK+IKL) = (PU_UP(:,JK)*(1-0.5*ZRELAX_ALPHA_COEFF(:)*ZMIX2(:)) + &
                                        &PUM(:,JK)*ZRELAX_ALPHA_COEFF(:)*ZMIX2(:)+ &
                          0.5*PARAMMF%XPRES_UV*(PZZ(:,JK+IKL)-PZZ(:,JK))*&
                          ((PUM(:,JK+IKL)-PUM(:,JK))/PDZZ(:,JK+IKL)+&
                           (PUM(:,JK)-PUM(:,JK-IKL))/PDZZ(:,JK))        )   &
                          /(1+0.5*ZRELAX_ALPHA_COEFF(:)*ZMIX2(:))
        PV_UP(:,JK+IKL) = (PV_UP(:,JK)*(1-0.5*ZRELAX_ALPHA_COEFF(:)*ZMIX2(:)) + &
                                        &PVM(:,JK)*ZRELAX_ALPHA_COEFF(:)*ZMIX2(:)+ &
                          0.5*PARAMMF%XPRES_UV*(PZZ(:,JK+IKL)-PZZ(:,JK))*&
  ...
  IF(PARAMMF%LMIXTKE) THEN
    !$mnh_expand_where(JIJ=IIJB:IIJE)
    WHERE(GTEST(:))
      PTKE_UP(:,JK+IKL) = (PTKE_UP(:,JK)*(1-0.5*ZRELAX_ALPHA_COEFF(:)*ZMIX2(:)) + &
                                   ZTKEM_F(:,JK)*ZRELAX_ALPHA_COEFF(:)*ZMIX2(:)) &
                                  /(1+0.5*ZRELAX_ALPHA_COEFF(:)*ZMIX2(:))
    ENDWHERE
    !$mnh_end_expand_where(JIJ=IIJB:IIJE)
  ENDIF
  ...
  DO JSV=1,KSV 
    ...
    !$mnh_expand_where(JIJ=IIJB:IIJE)
    WHERE(GTEST(:)) 
      PSV_UP(:,JK+IKL,JSV) = (PSV_UP(:,JK,JSV)*(1-0.5*ZRELAX_ALPHA_COEFF(:)*ZMIX2(:)) + &
                                      PSVM(:,JK,JSV)*ZRELAX_ALPHA_COEFF(:)*ZMIX2(:)) &
                                     /(1+0.5*ZRELAX_ALPHA_COEFF(:)*ZMIX2(:))
    ENDWHERE
    !$mnh_end_expand_where(JIJ=IIJB:IIJE)
  END DO  
...
!$mnh_expand_array(JIJ=IIJB:IIJE)
IF(KK/=KKB)THEN
  ZCOEFF_MINUS_HALF_THV(:)=((PTHVM(:,KK)-PTHVM(:,KK-KKL))/PDZZ(:,KK))
  ZTHV_MINUS_HALF(:) = PTHVM(:,KK) - &
                               & ZCOEFF_MINUS_HALF_THV(:)*0.5*(PZZ(:,KK+KKL)-PZZ(:,KK))
ELSE
  ZCOEFF_MINUS_HALF_THV(:)=0.
  ZTHV_MINUS_HALF(:) = PTHVM(:,KK)
ENDIF
ZCOEFF_PLUS_HALF_THV(:)  = ((PTHVM(:,KK+KKL)-PTHVM(:,KK))/PDZZ(:,KK+KKL))
ZTHV_PLUS_HALF(:)  = PTHVM(:,KK) + &
                             & ZCOEFF_PLUS_HALF_THV(:)*0.5*(PZZ(:,KK+KKL)-PZZ(:,KK))
!$mnh_end_expand_array(JIJ=IIJB:IIJE)
