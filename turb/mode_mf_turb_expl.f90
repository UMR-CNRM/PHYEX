...
IF(PARAMMF%LRELAX_ALPHA_MF) THEN
  ...
  ZRELAX_ALPHA_COEFF(:, :)=1./(1.-PFRAC_UP(:, :))
...
!$mnh_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
PFLXZTHLMF(:,:)  = PEMF(:,:)*ZRELAX_ALPHA_COEFF(:,:)*&
                             & (PTHL_UP(:,:)-PFLXZTHLMF(:,:))  ! ThetaL
PFLXZRMF(:,:)    = PEMF(:,:)*ZRELAX_ALPHA_COEFF(:,:)*&
                             & (PRT_UP(:,:)-PFLXZRMF(:,:))  ! Rt
PFLXZTHVMF(:,:)  = PEMF(:,:)*ZRELAX_ALPHA_COEFF(:,:)*&
                             & (PTHV_UP(:,:)-PFLXZTHVMF(:,:))  ! ThetaV

ZFLXZTHSMF(:,:)  = PEMF(:,:)*ZRELAX_ALPHA_COEFF(:,:)*&
                             & (ZTHS_UP(:,:)-ZTHSM(:,:))    ! Theta S flux
!$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
...
IF (PARAMMF%LMIXUV) THEN
  ...
  !$mnh_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
  PFLXZUMF(:,:)  = PEMF(:,:)*ZRELAX_ALPHA_COEFF(:,:)*&
                             & (PU_UP(:,:)-PFLXZUMF(:,:))  ! U
  PFLXZVMF(:,:)  = PEMF(:,:)*ZRELAX_ALPHA_COEFF(:,:)*&
                             & (PV_UP(:,:)-PFLXZVMF(:,:))  ! V
  !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
ELSE
  PFLXZUMF(:,:) = 0.
...
IF (PARAMMF%LMIXTKE) THEN
  ...
  !$mnh_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
  PFLXZTKEMF(:,:) = PEMF(:,:)*ZRELAX_ALPHA_COEFF(:,:)*&
                              & (PTKE_UP(:,:)-PFLXZTKEMF(:,:))
  !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
ELSE
  PFLXZTKEMF(:,:) = 0.
