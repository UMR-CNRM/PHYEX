...
FLUSH(UNIT=KLUOUT)
!
CTEMP=TRIM(HVAR) // ' is not allowed for variable ' // TRIM(HNAME)
CALL PRINT_MSG(NVERB_FATAL, 'GEN', 'CHECK_NAM_VAL_CHAR', CTEMP)
!
...
DO II=1, INUM
  ...
  SELECT CASE (CSIGN)
    ...
    CASE ('==')
      LOK=LOK .AND. PVALUE == ZVAL
    CASE DEFAULT
      CTEMP=TRIM(CSIGN) // ' is not allowed as comparator'
      CALL PRINT_MSG(NVERB_FATAL, 'GEN', 'CHECK_NAM_VAL_REAL', CTEMP)
  END SELECT
ENDDO
!
!** PRINTS AND ABORT
...
IF(.NOT. LOK) THEN
  ...
  CTEMP=TRIM(CHAR_VAL) // ' is not allowed for variable ' // TRIM(HNAME)
  CALL PRINT_MSG(NVERB_FATAL, 'GEN', 'CHECK_NAM_VAL_REAL', CTEMP)
ENDIF
!
