...
USE MODE_CONVECT_CLOSURE_SHAL, ONLY: CONVECT_CLOSURE_SHAL
USE MODE_CONVECT_UPDRAFT_SHAL, ONLY: CONVECT_UPDRAFT_SHAL
...
REAL               ,DIMENSION(D%NIJT,D%NKT)      ,INTENT(IN)   :: PTHT,PSTHV,PSTHES  ! grid scale theta, theta_v
INTEGER            ,DIMENSION(D%NIJT)            ,INTENT(IN)   :: ISDPL   ! index for parcel departure level
INTEGER            ,DIMENSION(D%NIJT)            ,INTENT(IN)   :: ISPBL   ! index for source layer top
INTEGER            ,DIMENSION(D%NIJT)            ,INTENT(IN)   :: ISLCL   ! index for lifting condensation level
REAL               ,DIMENSION(D%NIJT)            ,INTENT(IN)   :: PSTHLCL ! updraft theta at LCL/L
REAL               ,DIMENSION(D%NIJT)            ,INTENT(IN)   :: PSTLCL  ! updraft temp. at LCL
REAL               ,DIMENSION(D%NIJT)            ,INTENT(IN)   :: PSRVLCL ! updraft rv at LCL
REAL               ,DIMENSION(D%NIJT)            ,INTENT(IN)   :: PSWLCL  ! updraft w at LCL
REAL               ,DIMENSION(D%NIJT)            ,INTENT(IN)   :: PSZLCL  ! LCL height
REAL               ,DIMENSION(D%NIJT)            ,INTENT(IN)   :: PSTHVELCL! envir. theta_v at LCL
LOGICAL            ,DIMENSION(D%NIJT)            ,INTENT(IN)   :: GTRIG1  ! logical mask for convection
REAL               ,DIMENSION(D%NIJT,D%NKT)      ,INTENT(OUT)  :: PUMF    ! updraft mass flux (kg/s)
REAL               ,DIMENSION(D%NIJT,D%NKT)      ,INTENT(OUT)  :: PTHC    ! conv. adj. grid scale theta
REAL               ,DIMENSION(D%NIJT,D%NKT)      ,INTENT(OUT)  :: PRVC    ! conv. adj. grid scale r_w
...
INTEGER            ,DIMENSION(D%NIJT)            ,INTENT(OUT)  :: ICTL    ! index for cloud top level
INTEGER            ,DIMENSION(D%NIJT)            ,INTENT(OUT)  :: IMINCTL ! min between index for cloud top level
                                                        ! and lifting condensation level
REAL               ,DIMENSION(D%NIJT,D%NKT,KCH1) ,INTENT(OUT)  :: PPCH1TEN
!
!
REAL, DIMENSION(D%NIJT)             :: ZWORK2, ZWORK2B ! work array
...
!
INTEGER, DIMENSION(D%NIJT)  :: IETL    ! index for zero buoyancy level
INTEGER, DIMENSION(D%NIJT)  :: ILFS    ! index for level of free sink
!
! grid scale variables
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZDPRES  ! pressure difference between
                                              ! bottom and top of layer (Pa)
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZTHL    ! grid scale enthalpy (J/kg)
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZRW     ! grid scale total water (kg/kg)
!
! updraft variables
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZUER    ! updraft entrainment (kg/s)
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZUDR    ! updraft detrainment (kg/s)
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZUTHL   ! updraft enthalpy (J/kg)
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZUTHV   ! updraft theta_v (K)
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZURW    ! updraft total water (kg/kg)
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZURC    ! updraft cloud water (kg/kg)
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZURI    ! updraft cloud ice   (kg/kg)
REAL, DIMENSION(D%NIJT)       :: ZCAPE   ! available potent. energy
!
! downdraft variables
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZDMF    ! downdraft mass flux (kg/s)
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZDER    ! downdraft entrainment (kg/s)
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZDDR    ! downdraft detrainment (kg/s)
!
! closure variables
REAL, DIMENSION(D%NIJT)       :: ZDXDY
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZLMASS  ! mass of model layer (kg)
REAL, DIMENSION(D%NIJT)       :: ZTIMEC  ! advective time period
!
REAL, DIMENSION(D%NIJT,D%NKT)  :: ZWSUB   ! envir. compensating subsidence (Pa/s)
!
LOGICAL, DIMENSION(D%NIJT)    :: GTRIG2  ! logical mask for convection
REAL, DIMENSION(D%NIJT)       :: ZCPH    ! specific heat C_ph
REAL, DIMENSION(D%NIJT)       :: ZLV, ZLS! latent heat of vaporis., sublim.
!
! Chemical Tracers:
REAL, DIMENSION(D%NIJT,KCH1)     :: ZWORK3  ! conv. adjust. chemical specy 1
REAL, DIMENSION(D%NIJT,D%NKT,KCH1):: ZCH1C   ! conv. adjust. chemical specy 1
...
IF ( OCH1CONV ) THEN
  ZDXDY(:)=CVP_SHAL%XA25
  CALL CONVECT_CHEM_TRANSPORT(CVPEXT, D, NSV, CST, KCH1, PCH1, ZCH1C, ISDPL,&
                              ISPBL, ISLCL, ICTL, ILFS, ILFS, PUMF,    &
                              ZUER, ZUDR, ZDMF, ZDER, ZDDR, ZTIMEC,    &
                              ZDXDY, ZDMF(:,1), ZLMASS, ZWSUB, &
                              IFTSTEPS)
!
!
    ...
    IF(JN < NSV%NSV_LGBEG .OR. JN>NSV%NSV_LGEND-1) THEN ! no correction for xy lagrangian variables
      ...
      DO JK = IKB+1, JKM
        ...
        DO JI = D%NIJB,D%NIJE
          ...
          ZWORK3(JI,JN) = ZWORK3(JI,JN) + (ZCH1C(JI,JK,JN)-PCH1(JI,JK,JN)) * ZW1
