...
def pybinding(fortran_in, scope, fortran_out, python_out, libso,
    ...
    for N in scopeNode.findall('.//{*}dummy-arg-LT/{*}arg-N/{*}N'):
        ...
        if vartype == 'TYPE(DIMPHYEX_T)':
            ...
            copyList.append('PHYAERO%CRGUNITS      = MISC%CRGUNITS')
            copyList.append('PHYAERO%XINIRADIUS_SLT=> MISC%XINIRADIUS_SLT')
            copyList.append('PHYAERO%XDENSITY_SALT = MISC%XDENSITY_SALT')
            ...
            if not (dim is None or len(dim) == 0):
                for d in dim:
                    ...
                    for elem in findcomponents(d[1]):
                        ...
                        elif elem in ('JPSVMAX', 'NSV', 'NSP', 'NCARB', 'NSOA', 'KGRADIENTSLEO', 'KGRADIENTSGOG', 'KSOA'):
