...
"""Compare variables and files between user and reference outputs."""
...
def compareBACKUPFiles(file_user, file_ref):
    """Compare all variables in two BACKUP NetCDF files."""
    status = 0
    ...
    for var in [var for var in variables if da[var].dtype.char != 'S']:
        try:
            ...
            elif da[var].ndim == 3 and da['L2D'] == 0:
                s = (0, slice(JPHEXT, nj - JPHEXT),
                     slice(JPHEXT, ni - JPHEXT))
                ecart_min = float(da2[var][s].min()) - float(da[var][s].min())
                ecart_moy = float(da2[var][s].mean()) - float(da[var][s].mean())
                ecart_max = float(da2[var][s].max()) - float(da[var][s].max())
            elif da[var].ndim == 3 and da['L2D'] == 1:
...
def compareTSERIESFiles(file_user, file_ref, tol_ad=1E-12):
    """Compare all variables in two diachronic (time series) NetCDF files,
    including group comparison."""
    status = 0
    da = xr.open_dataset(file_user)
    da2 = xr.open_dataset(file_ref)
    variables = list(da.keys())
...
def comp_DDH(filename1, filename2, output_fig, tol_ad=3E-7, tol_rd=1.E-6, verbose=False):
    """Compare two DDH files field by field, with optional plots of differences."""
    import epygram
    epygram.init_env()
    ...
    if filename1 != filename2:
            ...
            if not t:
                ...
                for i in range(len(v1)):
                    if v1[i] - v2[i] != 0.:
                        ...
                        if verbose:
                            print("   v1={v1}, v2={v2}, diff={ad}, rdiff={rd}".format(
                                v1=v1[i], v2=v2[i], ad=ad, rd=rd))
        ...
        if pb_val and output_fig is not None and len(output_fig) > 0:
            ...
            for ifid, fid in enumerate(toplt):
                ...
                for i in range(len(v1)):
                    ...
                    ax[ifid].plot(v1[i].getdata(), v1[i].geometry.vcoordinate.levels,
                                  label='v1')
                    ax[ifid].plot(v2[i].getdata(), v2[i].geometry.vcoordinate.levels,
                                  label='v2')
                    ax[ifid].legend()
                    ax[ifid].twiny().plot(ad, v1[i].geometry.vcoordinate.levels,
                                          label='diff', color='black', ls=':')
                    ad_abs = numpy.abs(ad)
                    rd = (200 * numpy.abs(v1[i].getdata() - v2[i].getdata())
                          / numpy.abs(v1[i].getdata() + v2[i].getdata()))
                    rd = rd[ad_abs != 0.].max()
                    ad_abs = ad_abs.max()
                    ax[ifid].set_title(f"{fid}:\nmax_ad={ad_abs}\nmax_rd={rd}%")
        ...
        if pb_var and pb_val:
            ...
            message = ""
        if pb_val and output_fig is not None and len(output_fig) > 0:
            message += ", plot is available in the following file(s): " + ', '.join(output_fig)
    else:
        pb_var = False
        pb_val = False
        message = ""
...
def comp_NODE(f1, f2, spnorms=None, gpnorms=None, norm_max_diff=0.050000):
    ...
    if spnorms is None:
        spnorms = default_spnorms
    if gpnorms is None:
        gpnorms = default_gpnorms
        ...
        with open(f, 'r', encoding='utf-8') as fh:
            ...
            if opts['gpnorms']:
                if len(opts['gpnorms']) == 1 and opts['gpnorms'][0] == '*':
                    ...
                    gpregs = [re.compile(rf'^\s*GPNORM\s+\b({re.escape(gp)})\b')
                              for gp in opts['gpnorms']]
            x = []
            for line in fh:
                line = line.strip()
                if re.match(r'^\s*GPNORM\s+', line):
                    ...
                    while True:
                        ...
                        spnormk = sorted(
                            [sp for sp in opts['spnorms'] if sp in index.keys()],
                            key=lambda spnorm: index[spnorm] if index[spnorm] >= 0
                            else float('inf'))
                        line = next(fh)
                        if not re.match(r'\s+AVE\s+', line):
                            break
                        spnormv = line.split()[1:]
    ...
    def main(f1, f2):
        ...
        while fx1 and fx2:
            ...
            if x1 and x2:
                ...
                x[-1].append(
                    " {} | {:20} | {:17.9e}  |  {:17.9e}  |  {:17}  |  {:17} {}\n".format(
                        tag1, center(field1, 20), float(x1), float(x2), sdx, sdr,
                        '*' if dr > opts['norm_max_diff'] else ''))
...
def comp_ncdump(f1, f2, nbytes):
    ...
    diff = ''.join(difflib.unified_diff(
        *[ncdumps[i].splitlines(keepends=True) for i in (0, 1)]))
    if diff != '':
        print(diff)
...
def comp_testprogs(f1, f2):
    """Compare two testprogs output listing files."""
...
def main():
    """Parse command-line arguments and dispatch to the appropriate comparison function."""
    parser = argparse.ArgumentParser(
        description='Compare all the variables in the different files')
