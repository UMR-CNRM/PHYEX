...
class CheckCommitLmdz(CheckCommitBase):
    ...
    def __init__(self, prepCodeOpts='', buildSys=None, **kwargs):
        ...
        self.phyexlmdzdir = os.path.join(self.LMDZPACK, self.name, 'phyex-lmdz')
    ...
    def pack_update(self):
        ...
        if self.link:
            ...
            if self.packupdate:
                ...
                shutil.move(os.path.join(packdir, 'phyex-lmdz'),
                            os.path.join(packdir, 'phyex-lmdzori'))
                        ...
                        for f in os.listdir(sub_full):
                            ...
                            if os.path.isfile(src):
                                if os.path.exists(dst):
                                    os.unlink(dst)
                                os.symlink(src, dst)
                sub_full = os.path.join(self.phyexlmdzdir, 'src')
                for f in os.listdir(sub_full):
                    src = os.path.join(sub_full, f)
                    dst = os.path.join(target, f)
                    if os.path.isfile(src):
                        if os.path.exists(dst):
                            os.unlink(dst)
