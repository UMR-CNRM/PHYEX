...
!
   ...
   WHERE ( PRT(D%NIJB:D%NIJE,:,2) > 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NC)<ZSVTHR)
      PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NC) = ZCONC
   END WHERE
   WHERE ( PRT(D%NIJB:D%NIJE,:,2) <= 1.E-11  .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NC)<ZSVTHR)
      PRT(D%NIJB:D%NIJE,:,2)  = 0.0
      PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NC) = 0.0
   END WHERE
   
   IF (NMOD_CCN .GE. 1) THEN
      WHERE ( PRT(D%NIJB:D%NIJE,:,2) > 1.E-11  .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NC)<ZSVTHR)
         PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_CCN_ACTI) = ZCONC
      END WHERE
      WHERE ( PRT(D%NIJB:D%NIJE,:,2) <= 1.E-11  .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NC)<ZSVTHR)
         PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_CCN_ACTI) = 0.0
      END WHERE
   END IF
...
!
   ...
   IF (HGETCLOUD == 'INI1') THEN ! init from REVE scheme
      PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NR) = 0.0
   ELSE ! init from KESS, ICE3...
      WHERE ( PRT(D%NIJB:D%NIJE,:,3) > 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NR)<ZSVTHR )
         PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NR) = MAX( SQRT(SQRT(PRHODREF(D%NIJB:D%NIJE,:)*PRT(D%NIJB:D%NIJE,:,3) &
              *ZCONC)),1. )
      END WHERE
      WHERE ( PRT(D%NIJB:D%NIJE,:,3) <= 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NR)<ZSVTHR )
         PRT(D%NIJB:D%NIJE,:,3)  = 0.0
         PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NR) = 0.0
      END WHERE
   END IF
END IF
...
!
   ...
   IF (.NOT. LCRYSTAL_SHAPE) THEN
    ...
    END WHERE
    WHERE ( PRT(D%NIJB:D%NIJE,:,4) <= 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI)<ZSVTHR )
      PRT(D%NIJB:D%NIJE,:,4)  = 0.0
      PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI) = 0.0
    END WHERE
   ELSE
      ZNI_TOT(:,:) = 0.
      DO ISH = 1, NNB_CRYSTAL_SHAPE
         WHERE ( PRT(D%NIJB:D%NIJE,:,4) > 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI+ISH-1) < ZSVTHR) 
            PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI+ISH-1) = MIN(PRT(D%NIJB:D%NIJE,:,4)/(XAI_SHAPE(ISH)*(10.E-06)**XBI_SHAPE(ISH)),ZCONC )
         END WHERE
         ZNI_TOT(D%NIJB:D%NIJE,:) = ZNI_TOT(D%NIJB:D%NIJE,:) + PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI+ISH-1)
      END DO
      WHERE (PRT(D%NIJB:D%NIJE,:,4) <= 1.E-11 .AND. ZNI_TOT(D%NIJB:D%NIJE,:)<ZSVTHR )
         PRT(D%NIJB:D%NIJE,:,4)  = 0.0
!++cb-- il faudrait trouver une plus jolie solution, mais ca compile
         !PSVT(:,:,:,ISV_LIMA_NI:ISV_LIMA_NI+NNB_CRYSTAL_SHAPE-1) = 0.0
         PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI)   = 0.0
         PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI+1) = 0.0
         PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI+2) = 0.0
         PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI+3) = 0.0
      END WHERE
   END IF

   IF (NMOD_IFN .GE. 1) THEN
      WHERE ( PRT(D%NIJB:D%NIJE,:,4) > 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI)<ZSVTHR )
         PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_IFN_NUCL) = PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI)
      END WHERE
      WHERE ( PRT(D%NIJB:D%NIJE,:,4) <= 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NI)<ZSVTHR )
...
!
   ...
   WHERE ( PRT(D%NIJB:D%NIJE,:,5) > 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NS)<ZSVTHR )
      PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NS) = PRT(D%NIJB:D%NIJE,:,5) * ZCONC
   END WHERE
   WHERE ( PRT(D%NIJB:D%NIJE,:,5) <= 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NS)<ZSVTHR )
      PRT(D%NIJB:D%NIJE,:,5)  = 0.0
      PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NS) = 0.0
   END WHERE
END IF
!
...
!
   ...
   WHERE ( PRT(D%NIJB:D%NIJE,:,6) > 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NG)<ZSVTHR )
      PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NG) = PRT(D%NIJB:D%NIJE,:,6) * ZCONC
   END WHERE
   WHERE ( PRT(D%NIJB:D%NIJE,:,6) <= 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NG)<ZSVTHR )
      PRT(D%NIJB:D%NIJE,:,6)  = 0.0
      PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NG) = 0.0
   END WHERE
END IF
!
...
!
   ...
   WHERE ( PRT(D%NIJB:D%NIJE,:,7) > 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NH)<ZSVTHR )
      PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NH) = PRT(:,:,7) * ZCONC
   END WHERE
   WHERE ( PRT(D%NIJB:D%NIJE,:,7) <= 1.E-11 .AND. PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NH)<ZSVTHR )
      PRT(D%NIJB:D%NIJE,:,7)  = 0.0
      PSVT(D%NIJB:D%NIJE,:,ISV_LIMA_NH) = 0.0
   END WHERE
END IF
!
