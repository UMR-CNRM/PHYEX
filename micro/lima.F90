...
!
IF ( LIMAP%NMOD_CCN .GE. 1 ) ZCCNFT(D%NIJB:D%NIJE,:,:) = &
     PSVS(D%NIJB:D%NIJE,:,ISV_LIMA_CCN_FREE:ISV_LIMA_CCN_FREE+LIMAP%NMOD_CCN-1) * PTSTEP
IF ( LIMAP%NMOD_CCN .GE. 1 ) ZCCNAT(D%NIJB:D%NIJE,:,:) = &
     PSVS(D%NIJB:D%NIJE,:,ISV_LIMA_CCN_ACTI:ISV_LIMA_CCN_ACTI+LIMAP%NMOD_CCN-1) * PTSTEP
...
!
IF ( LIMAP%NMOD_IFN .GE. 1 ) ZIFNFT(D%NIJB:D%NIJE,:,:) = &
     PSVS(D%NIJB:D%NIJE,:,ISV_LIMA_IFN_FREE:ISV_LIMA_IFN_FREE+LIMAP%NMOD_IFN-1) * PTSTEP
IF ( LIMAP%NMOD_IFN .GE. 1 ) ZIFNNT(D%NIJB:D%NIJE,:,:) = &
     PSVS(D%NIJB:D%NIJE,:,ISV_LIMA_IFN_NUCL:ISV_LIMA_IFN_NUCL+LIMAP%NMOD_IFN-1) * PTSTEP
...
!
IF ( LIMAP%NMOD_IMM .GE. 1 ) ZIMMNT(D%NIJB:D%NIJE,:,:) = &
     PSVS(D%NIJB:D%NIJE,:,ISV_LIMA_IMM_NUCL:ISV_LIMA_IMM_NUCL+LIMAP%NMOD_IMM-1) * PTSTEP
...
IF ( BUCONF%LBU_ENABLE ) THEN
  ...
  IF ( BUCONF%LBUDGET_SV ) THEN
    ...
    IF ( LIMAP%LSEDI .AND. LIMAP%NMOM_I.GE.2) THEN
      ...
      ELSE
        DO ISH = 1, LIMAP%NNB_CRYSTAL_SHAPE
          CALL TBUDGETS(NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_NI + ISH - 1)%PTR%INIT_PHY(D, 'SEDI', &
...
IF ( BUCONF%LBU_ENABLE ) then
  ...
  IF ( BUCONF%LBUDGET_SV ) then
    ...
    IF ( LIMAP%LSEDI .AND. LIMAP%NMOM_I.GE.2 ) THEN
      ...
      ELSE
        DO ISH = 1, LIMAP%NNB_CRYSTAL_SHAPE
          CALL TBUDGETS(NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_NI + ISH - 1)%PTR%END_PHY(D, 'SEDI', &
               ZCIS_SHAPE( :, :, ISH) * PRHODJ( :, :) )
        END DO
      END IF
    END IF
...
IF (LIMAP%NMOM_C.GE.1 .AND. LIMAP%LDEPOC) THEN
  ...
  WHERE (GDEP)
     ...
     PINPRC(D%NIJB:D%NIJE) = PINPRC(D%NIJB:D%NIJE) + &
          LIMAP%XVDEPOC * ZRCT(D%NIJB:D%NIJE,D%NKB) * PRHODREF(D%NIJB:D%NIJE,D%NKB) / CST%XRHOLW
...
!
IF ( LIMAP%NMOD_CCN .GE. 1 )   PSVS(D%NIJB:D%NIJE,:,ISV_LIMA_CCN_FREE:ISV_LIMA_CCN_FREE+LIMAP%NMOD_CCN-1) = &
     ZCCNFT(D%NIJB:D%NIJE,:,:) *ZINV_TSTEP
IF ( LIMAP%NMOD_CCN .GE. 1 )   PSVS(D%NIJB:D%NIJE,:,ISV_LIMA_CCN_ACTI:ISV_LIMA_CCN_ACTI+LIMAP%NMOD_CCN-1) = &
     ZCCNAT(D%NIJB:D%NIJE,:,:) *ZINV_TSTEP
IF ( LIMAP%NMOD_IFN .GE. 1 )   PSVS(D%NIJB:D%NIJE,:,ISV_LIMA_IFN_FREE:ISV_LIMA_IFN_FREE+LIMAP%NMOD_IFN-1) = &
     ZIFNFT(D%NIJB:D%NIJE,:,:) *ZINV_TSTEP
IF ( LIMAP%NMOD_IFN .GE. 1 )   PSVS(D%NIJB:D%NIJE,:,ISV_LIMA_IFN_NUCL:ISV_LIMA_IFN_NUCL+LIMAP%NMOD_IFN-1) = &
     ZIFNNT(D%NIJB:D%NIJE,:,:) *ZINV_TSTEP
IF ( LIMAP%NMOD_IMM .GE. 1 )   PSVS(D%NIJB:D%NIJE,:,ISV_LIMA_IMM_NUCL:ISV_LIMA_IMM_NUCL+LIMAP%NMOD_IMM-1) = &
     ZIMMNT(D%NIJB:D%NIJE,:,:) *ZINV_TSTEP
