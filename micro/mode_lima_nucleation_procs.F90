...
IF ( LIMAP%LACTI .AND. LIMAP%NMOD_CCN >=1 .AND. LIMAP%NMOM_C.GE.2) THEN
  ...
  IF ( BUCONF%LBU_ENABLE ) then
       IF ( BUCONF%LBUDGET_TH ) ZWKBUD(:,:) = PTHT(:,:) * PRHODJ(:,:) / PTSTEP
       IF ( BUCONF%LBUDGET_TH ) &
            CALL TBUDGETS(NBUDGET_TH)%PTR%INIT_PHY(D, 'HENU', ZWKBUD )
       IF ( BUCONF%LBUDGET_RV ) ZWKBUD(:,:) = PRVT(:,:) * PRHODJ(:,:) / PTSTEP
       IF ( BUCONF%LBUDGET_RV ) &
            CALL TBUDGETS(NBUDGET_RV)%PTR%INIT_PHY(D, 'HENU', ZWKBUD )
       IF ( BUCONF%LBUDGET_RC ) ZWKBUD(:,:) = PRCT(:,:) * PRHODJ(:,:) / PTSTEP
       IF ( BUCONF%LBUDGET_RC ) &
            CALL TBUDGETS(NBUDGET_RC)%PTR%INIT_PHY(D, 'HENU', ZWKBUD )
     IF ( BUCONF%LBUDGET_SV ) then
       ZWKBUD(:,:) = PCCT(:,:) * PRHODJ(:,:) / PTSTEP
       CALL TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NC)%PTR%INIT_PHY(D, 'HENU', ZWKBUD )
       DO IL = 1, LIMAP%NMOD_CCN
         IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_FREE - 1 + IL
         ZWKBUD(:,:) = PNFT(:,:, IL) * PRHODJ(:,:) / PTSTEP
         CALL TBUDGETS(IDX)%PTR%INIT_PHY(D, 'HENU', ZWKBUD )
         IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_ACTI - 1 + IL
         ZWKBUD(:,:) = PNAT(:,:, IL) * PRHODJ(:,:) / PTSTEP
         CALL TBUDGETS(IDX)%PTR%INIT_PHY(D, 'HENU', ZWKBUD )
       END DO
     END IF
   END IF
    ...
    IF ( BUCONF%LBU_ENABLE ) then
       IF ( BUCONF%LBUDGET_TH ) ZWKBUD(:,:) = PTHT(:,:) * PRHODJ(:,:) / PTSTEP
       IF ( BUCONF%LBUDGET_TH ) &
            CALL TBUDGETS(NBUDGET_TH)%PTR%END_PHY(D, 'HENU', ZWKBUD )
       IF ( BUCONF%LBUDGET_RV ) ZWKBUD(:,:) = PRVT(:,:) * PRHODJ(:,:) / PTSTEP
       IF ( BUCONF%LBUDGET_RV ) &
            CALL TBUDGETS(NBUDGET_RV)%PTR%END_PHY(D, 'HENU', ZWKBUD )
       IF ( BUCONF%LBUDGET_RC ) ZWKBUD(:,:) = PRCT(:,:) * PRHODJ(:,:) / PTSTEP
       IF ( BUCONF%LBUDGET_RC ) &
            CALL TBUDGETS(NBUDGET_RC)%PTR%END_PHY(D, 'HENU', ZWKBUD )
      IF ( BUCONF%LBUDGET_SV ) then
        ZWKBUD(:,:) = PCCT(:,:) * PRHODJ(:,:) / PTSTEP
        CALL TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NC)%PTR%END_PHY(D, 'HENU', ZWKBUD )
        DO IL = 1, LIMAP%NMOD_CCN
          IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_FREE - 1 + IL
          ZWKBUD(:,:) = PNFT(:,:, IL) * PRHODJ(:,:) / PTSTEP
          CALL TBUDGETS(IDX)%PTR%END_PHY(D, 'HENU', ZWKBUD )
          IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_ACTI - 1 + IL
          ZWKBUD(:,:) = PNAT(:,:, IL) * PRHODJ(:,:) / PTSTEP
          CALL TBUDGETS(IDX)%PTR%END_PHY(D, 'HENU', ZWKBUD )
        END DO
  ...
  IF ( BUCONF%LBU_ENABLE ) then
    IF ( BUCONF%LBUDGET_SV ) then
      DO IL = 1, LIMAP%NMOD_IFN
        ...
        ZWKBUD(:,:) = PIFT(:,:, IL) * PRHODJ(:,:) / PTSTEP
        CALL TBUDGETS(IDX)%PTR%INIT_PHY(D, 'HIND', ZWKBUD )
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_IFN_NUCL - 1 + IL
        ZWKBUD(:,:) = PINT(:,:, IL) * PRHODJ(:,:) / PTSTEP
        CALL TBUDGETS(IDX)%PTR%INIT_PHY(D, 'HIND', ZWKBUD )
      END DO
      ...
      DO IL = 1, LIMAP%NMOD_CCN
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_ACTI - 1 + IL
        ZWKBUD(:,:) = PNAT(:,:, IL) * PRHODJ(:,:) / PTSTEP
        CALL TBUDGETS(IDX)%PTR%INIT_PHY(D, 'HINC', ZWKBUD )
      END DO
      DO IL = 1, LIMAP%NMOD_IMM
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_IMM_NUCL - 1 + IL
        ZWKBUD(:,:) = PNIT(:,:, IL) * PRHODJ(:,:) / PTSTEP
        CALL TBUDGETS(IDX)%PTR%INIT_PHY(D, 'HINC', ZWKBUD )
      END DO
    END IF
  END IF
...
!
  IF ( BUCONF%LBU_ENABLE ) then
     IF ( BUCONF%LBUDGET_TH ) ZWKBUD(:,:) = Z_TH_HIND(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_TH ) &
          CALL  TBUDGETS(NBUDGET_TH)%PTR%ADD_PHY(D, 'HIND', ZWKBUD )
     IF ( BUCONF%LBUDGET_RV ) ZWKBUD(:,:) = -PTOT_RI_HIND(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_RV ) &
          CALL  TBUDGETS(NBUDGET_RV)%PTR%ADD_PHY(D, 'HIND', ZWKBUD )
     IF ( BUCONF%LBUDGET_RI ) ZWKBUD(:,:) = PTOT_RI_HIND(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_RI ) &
          CALL  TBUDGETS(NBUDGET_RI)%PTR%ADD_PHY(D, 'HIND', ZWKBUD )
    IF ( BUCONF%LBUDGET_SV ) THEN
      IF (.NOT. LIMAP%LCRYSTAL_SHAPE) THEN
        ZWKBUD(:,:) = Z_CI_HIND(:,:) * PRHODJ(:,:) / PTSTEP
        CALL  TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NI)%PTR%ADD_PHY(D, 'HIND', ZWKBUD )
      ELSE
        DO ISH = 1, LIMAP%NNB_CRYSTAL_SHAPE
          ZWKBUD(:,:) = Z_SHCI_HIND(:, :, ISH) * PRHODJ(:, :) / PTSTEP
          CALL  TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NI+ISH-1)%PTR%ADD_PHY(D, 'HIND', ZWKBUD )
        END DO
      END IF
      DO IL = 1, LIMAP%NMOD_IFN
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_IFN_FREE - 1 + IL
        ZWKBUD(:,:) = PIFT(:,:, IL) * PRHODJ(:,:) / PTSTEP
        CALL TBUDGETS(IDX)%PTR%END_PHY(D, 'HIND', ZWKBUD )
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_IFN_NUCL - 1 + IL
        ZWKBUD(:,:) = PINT(:,:, IL) * PRHODJ(:,:) / PTSTEP
        CALL TBUDGETS(IDX)%PTR%END_PHY(D, 'HIND', ZWKBUD )
      END DO
    ...
    IF ( BUCONF%LBUDGET_TH ) ZWKBUD(:,:) = Z_TH_HINC(:,:) * PRHODJ(:,:) / PTSTEP
    IF ( BUCONF%LBUDGET_TH ) &
         CALL  TBUDGETS(NBUDGET_TH)%PTR%ADD_PHY(D, 'HINC', ZWKBUD )
    IF ( BUCONF%LBUDGET_RC ) ZWKBUD(:,:) = PTOT_RC_HINC(:,:) * PRHODJ(:,:) / PTSTEP
    IF ( BUCONF%LBUDGET_RC ) &
         CALL  TBUDGETS(NBUDGET_RC)%PTR%ADD_PHY(D, 'HINC', ZWKBUD )
    IF ( BUCONF%LBUDGET_RI ) ZWKBUD(:,:) = -PTOT_RC_HINC(:,:) * PRHODJ(:,:) / PTSTEP
    IF ( BUCONF%LBUDGET_RI ) &
         CALL  TBUDGETS(NBUDGET_RI)%PTR%ADD_PHY(D, 'HINC', ZWKBUD )
    IF ( BUCONF%LBUDGET_SV ) then
       IF (LIMAP%NMOM_C.GE.2) then
         ZWKBUD(:,:) = Z_CC_HINC(:,:) * PRHODJ(:,:) / PTSTEP
         CALL  TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NC)%PTR%ADD_PHY(D, 'HINC', ZWKBUD )
       END IF
      IF (.NOT. LIMAP%LCRYSTAL_SHAPE) THEN
         ZWKBUD(:,:) = -Z_CC_HINC(:,:) * PRHODJ(:,:) / PTSTEP
         CALL  TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NI)%PTR%ADD_PHY(D, 'HINC', ZWKBUD )
      ELSE
        DO ISH = 1, LIMAP%NNB_CRYSTAL_SHAPE
          ZWKBUD(:,:) = Z_SHCI_HIND(:, :, ISH) * PRHODJ(:, :) / PTSTEP
          CALL  TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NI+ISH-1)%PTR%ADD_PHY(D, 'HINC', ZWKBUD )
        END DO
      END IF
      DO IL = 1, LIMAP%NMOD_CCN
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_ACTI - 1 + IL
        ZWKBUD(:,:) = PNAT(:,:, IL) * PRHODJ(:,:) / PTSTEP
        CALL TBUDGETS(IDX)%PTR%END_PHY(D, 'HINC', ZWKBUD )
      END DO
      DO IL = 1, LIMAP%NMOD_IMM
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_IMM_NUCL - 1 + IL
        ZWKBUD(:,:) = PNIT(:,:, IL) * PRHODJ(:,:) / PTSTEP
...
IF (LIMAP%LNUCL .AND. LIMAP%NMOM_I>=2 .AND. LIMAP%LMEYERS) THEN
  ...
  IF ( BUCONF%LBU_ENABLE ) then
    ...
    IF ( BUCONF%LBUDGET_SV ) then
      ZWKBUD(:,:) = Z_CI_HIND(:,:) * PRHODJ(:,:) / PTSTEP
      CALL  TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NI)%PTR%ADD_PHY(D, 'HIND', ZWKBUD )
      IF (LIMAP%NMOD_IFN > 0 ) THEN
        ZWKBUD(:,:) = Z_CI_HIND(:,:) * PRHODJ(:,:) / PTSTEP
        CALL  TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_IFN_NUCL)%PTR%ADD_PHY(D, 'HIND', ZWKBUD )
      END IF
    END IF
    ...
    IF ( BUCONF%LBUDGET_SV ) then
      ...
      IF (LIMAP%NMOD_IFN > 0 ) THEN
        ZWKBUD(:,:) = -Z_CC_HINC(:,:) * PRHODJ(:,:) / PTSTEP
        CALL  TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_IFN_NUCL)%PTR%ADD_PHY(D, 'HINC', ZWKBUD )
      END IF
    END IF
  END IF
END IF
...
IF (LIMAP%LNUCL .AND. LIMAP%NMOM_I.EQ.1) THEN
  ...
  !
  IF ( BUCONF%LBU_ENABLE ) then
     IF ( BUCONF%LBUDGET_TH ) ZWKBUD(:,:) = ZRVHENIMR(:,:)*ZLSFACT(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_TH ) &
          CALL  TBUDGETS(NBUDGET_TH)%PTR%ADD_PHY(D, 'HIND', ZWKBUD )
     IF ( BUCONF%LBUDGET_RV ) ZWKBUD(:,:) = -ZRVHENIMR(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_RV ) &
          CALL  TBUDGETS(NBUDGET_RV)%PTR%ADD_PHY(D, 'HIND', ZWKBUD )
     IF ( BUCONF%LBUDGET_RI ) ZWKBUD(:,:) = ZRVHENIMR(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_RI ) &
          CALL  TBUDGETS(NBUDGET_RI)%PTR%ADD_PHY(D, 'HIND', ZWKBUD )
  END IF
!  Z_TH_HINC=0.
...
IF ( LIMAP%LNUCL .AND. LIMAP%LHHONI .AND. LIMAP%NMOD_CCN >= 1 .AND. LIMAP%NMOM_I.GE.2) THEN
  IF ( BUCONF%LBU_ENABLE ) then
     IF ( BUCONF%LBUDGET_TH ) ZWKBUD(:,:) = PTHT(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_TH ) &
          CALL TBUDGETS(NBUDGET_TH)%PTR%INIT_PHY(D, 'HONH', ZWKBUD )
     IF ( BUCONF%LBUDGET_RV ) ZWKBUD(:,:) = PRVT(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_RV ) &
          CALL TBUDGETS(NBUDGET_RV)%PTR%INIT_PHY(D, 'HONH', ZWKBUD )
     IF ( BUCONF%LBUDGET_RI ) ZWKBUD(:,:) = PRIT(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_RI ) &
          CALL TBUDGETS(NBUDGET_RI)%PTR%INIT_PHY(D, 'HONH', ZWKBUD )
    IF ( BUCONF%LBUDGET_SV ) THEN
      IF (.NOT. LIMAP%LCRYSTAL_SHAPE) THEN
        ZWKBUD(:,:) = PCIT(:,:)*PRHODJ(:,:)/PTSTEP
        CALL TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NI)%PTR%INIT_PHY(D,'HONH',ZWKBUD )
      ELSE
        DO ISH = 1, LIMAP%NNB_CRYSTAL_SHAPE
          ZWKBUD(:,:) = PCIT_SHAPE(:, :, ISH)*PRHODJ(:, :)/PTSTEP
          CALL TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NI+ISH-1)%PTR%INIT_PHY(D,'HONH', ZWKBUD )
        END DO
      END IF
      DO IL = 1, LIMAP%NMOD_CCN
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_FREE - 1 + IL
        ZWKBUD(:,:) = PNFT(:,:, IL) * PRHODJ(:,:) / PTSTEP
        CALL TBUDGETS(IDX)%PTR%INIT_PHY(D, 'HONH', ZWKBUD )
      END DO
      ZWKBUD(:,:) = PNHT(:,:)*PRHODJ(:,:)/PTSTEP
      CALL TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_HOM_HAZE)%PTR%INIT_PHY(D,'HONH',ZWKBUD)
    END IF
  END IF
...
!
  IF ( BUCONF%LBU_ENABLE ) then
     IF ( BUCONF%LBUDGET_TH ) ZWKBUD(:,:) = PTHT(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_TH ) &
          CALL TBUDGETS(NBUDGET_TH)%PTR%END_PHY(D, 'HONH', ZWKBUD )
     IF ( BUCONF%LBUDGET_RV ) ZWKBUD(:,:) = PRVT(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_RV ) &
          CALL TBUDGETS(NBUDGET_RV)%PTR%END_PHY(D, 'HONH', ZWKBUD )
     IF ( BUCONF%LBUDGET_RI ) ZWKBUD(:,:) = PRIT(:,:) * PRHODJ(:,:) / PTSTEP
     IF ( BUCONF%LBUDGET_RI ) &
          CALL TBUDGETS(NBUDGET_RI)%PTR%END_PHY(D, 'HONH', ZWKBUD )
    IF ( BUCONF%LBUDGET_SV ) then
      IF (.NOT. LIMAP%LCRYSTAL_SHAPE) THEN
        ZWKBUD(:,:) = PCIT(:,:)*PRHODJ(:,:)/PTSTEP
        CALL TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NI)%PTR%END_PHY(D,'HONH',ZWKBUD )
      ELSE
        DO ISH = 1, LIMAP%NNB_CRYSTAL_SHAPE
          ZWKBUD(:,:) = PCIT_SHAPE(:, :, ISH)*PRHODJ(:, :)/PTSTEP
          CALL TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_NI+ISH-1)%PTR%END_PHY(D,'HONH', ZWKBUD )
        END DO
      END IF
      DO IL = 1, LIMAP%NMOD_CCN
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_FREE - 1 + IL
        ZWKBUD(:,:) = PNFT(:,:, IL) * PRHODJ(:,:) / PTSTEP
        CALL TBUDGETS(IDX)%PTR%END_PHY(D, 'HONH', ZWKBUD )
      END DO
      ZWKBUD(:,:) = PNHT(:,:)*PRHODJ(:,:)/PTSTEP
      CALL TBUDGETS(NBUDGET_SV1-1+TNSV%NSV_LIMA_HOM_HAZE)%PTR%END_PHY(D,'HONH',ZWKBUD)
    END IF
  END IF
ENDIF
!
