...
IF (LDPACK) THEN
  ...
  IF (KLEN>0) THEN
    !Index computation
    DO JL=1,KLEN 
      PBUF1(JL) = MAX(1.00001, MIN(REAL(KNUM)-0.00001, P1 * LOG(PBUF1(JL)) + P2))
      KBUF2(JL) = INT(PBUF1(JL))
      PBUF1(JL) = PBUF1(JL) - REAL(KBUF2(JL))
    ENDDO

    !Interpolation and unpack
    DO JL=1,KLEN 
      PBUF2(JL) = PLT1(KBUF2(JL)+1) *  PBUF1(JL)       &
      &-PLT1(KBUF2(JL)  ) * (PBUF1(JL) - 1.0)
    ENDDO
    POUT1(:)=0.
    DO JL=1, KLEN
    ...
    IF(PRESENT(PLT2)) THEN
      DO JL=1,KLEN 
        PBUF2(JL) = PLT2(KBUF2(JL)+1) *  PBUF1(JL)       &
        &-PLT2(KBUF2(JL)  ) * (PBUF1(JL) - 1.0)
      ENDDO
      POUT2(:)=0.
      DO JL=1, KLEN
    ...
    IF(PRESENT(PLT3)) THEN
      DO JL=1,KLEN 
        PBUF2(JL) = PLT3(KBUF2(JL)+1) *  PBUF1(JL)       &
        &-PLT3(KBUF2(JL)  ) * (PBUF1(JL) - 1.0)
      ENDDO
      POUT3(:)=0.
      DO JL=1, KLEN
...
IF (LDPACK) THEN
  ...
  IF (KLEN>0) THEN
    !Index computation
    DO JL=1,KLEN 
      PBUF1(JL) = MAX(1.00001, MIN(REAL(KNUM1)-0.00001, P11 * LOG(PBUF1(JL)) + P12))
      KBUF1(JL) = INT(PBUF1(JL))
      PBUF1(JL) = PBUF1(JL) - REAL(KBUF1(JL))
      
      PBUF2(JL) = MAX(1.00001, MIN(REAL(KNUM2)-0.00001, P21 * LOG(PBUF2(JL)) + P22))
      KBUF2(JL) = INT(PBUF2(JL))
      PBUF2(JL) = PBUF2(JL) - REAL(KBUF2(JL))
    ENDDO
