...
!
  IF(HPROGRAM=='AROME' .OR. HPROGRAM=='PHYEX') THEN
    IF(.NOT. LPTSPLIT) THEN
      CTEMP="LPTSPLIT MUST BE .TRUE. WITH THIS PROGRAM: " // HPROGRAM
      CALL PRINT_MSG(NVERB_FATAL, 'GEN', 'MODD_PARAM_LIMA', CTEMP)
    ENDIF
    IF(LSPRO) THEN
      CTEMP="LSPRO MUST BE .FALSE. WITH THIS PROGRAM: " // HPROGRAM
      CALL PRINT_MSG(NVERB_FATAL, 'GEN', 'MODD_PARAM_LIMA', CTEMP)
    ENDIF
  ENDIF
ENDIF
