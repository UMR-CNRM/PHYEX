...
!$mnh_end_do()
  ...
  DO JL=1, KSIZE
    ...
    ELSE
      ZZW(JL) = EXP(CST%XALPI-CST%XBETAI/ZT(JL)-CST%XGAMI*LOG(ZT(JL)))
...
CALL ICE4_COMPUTE_PDF(CST, ICEP, ICED, PARAMI, KSIZE, &
...
DO JL=1, KSIZE
  ...
  IF (PARAMI%LOCND2) PATH(JL) = PATH(JL) + PBU_INST(JL, IRVDEPI)*PLSFACT(JL)
