...
#else
          *   PRST(JL)*(1+(ICEP%XFVELOS/PLBDAS(JL))**ICED%XALPHAS)**(-ICEP%XNUS+ICEP%XEXCRIMSS/ICED%XALPHAS) &
          * PRHODREF(JL)**(-ICED%XCEXVT+1.) &
          				      * (PLBDAS(JL)) ** (ICEP%XEXCRIMSS+ICEP%XBS)
#endif
        ENDIF
      ENDDO
    !
    ...
    !
      DO JL=1,KSIZE 
        IF(GRIM(JL))THEN
          ...
          * PRHODREF(JL)**(-ICED%XCEXVT)
#else
          * PRST(JL)*(1+(ICED%XFVELOS/PLBDAS(JL))**(ICED%XALPHAS))**(-ICED%XNUS+ICED%XEXCRIMSG/ICED%XALPHAS) &
          * PRHODREF(JL)**(-ICED%XCEXVT+1.) &
          * PLBDAS(JL)**(ICEP%XBS+ICEP%XEXCRIMSG)
#endif
        ENDIF
      ENDDO
...
#else
            PRS_TEND(JL,IRSRIMCG)=ICEP%XSRIMCG * PRST(JL)*PRHODREF(JL)*PLBDAS(JL)**(ICEP%XEXSRIMCG+ICEP%XBS)*(1.0-ZZW(JL))
#endif
...
#else
            ICEP%XSRIMCG3*ICEP%XSRIMCG2*PRST(JL)*PRHODREF(JL)*PLBDAS(JL)***ICEP%XEXSRIMCG2*(1.-ZZW2(JL)) - &
#endif
