...
#else
          *   PRST(JL)*(1+(ICED%XFVELOS/PLBDAS(JL))**ICED%XALPHAS)**(-ICED%XNUS+ICEP%XEXCRIMSS/ICED%XALPHAS) &
    ...
    !
      DO JL=1,KSIZE 
        IF(GRIM(JL))THEN
          ...
          * PRHODREF(JL)**(-ICED%XCEXVT)
#else
          * PRST(JL)*(1+(ICED%XFVELOS/PLBDAS(JL))**(ICED%XALPHAS))**(-ICED%XNUS+ICEP%XEXCRIMSG/ICED%XALPHAS) &
          * PRHODREF(JL)**(-ICED%XCEXVT+1.) &
...
#else
            PRS_TEND(JL,IRSRIMCG)=ICEP%XSRIMCG * PRST(JL)*PRHODREF(JL)*PLBDAS(JL)**(ICEP%XEXSRIMCG+ICED%XBS)*(1.0-ZZW(JL))
#endif
