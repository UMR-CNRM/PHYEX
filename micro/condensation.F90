...
!     ######spl
    SUBROUTINE CONDENSATION(D, CST, ICEP, NEBN, TURBN, &
                           ...
                           &PRCRIAUTI, PRCRIAUTC, PXACRIAUTI, PXBCRIAUTI,                                   &
                           &PLV, PLS, PCPH,                                                                 &
...
REAL, DIMENSION(D%NIJT),       OPTIONAL, INTENT(IN)    :: PRCRIAUTI ! SPP for microphysics
REAL, DIMENSION(D%NIJT),       OPTIONAL, INTENT(IN)    :: PRCRIAUTC ! SPP for microphysics
REAL, DIMENSION(D%NIJT),       OPTIONAL, INTENT(IN)    :: PXACRIAUTI! SPP for microphysics
REAL, DIMENSION(D%NIJT),       OPTIONAL, INTENT(IN)    :: PXBCRIAUTI! SPP for microphysics
...
! end OCND2
! SPP helper variables
REAL,    DIMENSION(D%NIJT)  :: ZRCRIAUTI, ZRCRIAUTC, ZXACRIAUTI, ZXBCRIAUTI
LOGICAL :: ALL_SPP_FIELDS, ANY_SPP_FIELD
...
ANY_SPP_FIELD = ALL((/PRESENT(PRCRIAUTI), PRESENT(PRCRIAUTC), PRESENT(PXACRIAUTI), PRESENT(PXBCRIAUTI)/))
ALL_SPP_FIELDS = ANY((/PRESENT(PRCRIAUTI), PRESENT(PRCRIAUTC), PRESENT(PXACRIAUTI), PRESENT(PXBCRIAUTI)/))

IF ( ANY_SPP_FIELD .AND. .NOT. ALL_SPP_FIELDS ) THEN
  CALL ABOR1("Erroneous combination of optional fields in condensation()")
ENDIF

IF ( ALL_SPP_FIELDS) THEN
  ZRCRIAUTI(:) = PRCRIAUTI(:)
  ZRCRIAUTC(:) = PRCRIAUTC(:)
  ZXACRIAUTI(:) = PXACRIAUTI(:)
  ZXBCRIAUTI(:) = PXBCRIAUTI(:)
ELSE
  ZRCRIAUTI(:) = ICEP%XCRIAUTI
  ZRCRIAUTC(:) = ICEP%XCRIAUTC
  ZXACRIAUTI(:) = ICEP%XACRIAUTI
  ZXBCRIAUTI(:) = ICEP%XBCRIAUTI
ENDIF
  ...
  IF(HCONDENS == 'GAUS') THEN
    ...
    IF(PRESENT(PHLI_HCF) .AND. PRESENT(PHLI_HRI))THEN
        IF(ZFRAC(JIJ,JK) > 1.E-20)THEN
          ZCRIAUTI(JIJ,JK)=MIN(PRCRIAUTI(JIJ),10**(PXACRIAUTI(JIJ)*(PT(JIJ,JK)-CST%XTT)+PXBCRIAUTI(JIJ)))
