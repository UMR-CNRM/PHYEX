...
USE MODD_PARAM_ICE, ONLY : PARAM_ICE_INIT, PARAM_ICE
...
USE MODN_SERIES_n
USE MODN_STATION_n
USE MODD_TURB_n, ONLY: TURBN_INIT, CSUBG_AUCV_RC, CTOM, CTURBDIM, LRMC01, LSIGMAS, LSUBG_COND
USE MODN_TURB_CLOUD
USE MODN_VISCOSITY
USE MODD_FIRE
USE MODN_FIRE
...
IF (KMI == 1) THEN                                               
  ...
  CALL PARAM_ICE_INIT(CPROGRAM, ILUSEG, .FALSE., ILUOUT, .FALSE., .TRUE., .FALSE., 0)
...
!
CALL PARAM_ICE_INIT(CPROGRAM, 0, .FALSE., ILUOUT, .FALSE., .FALSE., .TRUE., 0)
IF( CCLOUD == 'C3R5' ) THEN
