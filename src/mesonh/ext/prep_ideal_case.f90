...
ELSE
  ...
  CALL FILL_DIMPHYEX(YLDIMPHYEX, NIU, NJU, NKU, .TRUE.)
  DO JK=1, NKU
    CALL TH_R_FROM_THL_RT(YLDIMPHYEX, CST, NEBN, 'T',ZFRAC_ICE(:,:,JK),XPABST(:,:,JK), &
                          ZTHL(:,:,JK),ZRT(:,:,JK),XTHT(:,:,JK),XRT(:,:,JK,1), &
                          XRT(:,:,JK,2),XRT(:,:,JK,4),ZRSATW(:,:,JK), ZRSATI(:,:,JK),OOCEAN=.FALSE.,&
                          PBUF=ZBUF)
  ENDDO
END IF
  DEALLOCATE(ZEXN)         
