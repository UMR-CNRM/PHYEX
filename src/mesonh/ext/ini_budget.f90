...
use modd_budget, only: nbudgets, tbudgets,  tbudgets_ptr,                          &
...
use mode_budget_phy, only: tbudgets_mnh
...
allocate( tbudgets_ptr( nbudgets ) )
allocate( tbudgets_mnh( nbudgets ) )
do ibudget=1, nbudgets
  tbudgets_mnh(ibudget)%tbudget => tbudgets(ibudget)
  tbudgets_ptr(ibudget)%ptr => tbudgets_mnh(ibudget)
enddo
...
#else
  !Do the Move_alloc by hand to workaround a Cray CCE compiler bug (version 18.0.0)
  if ( allocated( hbulist ) ) deallocate( hbulist )
  allocate( hbulist(size(ybulist_new)) )
  hbulist(:) = ybulist_new(:)
#endif
