...
!
      SUBROUTINE READ_FIELD(KOCEMI,TPINIFILE,KIU,KJU,KKU,                    &
            ...
            PSIGS,PSRCT,PCLDFR,PICEFR,PBL_DEPTH,PSBL_DEPTH,                  &
            PWTHVMF,PWUMF,PWVMF,PPHC,PPHR,                                   &
...
REAL, DIMENSION(:,:,:),    INTENT(OUT) :: PWUMF           ! MassFlux U flux
REAL, DIMENSION(:,:,:),    INTENT(OUT) :: PWVMF           ! MassFlux V flux
!
...
SELECT CASE(HGETTKET)                   
  CASE('READ') 
    IF (CSCONV=='EDKF') THEN 
      ...
      CALL IO_Field_read(TPINIFILE,'WUMF',PWUMF)
      CALL IO_Field_read(TPINIFILE,'WVMF',PWVMF)
    ELSE
      PWTHVMF(:,:,:)=0
      PWUMF(:,:,:)=0
      PWVMF(:,:,:)=0
