...
IF (CTURB /= 'NONE') THEN
  ...
  ALLOCATE(XWUMF(IIU,IJU,IKU))
  ALLOCATE(XWVMF(IIU,IJU,IKU))
...
ELSE
  ...
  ALLOCATE(XWUMF(0,0,0))
  ALLOCATE(XWVMF(0,0,0))
  ALLOCATE(XDYP(0,0,0))
  ALLOCATE(XTHP(0,0,0))
  ALLOCATE(XTR(0,0,0))
...
ALLOCATE(XHLC_HRC_MF(IIU,IJU,IKU)) ; XHLC_HRC_MF=0.
...
!
IF (CCLOUD=='LIMA') THEN
  PHYAERO%LORILAM       = LORILAM
  PHYAERO%XINISIGI      = XINISIGI
  PHYAERO%XINISIGJ      = XINISIGJ
  PHYAERO%XINIRADIUSJ   = XINIRADIUSJ
  PHYAERO%CRGUNIT       = CRGUNIT
  IF(ALLOCATED(XRHOI)) PHYAERO%XRHOI => XRHOI
  PHYAERO%NCARB         = NCARB
  PHYAERO%NSOA          = NSOA
  PHYAERO%NSP           = NSP
  PHYAERO%JP_AER_OC     = JP_AER_OC
  PHYAERO%JP_AER_H2O    = JP_AER_H2O
  PHYAERO%JP_AER_DST    = JP_AER_DST
  PHYAERO%JP_AER_BC     = JP_AER_BC
  PHYAERO%JP_AER_OC     = JP_AER_OC
  PHYAERO%JP_AER_SO4    = JP_AER_SO4
  PHYAERO%LSALT         = LSALT
  PHYAERO%NMODE_SLT     = NMODE_SLT
  PHYAERO%XINISIG_SLT   => XINISIG_SLT
  PHYAERO%CRGUNITS      = CRGUNITS
  PHYAERO%XINIRADIUS_SLT=> XINIRADIUS_SLT
  PHYAERO%XDENSITY_SALT = XDENSITY_SALT
  PHYAERO%LDUST         = LDUST
  PHYAERO%JPDUSTORDER   = JPDUSTORDER
  PHYAERO%XINIRADIUS    => XINIRADIUS
  PHYAERO%CRGUNITD      = CRGUNITD
  PHYAERO%XINISIG       => XINISIG
  PHYAERO%XDENSITY_DUST = XDENSITY_DUST
  CALL INIT_AEROSOL_PROPERTIES(PHYAERO)
  IF(ASSOCIATED(PHYAERO%XRHOI)) THEN
    IF(.NOT. ALLOCATED(XRHOI)) ALLOCATE(XRHOI(SIZE(PHYAERO%XRHOI)))
    XRHOI(:)=PHYAERO%XRHOI(:)
  ENDIF
ENDIF
!
