...
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PZZF
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PRHODJ
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PEXNREF
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PRHODREF
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PPABSM
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PTHT
REAL, DIMENSION(KLON*2,1,KLEV,KRR)   :: ZP_PRT
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PSIGS
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PMFCONV
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PRC_MF, ZP_PRI_MF, ZP_PCF_MF
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PTHS
REAL, DIMENSION(KLON*2,1,KLEV,KRR)   :: ZP_PRS
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PSRCS
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PCLDFR
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PHLC_HRC
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PHLC_HCF
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PHLI_HRI
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_PHLI_HCF
REAL, DIMENSION(KLON*2,1,KLEV,0:KRR) :: ZP_ZRS
REAL, DIMENSION(KLON*2,1)            :: ZP_ZSIGQSAT, ZP_ZICE_CLD_WGT
REAL, DIMENSION(KLON*2,1,KLEV)       :: ZP_ZZZ
!------------------------------------------------------------------------------
!
...
CALL FILL_DIMPHYEX(YLDIMPHYEX, KLON*2, 1, KLEV, 0, KFDIA)
...
ZP_PZZF(:,:,:)=HUGE(PZZF)
ZP_PRHODJ(:,:,:)=HUGE(PZZF)
ZP_PEXNREF(:,:,:)=HUGE(PZZF)
ZP_PRHODREF(:,:,:)=HUGE(PZZF)
ZP_PPABSM(:,:,:)=HUGE(PZZF)
ZP_PTHT(:,:,:)=HUGE(PZZF)
ZP_PRT(:,:,:,:)=HUGE(PZZF)
ZP_PSIGS(:,:,:)=HUGE(PZZF)
ZP_PMFCONV(:,:,:)=HUGE(PZZF)
ZP_PRC_MF(:,:,:)=HUGE(PZZF)
ZP_PRI_MF(:,:,:)=HUGE(PZZF)
ZP_PCF_MF(:,:,:)=HUGE(PZZF)
ZP_PTHS(:,:,:)=HUGE(PZZF)
...
ZP_PSRCS(:,:,:)=HUGE(PZZF)
ZP_PCLDFR(:,:,:)=HUGE(PZZF)
ZP_PHLC_HRC(:,:,:)=HUGE(PZZF)
ZP_PHLC_HCF(:,:,:)=HUGE(PZZF)
ZP_PHLI_HRI(:,:,:)=HUGE(PZZF)
ZP_PHLI_HCF(:,:,:)=HUGE(PZZF)
ZP_ZRS(:,:,:,:)=HUGE(PZZF)
ZP_ZSIGQSAT(:,:)=HUGE(PZZF)
ZP_ZICE_CLD_WGT(:,:)=HUGE(PZZF)
ZP_ZZZ(:,:,:)=HUGE(PZZF)

ZP_PZZF(:KLON,:,:)=PZZF
ZP_PRHODJ(:KLON,:,:)=PRHODJ
ZP_PEXNREF(:KLON,:,:)=PEXNREF
ZP_PRHODREF(:KLON,:,:)=PRHODREF
ZP_PPABSM(:KLON,:,:)=PPABSM
ZP_PTHT(:KLON,:,:)=PTHT
ZP_PRT(:KLON,:,:,:)=PRT
ZP_PSIGS(:KLON,:,:)=PSIGS
ZP_PMFCONV(:KLON,:,:)=PMFCONV
...
ZP_PRS(:KLON,:,:,:)=PRS
!ZP_PSRCS(:KLON,:,:)=PSRCS
ZP_PCLDFR(:KLON,:,:)=PCLDFR
!ZP_PHLC_HRC(:KLON,:,:)=PHLC_HRC
!ZP_PHLC_HCF(:KLON,:,:)=PHLC_HCF
!ZP_PHLI_HRI(:KLON,:,:)=PHLI_HRI
!ZP_PHLI_HCF(:KLON,:,:)=PHLI_HCF
ZP_ZRS(:KLON,:,:,:)=ZRS
ZP_ZSIGQSAT(:KLON,:)=ZSIGQSAT
ZP_ZICE_CLD_WGT(:KLON,:)=ZICE_CLD_WGT
ZP_ZZZ(:KLON,:,:)=ZZZ

TBUCONF%LBUDGET_U = .FALSE.
TBUCONF%LBUDGET_V = .FALSE.
TBUCONF%LBUDGET_W = .FALSE.
TBUCONF%LBUDGET_TH = .FALSE.
TBUCONF%LBUDGET_RV = .FALSE.
TBUCONF%LBUDGET_RC = .FALSE.
TBUCONF%LBUDGET_RI = .FALSE.
TBUCONF%LBUDGET_SV = .FALSE.
TBUCONF%LBUDGET_TKE = .FALSE.
...
IF (KRR==6) THEN
  CALL ICE_ADJUST ( YLDIMPHYEX, CST=CST, ICEP=RAIN_ICE_PARAM, NEB=NEB, BUCONF=TBUCONF, KRR=KRR,&
    ...
    & PTSTEP=ZTWOTSTEP,PSIGQSAT=ZP_ZSIGQSAT, &
    & PRHODJ=ZP_PRHODJ ,PEXNREF=ZP_PEXNREF, PRHODREF=ZP_PRHODREF,   &
    & PSIGS=ZP_PSIGS, LMFCONV=.TRUE., PMFCONV=ZP_PMFCONV, PPABST=ZP_PPABSM, PZZ=ZP_ZZZ,    &
    & PEXN=ZP_PEXNREF, PCF_MF=ZP_PCF_MF,PRC_MF=ZP_PRC_MF,PRI_MF=ZP_PRI_MF, &
    & PRV=ZP_ZRS(:,:,:,1), PRC=ZP_ZRS(:,:,:,2),  &
    & PRVS=ZP_PRS(:,:,:,1), PRCS=ZP_PRS(:,:,:,2), &
    & PTH=ZP_ZRS(:,:,:,0), PTHS=ZP_PTHS,OCOMPUTE_SRC=.TRUE.,PSRCS=ZP_PSRCS, PCLDFR=ZP_PCLDFR, &
    & PRR=ZP_ZRS(:,:,:,3), &
    & PRI=ZP_ZRS(:,:,:,4), PRIS=ZP_PRS(:,:,:,4), &
    & PRS=ZP_ZRS(:,:,:,5), &
    & PRG=ZP_ZRS(:,:,:,6), &
    & PHLC_HRC=ZP_PHLC_HRC(:,:,:), PHLC_HCF=ZP_PHLC_HCF(:,:,:), &
    & PHLI_HRI=ZP_PHLI_HRI(:,:,:), PHLI_HCF=ZP_PHLI_HCF(:,:,:), &
    & PICE_CLD_WGT=ZP_ZICE_CLD_WGT(:,:), &
...
ELSE
  CALL ICE_ADJUST ( YLDIMPHYEX, CST=CST, ICEP=RAIN_ICE_PARAM, NEB=NEB, BUCONF=TBUCONF, KRR=KRR,&
    ...
    & TBUDGETS=YLBUDGET, KBUDGETS=SIZE(YLBUDGET))
ENDIF
TBUCONF%LBUDGET_U = .TRUE.
TBUCONF%LBUDGET_V = .TRUE.
TBUCONF%LBUDGET_W = .TRUE.
TBUCONF%LBUDGET_TH = .TRUE.
TBUCONF%LBUDGET_RV = .TRUE.
TBUCONF%LBUDGET_RC = .TRUE.
TBUCONF%LBUDGET_RI = .TRUE.
TBUCONF%LBUDGET_SV = .TRUE.
TBUCONF%LBUDGET_TKE = .TRUE.

!PZZF(:,:,:)=ZP_PZZF(:KLON,:,:)
!PRHODJ(:,:,:)=ZP_PRHODJ(:KLON,:,:)
!PEXNREF(:,:,:)=ZP_PEXNREF(:KLON,:,:)
!PRHODREF(:,:,:)=ZP_PRHODREF(:KLON,:,:)
!PPABSM(:,:,:)=ZP_PPABSM(:KLON,:,:)
!PTHT(:,:,:)=ZP_PTHT(:KLON,:,:)
PRT(:,:,:,:)=ZP_PRT(:KLON,:,:,:)
!PSIGS(:,:,:)=ZP_PSIGS(:KLON,:,:)
!PMFCONV(:,:,:)=ZP_PMFCONV(:KLON,:,:)
...
PRS(:,:,:,:)=ZP_PRS(:KLON,:,:,:)
PSRCS(:,:,:)=ZP_PSRCS(:KLON,:,:)
PCLDFR(:,:,:)=ZP_PCLDFR(:KLON,:,:)
PHLC_HRC(:,:,:)=ZP_PHLC_HRC(:KLON,:,:)
PHLC_HCF(:,:,:)=ZP_PHLC_HCF(:KLON,:,:)
PHLI_HRI(:,:,:)=ZP_PHLI_HRI(:KLON,:,:)
PHLI_HCF(:,:,:)=ZP_PHLI_HCF(:KLON,:,:)
!ZRS(:,:,:,:)=ZP_ZRS(:KLON,:,:,:)
!ZSIGQSAT(:,:)=ZP_ZSIGQSAT(:KLON,:)
!ZICE_CLD_WGT(:,:)=ZP_ZICE_CLD_WGT(:KLON,:)
!ZZZ(:,:,:)=ZP_ZZZ(:KLON,:,:)
