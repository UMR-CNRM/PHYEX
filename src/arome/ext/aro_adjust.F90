...
IF (YSPP_PSIGQSAT%LPERT) THEN
 CALL APPLY_SPP(YSPP_PSIGQSAT,KLON,1,KLON,NEBN%VSIGQSAT,ZSIGQSAT)
ELSE
 ZSIGQSAT(:,:) = NEBN%VSIGQSAT
ENDIF

IF (YSPP_ICE_CLD_WGT%LPERT) THEN
 CALL APPLY_SPP(YSPP_ICE_CLD_WGT,KLON,1,KLON,RAIN_ICE_PARAMN%XFRMIN(21),ZICE_CLD_WGT)
ELSE
 ZICE_CLD_WGT(:,:) = RAIN_ICE_PARAMN%XFRMIN(21)
ENDIF
...
!
IF (KRR==6) THEN
  CALL ICE_ADJUST ( YLDIMPHYEX, CST=CST, ICEP=RAIN_ICE_PARAMN, NEBN=NEBN, TURBN=TURBN, &
    & PARAMI=PARAM_ICEN, BUCONF=TBUCONF, KRR=KRR,&
    & HBUNAME=HBUNAME, &
