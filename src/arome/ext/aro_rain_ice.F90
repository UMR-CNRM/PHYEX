...
!
!
IF (CMICRO=='ICE4' .AND. PARAM_ICEN%LRED) THEN
    CALL RAIN_ICE(  YLDIMPHYEX, CST, PARAM_ICEN, RAIN_ICE_PARAMN, &
                 &  RAIN_ICE_DESCRN, TURBN, TBUCONF, &
                 &  PTSTEP=2*PTSTEP, &
...
ELSEIF (CMICRO=='ICE3' .AND. PARAM_ICEN%LRED) THEN
    CALL RAIN_ICE(  YLDIMPHYEX, CST, PARAM_ICEN, RAIN_ICE_PARAMN, &
                 &  RAIN_ICE_DESCRN, TURBN, TBUCONF, &
                 &  PTSTEP=2*PTSTEP, &
                 &  KRR=KRR, PEXN=PEXNREF,            &
...
ELSEIF (CMICRO=='ICE4' .AND. .NOT. PARAM_ICEN%LRED) THEN
    IF (YSPP_ICENU%LPERT) THEN
     CALL APPLY_SPP(YSPP_ICENU,KLON,1,KLON,RAIN_ICE_PARAMN%XFRMIN(9),ZICENU)
    ELSE
     ZICENU(:,:) = RAIN_ICE_PARAMN%XFRMIN(9)
    ENDIF
   
    IF (YSPP_KGN_ACON%LPERT) THEN
     CALL APPLY_SPP(YSPP_KGN_ACON,KLON,1,KLON,RAIN_ICE_PARAMN%XFRMIN(10),ZKGN_ACON)
    ELSE
     ZKGN_ACON(:,:) = RAIN_ICE_PARAMN%XFRMIN(10)
    ENDIF
   
    IF (YSPP_KGN_SBGR%LPERT) THEN
     CALL APPLY_SPP(YSPP_KGN_SBGR,KLON,1,KLON,RAIN_ICE_PARAMN%XFRMIN(11),ZKGN_SBGR)
    ELSE
     ZKGN_SBGR(:,:) = RAIN_ICE_PARAMN%XFRMIN(11)
    ENDIF
    CALL RAIN_ICE_OLD( OSEDIC=PARAM_ICEN%LSEDIC, OCND2=PARAM_ICEN%LOCND2, LKOGAN=LKOGAN, LMODICEDEP=LMODICEDEP, &
                 &  HSEDIM=PARAM_ICEN%CSEDIM, HSUBG_AUCV_RC=TURBN%CSUBG_AUCV_RC, &
                 &  OWARM=PARAM_ICEN%LWARM,KKA=KKA,KKU=KKU,KKL=KKL,KSPLITR=CLOUDPARN%NSPLITR, &
