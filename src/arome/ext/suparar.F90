...
IF ( CMICRO /= 'ICE3' .AND. CMICRO /= 'ICE4' .AND. CMICRO /= 'LIMA') THEN
  CALL ABOR1("AROME Microphysics must be ICE3, ICE4 or LIMA")
ENDIF
...
WRITE(UNIT=KULOUT,FMT='('' LOSUBG_COND = '',L5,'' CSUBG_AUCV_RC = '',A4,'' CSUBG_AUCV_RI = '',A4,&
 & '' LOSIGMAS ='',L5,'' LSQUALL = '',L5,&
 & '' LOLSMC = '',L5,'' LOTOWNC = '',L5,'' VSIGQSAT = '',F6.2)')&
 & LOSUBG_COND, CSUBG_AUCV_RC, CSUBG_AUCV_RI, LOSIGMAS,LSQUALL,LOLSMC,LOTOWNC,VSIGQSAT
WRITE(UNIT=KULOUT,FMT='('' CCONDENS = '', A80, '' CSUBG_MF_PDF = '', A80)')&
& CCONDENS, CSUBG_MF_PDF
