...
REAL,DIMENSION(KLON*2,1,KLEV+2)    :: ZP_ZDXX,ZP_ZDYY,ZP_ZDZZ,ZP_ZDZX,ZP_ZDZY,ZP_ZZZ
REAL,DIMENSION(KLON*2,1,KLEV+2)    :: &
   & ZP_PRHODJ,ZP_PTHVREF,                              &
   & ZP_PPABSM,ZP_PUM,ZP_PVM,ZP_PWM,ZP_PTKEM,ZP_PSRCM,ZP_PTHM,                  &
   & ZP_PLENGTHM,ZP_PLENGTHH,ZP_MFMOIST,                            &
   & ZP_ZCEI,    &
   & ZP_PRUS,ZP_PRVS,ZP_PRWS,ZP_PRTHS,ZP_PRTKES_OUT,         &
   & ZP_PSIGS,                                         &
   & ZP_PFLXZTHVMF,ZP_ZWTH,ZP_ZWRC,ZP_PDP,ZP_PTP,ZP_PTDIFF,ZP_PTDISS,&
   & ZP_PEDR,ZP_PTPMF,&
   & ZP_PDRUS_TURB,ZP_PDRVS_TURB,          &
   & ZP_PDRTHLS_TURB,ZP_PDRRTS_TURB

REAL,DIMENSION(KLON*2,1) :: ZP_PSFTH,ZP_PSFRV,ZP_PSFU,ZP_PSFV,      &
                        & ZP_ZBL_DEPTH,ZP_ZSBL_DEPTH                             
REAL, DIMENSION(KLON*2,1,KSV) :: ZP_PSFSV
REAL,DIMENSION(KLON*2,1,KLEV+2,KSV) :: ZP_ZWSV,ZP_ZSVM,ZP_ZRSVS,ZP_ZDRSVS_TURB       ! scalar flux
REAL, DIMENSION(KLON*2,1,KLEV+2,KRR) :: ZP_ZRM, ZP_ZRRS
!
...
CALL FILL_DIMPHYEX(YLDIMPHYEX, KLON*2, 1, KLEV+2, JPVEXT_TURB, KLON)
!
!
!
!------------------------------------------------------------------------------
...
ZP_ZDZY(:,:,:) = HUGE(ZDZY)
ZP_ZZZ(:,:,:) = HUGE(ZZZ)
ZP_PRHODJ(:,:,:) = HUGE(PRHODJ)
ZP_PTHVREF(:,:,:) = HUGE(PTHVREF)
ZP_PPABSM(:,:,:) = HUGE(PPABSM)
ZP_PUM(:,:,:) = HUGE(PUM)
ZP_PVM(:,:,:) = HUGE(PVM)
ZP_PWM(:,:,:) = HUGE(PWM)
ZP_PTKEM(:,:,:) = HUGE(PTKEM)
ZP_PSRCM(:,:,:) = HUGE(PSRCM)
ZP_PTHM(:,:,:) = HUGE(PTHM)
ZP_PLENGTHM(:,:,:) = HUGE(PLENGTHM)
ZP_PLENGTHH(:,:,:) = HUGE(PLENGTHH)
ZP_MFMOIST(:,:,:) = HUGE(MFMOIST)
ZP_ZCEI(:,:,:) = HUGE(ZCEI)
ZP_PRUS(:,:,:) = HUGE(PRUS)
ZP_PRVS(:,:,:) = HUGE(PRVS)
ZP_PRWS(:,:,:) = HUGE(PRWS)
ZP_PRTHS(:,:,:) = HUGE(PRTHS)
ZP_PRTKES_OUT(:,:,:) = HUGE(PRTKES_OUT)
ZP_PSFTH(:,:) = HUGE(PSFTH)
ZP_PSFRV(:,:) = HUGE(PSFRV)
ZP_PSFU(:,:) = HUGE(PSFU)
ZP_PSFV(:,:) = HUGE(PSFV)
ZP_ZBL_DEPTH(:,:) = HUGE(ZBL_DEPTH)
ZP_ZSBL_DEPTH(:,:) = HUGE(ZSBL_DEPTH)                      
ZP_PSFSV(:,:,:) = HUGE(PSFSV)
ZP_ZRRS(:,:,:,:) = HUGE(ZRRS)
ZP_ZSVM(:,:,:,:) = HUGE(ZSVM)
ZP_ZRSVS(:,:,:,:) = HUGE(ZRSVS)
...
ZP_PSRCM(:KLON,:,:) = PSRCM
ZP_PTHM(:KLON,:,:) = PTHM
ZP_PLENGTHM(:KLON,:,:) = PLENGTHM
ZP_PLENGTHH(:KLON,:,:) = PLENGTHH
ZP_MFMOIST(:KLON,:,:) = MFMOIST
ZP_ZCEI(:KLON,:,:) = ZCEI
ZP_PRUS(:KLON,:,:) = PRUS
ZP_PRVS(:KLON,:,:) = PRVS
ZP_PRWS(:KLON,:,:) = PRWS
...
ZP_PRTKES_OUT(:KLON,:,:) = PRTKES_OUT
ZP_PSFTH(:KLON,:) = PSFTH
ZP_PSFRV(:KLON,:) = PSFRV
ZP_PSFU(:KLON,:) = PSFU
ZP_PSFV(:KLON,:) = PSFV
ZP_ZBL_DEPTH(:KLON,:) = ZBL_DEPTH
ZP_ZSBL_DEPTH(:KLON,:) = ZSBL_DEPTH                      
ZP_PSFSV(:KLON,:,:) = PSFSV
ZP_ZRRS(:KLON,:,:,:) = ZRRS
ZP_ZSVM(:KLON,:,:,:) = ZSVM
ZP_ZRSVS(:KLON,:,:,:) = ZRSVS
...
CALL TURB (CST,CSTURB,TBUCONF,TURBN, YLDIMPHYEX,&
   ...
   & ZIMPL,                                    &
   & 2*PTSTEP,ZTFILE,                                      &
   & ZP_ZDXX,ZP_ZDYY,ZP_ZDZZ,ZP_ZDZX,ZP_ZDZY,ZP_ZZZ,          &
   & ZDIRCOSXW,ZDIRCOSYW,ZDIRCOSZW,ZCOSSLOPE,ZSINSLOPE,    &
   & ZP_PRHODJ,ZP_PTHVREF,                              &
   & ZP_PSFTH,ZP_PSFRV,ZP_PSFSV,ZP_PSFU,ZP_PSFV,                          &
   & ZP_PPABSM,ZP_PUM,ZP_PVM,ZP_PWM,ZP_PTKEM,ZP_ZSVM,ZP_PSRCM,                  &
   & ZP_PLENGTHM,ZP_PLENGTHH,ZP_MFMOIST,                            &
   & ZP_ZBL_DEPTH,ZP_ZSBL_DEPTH,                                 &
   & ZP_ZCEI,ZCEI_MIN,ZCEI_MAX,ZCOEF_AMPL_SAT,    &
   & ZP_PTHM,ZP_ZRM, &
   & ZP_PRUS,ZP_PRVS,ZP_PRWS,ZP_PRTHS,ZP_ZRRS,ZP_ZRSVS,ZP_PRTKES_OUT,         &
   & ZP_PSIGS,                                         &
   & ZP_PFLXZTHVMF,ZP_ZWTH,ZP_ZWRC,ZP_ZWSV,ZP_PDP,ZP_PTP,ZP_PTDIFF,ZP_PTDISS,&
   & YLBUDGET, KBUDGETS=SIZE(YLBUDGET),PEDR=ZP_PEDR,PTPMF=ZP_PTPMF,&
   & PDRUS_TURB=ZP_PDRUS_TURB,PDRVS_TURB=ZP_PDRVS_TURB,          &
   & PDRTHLS_TURB=ZP_PDRTHLS_TURB,PDRRTS_TURB=ZP_PDRRTS_TURB,PDRSVS_TURB=ZP_ZDRSVS_TURB)
!
...
PDP(:,:,:) = ZP_PDP(:KLON,:,:)
PTP(:,:,:) = ZP_PTP(:KLON,:,:) 
PTDIFF(:,:,:) = ZP_PTDIFF(:KLON,:,:)
PTDISS(:,:,:) = ZP_PTDISS(:KLON,:,:)
PEDR(:,:,:) = ZP_PEDR(:KLON,:,:)
PTPMF(:,:,:) = ZP_PTPMF(:KLON,:,:)
PDRUS_TURB(:,:,:) = ZP_PDRUS_TURB(:KLON,:,:)
PDRVS_TURB(:,:,:) = ZP_PDRVS_TURB(:KLON,:,:)
PDRTHLS_TURB(:,:,:) = ZP_PDRTHLS_TURB(:KLON,:,:)
PDRRTS_TURB(:,:,:) = ZP_PDRRTS_TURB(:KLON,:,:)
ZWSV(:,:,:,:) = ZP_ZWSV(:KLON,:,:,:)
ZDRSVS_TURB(:,:,:,:) = ZP_ZDRSVS_TURB(:KLON,:,:,:)

! INOUT exchange
ZBL_DEPTH(:,:) = ZP_ZBL_DEPTH(:KLON,:) 
ZSBL_DEPTH(:,:) = ZP_ZSBL_DEPTH(:KLON,:)
PTHM(:,:,:) = ZP_PTHM(:KLON,:,:)
PRUS(:,:,:) = ZP_PRUS(:KLON,:,:)
PRVS(:,:,:) = ZP_PRVS(:KLON,:,:)
PRWS(:,:,:) = ZP_PRWS(:KLON,:,:)
PRTHS(:,:,:) = ZP_PRTHS(:KLON,:,:)
PRTKES_OUT(:,:,:) = ZP_PRTKES_OUT(:KLON,:,:)
ZRRS(:,:,:,:) = ZP_ZRRS(:KLON,:,:,:)
ZRSVS(:,:,:,:) = ZP_ZRSVS(:KLON,:,:,:)
ZRM(:,:,:,:) = ZP_ZRM(:KLON,:,:,:)
!
