...
ELSE
  ...
  !print*, 'HPROGRAM      =', HPROGRAM      
  !print*, 'O2D           =', O2D           
  !print*, 'ONOMIXLG      =', ONOMIXLG      
  !print*, 'OFLAT         =', OFLAT         
  !print*, 'LLES_CALL     =', LLES_CALL     
  !print*, 'OCOUPLES      =', OCOUPLES      
  !print*, 'OBLOWSNOW     =', OBLOWSNOW     
  !print*, 'OCOMPUTE_SRC  =', OCOMPUTE_SRC  
  !print*, 'OOCEAN        =', OOCEAN        
  !print*, 'ODEEPOC       =', ODEEPOC       
  !print*, 'CMICRO        =', CMICRO        
  !print*, 'ZTFILE%LOPENED=', ZTFILE%LOPENED
  !print*, 'ZCEI_MAX      =', ZCEI_MAX      
  !print*, 'ZCEI_MIN      =', ZCEI_MIN      
  !print*, 'ZCOEF_AMPL_SAT=', ZCOEF_AMPL_SAT
  !print*, 'PTSTEP        =', PTSTEP
  ...
  IF(LFILEEXISTS) THEN
    ...
    WRITE(IFILE) ZDZY
    WRITE(IFILE) ZZZ
    WRITE(IFILE) ZDIRCOSXW
    WRITE(IFILE) ZDIRCOSYW
    WRITE(IFILE) ZDIRCOSZW
    WRITE(IFILE) ZCOSSLOPE
    WRITE(IFILE) ZSINSLOPE
    WRITE(IFILE) PRHODJ
    WRITE(IFILE) PTHVREF
    WRITE(IFILE) PSFTH
    WRITE(IFILE) PSFRV
    ...
    WRITE(IFILE) ZSVM(:,:,:,:)
    WRITE(IFILE) PSRCM
    WRITE(IFILE) PLENGTHM
    WRITE(IFILE) PLENGTHH
    WRITE(IFILE) MFMOIST
    WRITE(IFILE) ZBL_DEPTH
    WRITE(IFILE) ZSBL_DEPTH
    WRITE(IFILE) ZCEI
    WRITE(IFILE) PTHM
    WRITE(IFILE) ZRM(:,:,:,:)
    WRITE(IFILE) PRUS
...
CALL TURB (CST,CSTURB,TBUCONF,TURBN, YLDIMPHYEX,&
  ...
  IF(LFILEEXISTS) THEN
    ...
    WRITE(IFILE) PEDR
    WRITE(IFILE) PTPMF
    WRITE(IFILE) PDRUS_TURB
    WRITE(IFILE) PDRVS_TURB
    WRITE(IFILE) PDRTHLS_TURB
    WRITE(IFILE) PDRRTS_TURB
    WRITE(IFILE) ZDRSVS_TURB
