...
!       Boolean
LOGICAL :: LLMSE, LLMSE_PARAM, LLMSE_DIAG, LLFLAT
LOGICAL :: LLAROME
LOGICAL :: LLRAD
...
& CST=>YDPARAR%CST,                                                                                                               &
& PARAM_ICEN=>YDPARAR%PARAM_ICEN, RAIN_ICE_DESCRN=>YDPARAR%RAIN_ICE_DESCRN, RAIN_ICE_PARAMN=>YDPARAR%RAIN_ICE_PARAMN, &
& CLOUDPARN=>YDPARAR%CLOUDPARN, PARAM_MFSHALLN=>YDPARAR%PARAM_MFSHALLN, &
& TURBN=>YDPARAR%TURBN, CSTURB=>YDPARAR%CSTURB, NEBN=>YDPARAR%NEBN, TLES=>YDPARAR%TLES, &
...
IF (LELAM) THEN
  LLFLAT=.NOT.YDGEOMETRY%YREGEO%LMAP
ELSE
  LLFLAT=.TRUE.
ENDIF
...
IF (LMICRO.OR.LTURB.OR.LLMSE.OR.LKFBCONV) THEN
 ...
 ! vent horizontal et TKE
  ...
  IF (NEBN%LSUBG_COND.AND..NOT.NEBN%LSIGMAS) THEN
    IF (LKFBCONV) THEN
      DO JLEV = 1, YDCPG_OPTS%KFLEVG
...
IF (LMICRO) THEN
  ...
  IF (LMFSHAL .AND. (PARAM_MFSHALLN%CMF_CLOUD=='DIRE'.OR.PARAM_MFSHALLN%CMF_CLOUD=='BIGA')) THEN
  ...
  IF (CMICRO == 'LIMA') THEN
    ...
    & NLIMA, YDCPG_OPTS%KSTEP+1, NEBN%LSUBG_COND, NEBN%LSIGMAS, ZDT, NEBN%VSIGQSAT, ZZZ_F_, ZRHODJM__(:, 1:YDCPG_OPTS%KFLEVG), &
  ...
  ELSE
    ...
    CALL ARO_ADJUST (CST, PARAM_ICEN, RAIN_ICE_PARAMN, TURBN, NEBN, &
...
IF (LMFSHAL) THEN
  IF (PARAM_MFSHALLN%CMF_UPDRAFT=='DUAL') THEN
    ...
    IF (PARAM_ICEN%LOCND2.AND.LMICRO.AND.CMICRO/='LIMA') THEN
      DO JLEV=1,YDCPG_OPTS%KFLEVG
  ...
  DO JDRAFT=IDRAFT,3
    ...
    CALL ARO_SHALLOW_MF (CST, PARAM_MFSHALLN, NEBN, TURBN, CSTURB,                                                    &
    & KKL=IKL, KLON=YDCPG_BNDS%KFDIA, KLEV=YDCPG_OPTS%KFLEVG, KFDIA=YDCPG_BNDS%KFDIA, KRR=NRR, KRRL=NRRL,                         &
    & KRRI=NRRI, KSV=NGFL_EXT, &
    & OMIXUV=PARAM_MFSHALLN%LMIXUV, ONOMIXLG=.FALSE., KSV_LGBEG=0, KSV_LGEND=0, PTSTEP=ZDT,                                       &
    ...
    IF ((PARAM_MFSHALLN%CMF_CLOUD=='DIRE'.OR.PARAM_MFSHALLN%CMF_CLOUD=='BIGA').AND.JDRAFT==3) THEN
...
! KSV_TURB, ZSFTURB, ZTURBM, ZTURBS, ZTENDSV_TURB
  CALL ARO_TURB_MNH(CST, TURBN, CSTURB, NEBN, TLES, &
...
!!$  ENDIF
  ...
  IF (NEBN%LSUBG_COND .AND. NEBN%LSIGMAS) THEN
    IF (PARAM_MFSHALLN%CMF_CLOUD=='DIRE'.OR.PARAM_MFSHALLN%CMF_CLOUD=='BIGA'.OR.PARAM_MFSHALLN%CMF_CLOUD=='NONE') THEN
      DO JLEV = 1,YDCPG_OPTS%KFLEVG
        YDVARS%SRC%T1(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZSIGS__(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)
      ENDDO
    ELSEIF (PARAM_MFSHALLN%CMF_CLOUD=='STAT') THEN
      DO JLEV = 1,YDCPG_OPTS%KFLEVG
        DO JLON = YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
...
IF (LMICRO) THEN
  ...
  IF (CMICRO == 'LIMA') THEN
    ...
    &              )
  ELSE
    CALL ARO_RAIN_ICE (CST, PARAM_ICEN, RAIN_ICE_PARAMN, RAIN_ICE_DESCRN, CLOUDPARN, &
