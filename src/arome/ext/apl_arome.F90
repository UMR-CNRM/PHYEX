...
& TURBN=>YDPARAR%TURBN, CSTURB=>YDPARAR%CSTURB, &
...
IF (LMICRO.OR.LTURB.OR.LLMSE.OR.LKFBCONV) THEN
 ...
 ! vent horizontal et TKE
  ...
  IF (TURBN%LSUBG_COND.AND..NOT.TURBN%LSIGMAS) THEN
    IF (LKFBCONV) THEN
      DO JLEV = 1, YDCPG_OPTS%KFLEVG
...
IF (LMICRO) THEN
  ...
  IF (CMICRO == 'LIMA') THEN
    ...
    & NLIMA, YDCPG_OPTS%KSTEP+1, TURBN%LSUBG_COND, TURBN%LSIGMAS, ZDT, TURBN%VSIGQSAT, ZZZ_F_, ZRHODJM__(:, 1:YDCPG_OPTS%KFLEVG), &
  ...
  ELSE
    ...
    CALL ARO_ADJUST (CST, PARAM_ICEN, RAIN_ICE_PARAMN, TURBN, &
...
IF (LMFSHAL) THEN
  ...
  DO JDRAFT=IDRAFT,3
    ...
    CALL ARO_SHALLOW_MF (CST, PARAM_ICEN, PARAM_MFSHALLN,                                                                          &
...
! KSV_TURB, ZSFTURB, ZTURBM, ZTURBS, ZTENDSV_TURB
  ...
  & KRRL=NRRL, KRRI= NRRI, KSV=NLIMA, KTCOUNT=YDCPG_OPTS%KSTEP+1, KGRADIENTS=NGRADIENTS, &
...
!!$  ENDIF
  ...
  IF (TURBN%LSUBG_COND .AND. TURBN%LSIGMAS) THEN
...
IF (LMICRO) THEN
  ...
  IF (CMICRO == 'LIMA') THEN
    ...
    &              )
  ELSE
    CALL ARO_RAIN_ICE (CST, PARAM_ICEN, RAIN_ICE_PARAMN, RAIN_ICE_DESCRN, CLOUDPARN, TURBN, &
