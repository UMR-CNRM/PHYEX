...
! THE DDH budgets
REAL(KIND=JPRB) :: ZDP__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1),          ZTP__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZTPMF__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1),        ZDPMF__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
...
REAL(KIND=JPRB), TARGET  :: ZFLXZUMF_SUM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1), ZFLXZVMF_SUM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
...
REAL(KIND=JPRB) :: ZHLC_HRC_MF_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG), ZHLC_HCF_MF_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZHLI_HRI_MF_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG), ZHLI_HCF_MF_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZWEIGHT_MF_CLOUD_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
...
REAL(KIND=JPRB) :: ZMFTKES_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)

REAL(KIND=JPRB) :: ZDTHRAD_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)

REAL(KIND=JPRB), TARGET :: ZFLXZTHVMF_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG), ZFLXZUMF_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG), ZFLXZVMF_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB), POINTER :: ZARG_FLXZTHVMF_(:,:), ZARG_FLXZUMF_(:,:), ZARG_FLXZVMF_(:,:)
...
!!! Initialisations temporaires d'arguments non-utilises
  ...
  IF (CMICRO=='LIMA') THEN
     ...
     ELSE
        DO JLEV = 1, YDCPG_OPTS%KFLEVG 
           ZDTHRAD_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,10)
        ENDDO
     ENDIF
  ENDIF
...
IF (LMICRO) THEN
  ...
  IF (LMFSHAL .AND. (PHYEX%PARAM_MFSHALLN%CMF_CLOUD=='DIRE'.OR.PHYEX%PARAM_MFSHALLN%CMF_CLOUD=='BIGA')) THEN
    ...
    IF (YDCPG_OPTS%NSTEP==0) THEN
      DO JLEV = 1, YDCPG_OPTS%KFLEVG 
        ...
        ZWEIGHT_MF_CLOUD_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=0._JPRB
        ZHLC_HRC_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=0._JPRB
        ZHLC_HCF_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=0._JPRB
        ZHLI_HRI_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=0._JPRB
        ZHLI_HCF_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=0._JPRB
      ENDDO
    ELSE
      DO JLEV = 1, YDCPG_OPTS%KFLEVG 
        ...
        ZWEIGHT_MF_CLOUD_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,4)
        ZHLC_HRC_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,5)
        ZHLC_HCF_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,6)
        ZHLI_HRI_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,7)
        ZHLI_HCF_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,8)
      ENDDO
    ENDIF
    ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDCPG_OPTS%KFLEVG,1:8)=0._JPRB
  ELSE
    DO JLEV = 1, YDCPG_OPTS%KFLEVG
      ZRC_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=0._JPRB
      ZRI_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=0._JPRB
  ...
  ELSE
    ...
    & ZMFM_, ZRC_MF_, ZRI_MF_, ZCF_MF_, ZWEIGHT_MF_CLOUD_, &
    & ZTHS__(:, 1:YDCPG_OPTS%KFLEVG), ZRS_, ZSRCS__(:, 1:YDCPG_OPTS%KFLEVG),                  &
    & ZNEBMNH_, &
    & ZICLDFR_,ZWCLDFR_,ZSSIO_,ZSSIU_,ZIFR_,&
    & ZHLC_HRC_, ZHLC_HCF_, ZHLI_HRI_, ZHLI_HCF_,                                                                               &
    & ZHLC_HRC_MF_, ZHLC_HCF_MF_, ZHLI_HRI_MF_, ZHLI_HCF_MF_, &
 ...
 IF (LRAYFM) THEN
  ...
  IF (CMICRO=='LIMA') THEN
     DO JLEV = 1, YDCPG_OPTS%KFLEVG
        ...
        END DO
        ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,10)=ZDTHRAD_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)
     ENDDO
  ENDIF
...
IF (LMFSHAL) THEN
  ...
  DO JDRAFT=IDRAFT,3
    ...
    IF (JDRAFT == IDRAFT) THEN
      ...
      ZARG_FLXZUMF_ => ZFLXZUMF_SUM__(:,1:YDCPG_OPTS%KFLEVG)
      ZARG_FLXZVMF_ => ZFLXZVMF_SUM__(:,1:YDCPG_OPTS%KFLEVG)
    ELSE
      ! increment
      ZARG_FLXZTHVMF_ => ZFLXZTHVMF_(:,1:YDCPG_OPTS%KFLEVG)
      ZARG_FLXZUMF_ => ZFLXZUMF_(:,1:YDCPG_OPTS%KFLEVG)
      ZARG_FLXZVMF_ => ZFLXZVMF_(:,1:YDCPG_OPTS%KFLEVG)
    ENDIF
    ...
    & PSVM=ZSVM_, PDUDT_MF=ZMFUS_, PDVDT_MF=ZMFVS_, PDTKEDT_MF=ZMFTKES_, PDTHLDT_MF=ZTHLS_, PDRTDT_MF=ZRTS_, PDSVDT_MF=ZSVXXX_,     &
    & PSIGMF=ZSIGMF_, PRC_MF=ZRC_MF_, PRI_MF=ZRI_MF_, PCF_MF=ZCF_MF_, &
    & PHLC_HRC_MF=ZHLC_HRC_MF_, PHLC_HCF_MF=ZHLC_HCF_MF_, PHLI_HRI_MF=ZHLI_HRI_MF_, PHLI_HCF_MF=ZHLI_HCF_MF_, &
    & PWEIGHT_MF_CLOUD=ZWEIGHT_MF_CLOUD_, PFLXZTHVMF=ZARG_FLXZTHVMF_,                                   &
    & PFLXZUMF=ZARG_FLXZUMF_, PFLXZVMF=ZARG_FLXZVMF_, PTHL_UP=ZTHETAL_UP_,              &
    ...
    IF (JDRAFT > IDRAFT) THEN
      ...
      ZFLXZUMF_SUM__(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDCPG_OPTS%KFLEVG)=ZFLXZUMF_SUM__(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDCPG_OPTS%KFLEVG)+ZFLXZUMF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDCPG_OPTS%KFLEVG)
      ZFLXZVMF_SUM__(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDCPG_OPTS%KFLEVG)=ZFLXZVMF_SUM__(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDCPG_OPTS%KFLEVG)+ZFLXZVMF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDCPG_OPTS%KFLEVG)
    ENDIF
    ...
    IF ((PHYEX%PARAM_MFSHALLN%CMF_CLOUD=='DIRE'.OR.PHYEX%PARAM_MFSHALLN%CMF_CLOUD=='BIGA').AND.JDRAFT==3) THEN
      ...
      DO JLEV = 1,YDCPG_OPTS%KFLEVG
        ...
        ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,4)=ZWEIGHT_MF_CLOUD_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)
        ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,5)=ZHLC_HRC_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)
        ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,6)=ZHLC_HCF_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)
        ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,7)=ZHLI_HRI_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)
        ZP1EZDIAG(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV,8)=ZHLI_HCF_MF_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)
      ENDDO
    ENDIF
    DO JLEV = 1,YDCPG_OPTS%KFLEVG
      DO JLON = YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        ...
        ZTKES_(JLON,JLEV)=ZTKES_(JLON,JLEV)+ZMFTKES_(JLON,JLEV)
        ...
        !TKE tendency
        ZTENDTKE(JLON,JLEV)=ZTENDTKE(JLON,JLEV)+ZMFTKES_(JLON,JLEV)
      ENDDO
    ENDDO
...
! KSV_TURB, ZSFTURB, ZTURBM, ZTURBS, ZTENDSV_TURB
  ...
  & PSIGS=ZSIGS__, PFLXZTHVMF=ZFLXZTHVMF_SUM__, PFLXZUMF=ZFLXZUMF_SUM__, PFLXZVMF=ZFLXZVMF_SUM__,                    &
  & PLENGTHM=ZLENGTHM__, PLENGTHH=ZLENGTHH__,    &
  & MFMOIST=ZMF_UP__, PDRUS_TURB=ZTENDU_TURB__, PDRVS_TURB=ZTENDV_TURB__, PDRTHLS_TURB=ZTENDTHL_TURB__,              &
  & PDRRTS_TURB=ZTENDRT_TURB__, PDRSVS_TURB=ZTENDSV_TURBLIMA_, PDP=ZDP__, PTP=ZTP__, &
  & PDPMF=ZDPMF__, PTPMF=ZTPMF__, PTDIFF=ZTDIFF__, &
...
!!$  ENDIF
  ...
  IF (LFLEXDIA) THEN
    DO JLEV = 1,YDCPG_OPTS%KFLEVG
      DO JLON = YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        ZDP__(JLON,JLEV)=(ZDP__(JLON,JLEV)-ZDPMF__(JLON,JLEV))*YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTP__(JLON,JLEV)=(ZTP__(JLON,JLEV)-ZTPMF__(JLON,JLEV))*YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZDPMF__(JLON,JLEV)=ZDPMF__(JLON,JLEV)*YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
    ...
    ENDDO
      ...
      & YDDDH            )
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH, ZDPMF__(:, 1:YDCPG_OPTS%KFLEVG), 'TKEPRDYMF', &
      & YDDDH)
  ...
  DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
    ...
    IF(GFL_WKA(JLON,YDCPG_BNDS%KBL)>0.0)THEN
      ...
      CALL ARO_WINDFARM(YDGEOMETRY,YDMODEL,YDVARS%GEOMETRY%OROG%T0(JLON), &
          ...
          & YDCPG_OPTS%NSTEP,JGFL,ZP1EZDIAG(JLON,1,10))
      DO JLEV=1,YDCPG_OPTS%KFLEVG
        YDMF_PHYS%OUT%TENDU(JLON,JLEV)=YDMF_PHYS%OUT%TENDU(JLON,JLEV)+ZTENDU_TURB__(JLON,JLEV)
        YDMF_PHYS%OUT%TENDV(JLON,JLEV)=YDMF_PHYS%OUT%TENDV(JLON,JLEV)+ZTENDV_TURB__(JLON,JLEV)
...
IF (LMICRO) THEN
  ...
  IF (LFPREC3D) THEN
    DO JR=2,NRR ! no precip for qv
      DO JLEV=1,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          ZP1EZDIAG(JLON,JLEV,9)=ZP1EZDIAG(JLON,JLEV,9)+ZPFPR_(JLON,JLEV,JR)*1000._JPRB*YDCPG_OPTS%ZDTPHY
        ENDDO
      ENDDO
    ENDDO
  ENDIF
