...
IF (KSPLT==1 .AND. TLES%LLES_CALL) THEN
  ...
  CALL LES_MEAN_SUBGRID_PHY(D, TLES,MXF(ZFLX), TLES%X_LES_SUBGRID_UThl ) 
...
!$acc update self(ZFLX)
  ...
  IF (KSPLT==1 .AND. TLES%LLES_CALL) THEN
    CALL SECOND_MNH(ZTIME1)
    CALL LES_MEAN_SUBGRID_PHY(D, TLES,MXF(ZFLX), TLES%X_LES_SUBGRID_URt ) 
    ZWKLES = MZF(MXF(GX_W_UW(OFLAT, PWM,PDXX,PDZZ,PDZX)*MZM(ZFLX)))
...
!
  ...
  IF (KRRL>0 .AND. KSPLT==1 .AND. TLES%LLES_CALL) THEN
    CALL SECOND_MNH(ZTIME1)
    CALL LES_MEAN_SUBGRID_PHY(D, TLES,MXF(ZFLXC), TLES%X_LES_SUBGRID_URc )
    CALL SECOND_MNH(ZTIME2)
    TLES%XTIME_LES = TLES%XTIME_LES + ZTIME2 - ZTIME1
  END IF
!
...
IF (KSPLT==1 .AND. TLES%LLES_CALL) THEN
  CALL SECOND_MNH(ZTIME1)
  CALL LES_MEAN_SUBGRID_PHY(D, TLES,MYF(ZFLX), TLES%X_LES_SUBGRID_VThl ) 
...
!$acc update self(ZFLX)
  ...
  IF (KSPLT==1 .AND. TLES%LLES_CALL) THEN
    CALL SECOND_MNH(ZTIME1)
    CALL LES_MEAN_SUBGRID_PHY(D, TLES,MYF(ZFLX), TLES%X_LES_SUBGRID_VRt ) 
    ZWKLES = MZF(MYF(GY_W_VW(OFLAT, PWM,PDYY,PDZZ,PDZY)*MZM(ZFLX)))
    CALL LES_MEAN_SUBGRID_PHY(D, TLES,ZWKLES, TLES%X_LES_RES_ddxa_W_SBG_UaRt , .TRUE. )
    ZWKLES = GY_M_M(OFLAT,PTHLM,PDYY,PDZZ,PDZY)*MYF(ZFLX)
    CALL LES_MEAN_SUBGRID_PHY(D, TLES,ZWKLES, TLES%X_LES_RES_ddxa_Thl_SBG_UaRt , .TRUE. )
...
!
  ...
  IF (KRRL>0 .AND. KSPLT==1 .AND. TLES%LLES_CALL) THEN
    CALL SECOND_MNH(ZTIME1)
    CALL LES_MEAN_SUBGRID_PHY(D, TLES,MYF(ZFLXC), TLES%X_LES_SUBGRID_VRc )
    CALL SECOND_MNH(ZTIME2)
    TLES%XTIME_LES = TLES%XTIME_LES + ZTIME2 - ZTIME1
  END IF
  !
