...
USE MODI_GRADIENT_M, ONLY: GX_M_U, GY_M_V
...
IF (TLES%LLES_CALL .AND. KSPLT==1) THEN
  ...
  CALL LES_MEAN_SUBGRID_PHY(D, TLES, MXF(ZFLX), TLES%X_LES_SUBGRID_UTke ) 
...
!
  IF (TLES%LLES_CALL .AND. KSPLT==1) THEN
    CALL SECOND_MNH(ZTIME1)
    CALL LES_MEAN_SUBGRID_PHY(D, TLES, MYF(ZFLX), TLES%X_LES_SUBGRID_VTke )
    CALL SECOND_MNH(ZTIME2)
    TLES%XTIME_LES = TLES%XTIME_LES + ZTIME2 - ZTIME1
  END IF
!
