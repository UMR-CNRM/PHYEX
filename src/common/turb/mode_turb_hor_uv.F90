...
!
USE MODI_GRADIENT_U, ONLY: GY_U_UV
USE MODI_GRADIENT_V, ONLY: GX_V_UV
USE MODI_SHUMAN, ONLY: DXF, DXM, DYF, DYM, DZF, MXF, MXM, MYF, MYM, MZM 
...
IF (TLES%LLES_CALL .AND. KSPLT==1) THEN
  ...
  CALL LES_MEAN_SUBGRID_PHY(D, TLES, MXF(MYF(ZFLX)), TLES%X_LES_SUBGRID_UV ) 
  CALL LES_MEAN_SUBGRID_PHY(D, TLES, MXF(MYF(GY_U_UV(PUM,PDYY,PDZZ,PDZY)*ZFLX)), TLES%X_LES_RES_ddxa_U_SBG_UaU , .TRUE.)
  CALL LES_MEAN_SUBGRID_PHY(D, TLES, MXF(MYF(GX_V_UV(PVM,PDXX,PDZZ,PDZX)*ZFLX)), TLES%X_LES_RES_ddxa_V_SBG_UaV , .TRUE.)
