...
!$acc end kernels
  ...
  IF (TLES%LLES_CALL) THEN
    CALL SECOND_MNH(ZTIME1)
    ZWKLES = MZF(MXF(GX_W_UW(OFLAT, PWM,PDXX,PDZZ,PDZX)*ZFLXZ))
    CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_W_SBG_UaW )
    ZWKLES = MXF(GX_M_U(OFLAT, PTHLM,PDXX,PDZZ,PDZX)* MZF(ZFLXZ))
    CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Thl_SBG_UaW )
    IF (KRR>=1) THEN
      ZWKLES = MXF(GX_U_M(OFLAT, PRM(:,:,1),PDXX,PDZZ,PDZX)*MZF(ZFLXZ))
      CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Rt_SBG_UaW )
    END IF
    DO JSV=1,KSV
      ZWKLES = MXF(GX_U_M(OFLAT, PSVM(:,:,JSV),PDXX,PDZZ,PDZX)*MZF(ZFLXZ))
...
!$acc end kernels
  ...
  IF (TLES%LLES_CALL) THEN
    CALL SECOND_MNH(ZTIME1)
    ZWKLES = MZF(MYF(GY_W_VW(OFLAT, PWM,PDYY,PDZZ,PDZY)*ZFLXZ(:,:)))
    CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_W_SBG_UaW , .TRUE. )
    ZWKLES = MYF(GY_M_V(OFLAT, PTHLM,PDYY,PDZZ,PDZY)*MZF(ZFLXZ))
    CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Thl_SBG_UaW , .TRUE. )
    IF (KRR>=1) THEN
      ZWKLES = MYF(GY_V_M(OFLAT, PRM(:,:,1),PDYY,PDZZ,PDZY)*MZF(ZFLXZ(:,:)))
