...
!$acc end kernels
    ...
    !
    CALL ETHETA(D, CST, KRR, KRRI, PTHLT, PRT, PLOCPEXNM, PATHETA, PSRCT, GOCEAN, OCOMPUTE_SRC, ZETHETA)
    CALL EMOIST(D, CST, KRR, KRRI, PTHLT, PRT, PLOCPEXNM, PAMOIST, PSRCT, GOCEAN, ZEMOIST)
    !
    IF (KRR > 0) THEN
!$acc kernels
