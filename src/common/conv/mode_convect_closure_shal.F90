...
REAL, DIMENSION(D%NIJT),  INTENT(INOUT) :: PTIMEC ! convection time step
REAL, DIMENSION(D%NIJT,D%NKT),INTENT(IN) :: PTHL   ! grid scale enthalpy (J/kg)
REAL, DIMENSION(D%NIJT,D%NKT),INTENT(IN) :: PTH    ! grid scale theta
REAL, DIMENSION(D%NIJT,D%NKT),INTENT(IN) :: PRW    ! grid scale total water
                                                ! mixing ratio
REAL, DIMENSION(D%NIJT,D%NKT),INTENT(IN) :: PRC    ! grid scale r_c
REAL, DIMENSION(D%NIJT,D%NKT),INTENT(IN) :: PRI    ! grid scale r_i
LOGICAL, DIMENSION(D%NIJT),  INTENT(IN) :: OTRIG1 ! logical to keep trace of
                                                ! convective arrays modified in UPDRAFT
!
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(IN) :: PPRES  ! pressure (P)
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(IN) :: PDPRES ! pressure difference between
                                                 ! bottom and top of layer (Pa)
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(IN) :: PLMASS ! mass of model layer (kg)
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(IN) :: PZ     ! height of model layer (m)
REAL, DIMENSION(D%NIJT),     INTENT(IN)  :: PCAPE  ! available potent. energy
...
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(IN)  :: PUTHL  ! updraft enthalpy (J/kg)
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(IN)  :: PURW   ! updraft total water (kg/kg)
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(IN)  :: PURC   ! updraft cloud water (kg/kg)
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(IN)  :: PURI   ! updraft cloud ice   (kg/kg)
!
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(OUT)  :: PTHC  ! conv. adj. grid scale theta
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(OUT)  :: PRWC  ! conv. adj. grid scale r_w
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(OUT)  :: PRCC  ! conv. adj. grid scale r_c
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(OUT)  :: PRIC  ! conv. adj. grid scale r_i
REAL, DIMENSION(D%NIJT,D%NKT), INTENT(OUT)  :: PWSUB ! envir. compensating subsidence(Pa/s)
!
...
REAL    :: ZEPS  
!
REAL, DIMENSION(D%NIJT,D%NKT) :: ZTHLC       ! convectively adjusted
                                          ! grid scale enthalpy
REAL, DIMENSION(D%NIJT,D%NKT) :: ZOMG        ! conv. environm. subsidence (Pa/s)
REAL, DIMENSION(D%NIJT,D%NKT) :: ZUMF        ! non-adjusted updraft mass flux
REAL, DIMENSION(D%NIJT,D%NKT) :: ZUER        !   "     updraft  entrainm. rate
REAL, DIMENSION(D%NIJT,D%NKT) :: ZUDR        !   "     updraft  detrainm. rate
REAL, DIMENSION(D%NIJT)     :: ZADJ         ! mass adjustment factor
REAL, DIMENSION(D%NIJT)     :: ZADJMAX      ! limit value for ZADJ
REAL, DIMENSION(D%NIJT)     :: ZCAPE        ! new CAPE after adjustment
REAL, DIMENSION(D%NIJT)     :: ZTIMEC       ! fractional convective time step
REAL, DIMENSION(D%NIJT,D%NKT):: ZTIMC        ! 2D work array for ZTIMEC
!
REAL, DIMENSION(D%NIJT)     :: ZTHLCL       ! new  theta at LCL
REAL, DIMENSION(D%NIJT)     :: ZRVLCL       ! new  r_v at LCL
REAL, DIMENSION(D%NIJT)     :: ZZLCL        ! height of LCL
REAL, DIMENSION(D%NIJT)     :: ZTLCL        ! temperature at LCL
REAL, DIMENSION(D%NIJT)     :: ZTELCL       ! envir. temper. at LCL
REAL, DIMENSION(D%NIJT)     :: ZTHEUL       ! theta_e for undilute ascent
REAL, DIMENSION(D%NIJT)     :: ZTHES1, ZTHES2! saturation environm. theta_e
REAL, DIMENSION(D%NIJT,D%NKT) :: ZTHMFIN, ZTHMFOUT, ZRWMFIN, ZRWMFOUT
REAL, DIMENSION(D%NIJT,D%NKT) :: ZRCMFIN, ZRCMFOUT, ZRIMFIN, ZRIMFOUT
                                    ! work arrays for environm. compensat. mass flux
REAL, DIMENSION(D%NIJT)     :: ZPI          ! (P/P00)**R_d/C_pd
REAL, DIMENSION(D%NIJT)     :: ZLV          ! latent heat of vaporisation
REAL, DIMENSION(D%NIJT)     :: ZLS          ! latent heat of sublimation
REAL, DIMENSION(D%NIJT)     :: ZCPH         ! specific heat C_ph
INTEGER, DIMENSION(D%NIJT)  :: ITSTEP       ! fractional convective time step
INTEGER, DIMENSION(D%NIJT)  :: ICOUNT       ! timestep counter
INTEGER, DIMENSION(D%NIJT)  :: ILCL         ! index lifting condens. level
REAL, DIMENSION(D%NIJT)     :: ZWORK1, ZWORK2, ZWORK3, ZWORK5
LOGICAL, DIMENSION(D%NIJT)  :: GWORK1, GWORK3! work arrays
LOGICAL, DIMENSION(D%NIJT,D%NKT) :: GWORK4    ! work array
!
...
INTEGER           :: JKPP
REAL, DIMENSION(D%NIJT,D%NKT) :: ZPIAUX 
!
!-------------------------------------------------------------------------------
!
...
DO JK = MAX(IKB,JDPLMIN+1) , MIN(IKE,JLCLMAX)
  DO JI = D%NIJB, D%NIJE
    IF( JK > KDPL(JI)) THEN
      IF(JK <= ILCL(JI) ) THEN
...
DO JITER = 1, 4  ! Enter adjustment loop to assure that all CAPE is
  ...
  ZTIMEC(:) = PTIMEC(:)
  DO JK=1, IKS
    GWORK4(D%NIJB:D%NIJE,JK) = GWORK1(D%NIJB:D%NIJE)
  ENDDO
  DO JK = IKB, IKE
    DO JI=D%NIJB, D%NIJE
  ...
  DO JSTEP = 1, KFTSTEPS ! Enter the fractional time step loop here
    ...
    IF (JKMAX-IKB <= 4) THEN
      ...
      DO JK = IKB + 1, JKMAX
        ...
        ENDDO
        JKP = MAX( IKB + 1, JK - 1 )
        DO JI = D%NIJB, D%NIJE !original loop 1
          IF ( GWORK3(JI) ) THEN
          ...
          ENDIF
        ENDDO
        DO JI = D%NIJB, D%NIJE  !original loop 2
          IF ( GWORK3(JI) ) THEN
    ...
    ELSE
      ...
      IF (JKMAX .LE. JCTLMAX) THEN
        JK= JKMAX
        JKP = MAX( IKB + 1, JK - 1 )
        DO JI = D%NIJB, D%NIJE !if JK=JKMAX, the column JK of the arrays is only modified by 
      ...
      ENDIF
      JK = IKB+1
      DO JI = D%NIJB, D%NIJE !if JK=IKB+1, the column JK of the arrays is modified by 
      ...
      DO JI = D%NIJB, D%NIJE !if JK=IBK+2, the column JKB+1 of the arrays is modified 
      ...
      DO JK = IKB + 3, MIN(JKMAX-1,JCTLMAX+1) 
        ...
        DO JI = D%NIJB, D%NIJE !if JK is greater than IKB+3, the column JK of the arrays is modified 
