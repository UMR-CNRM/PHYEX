...
IF ( BUCONF%LBU_ENABLE ) THEN
  ...
  IF ( BUCONF%LBUDGET_SV ) THEN
    ...
    IF ( LIMAP%LSEDI .AND. LIMAP%NMOM_I.GE.2) THEN
      ...
      ELSE
        DO ISH = 1, LIMAP%NNB_CRYSTAL_SHAPE
          CALL BUDGET_STORE_INIT_PHY(D, TBUDGETS(NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_NI + ISH - 1), 'SEDI', &
...
IF ( BUCONF%LBU_ENABLE ) then
  ...
  IF ( BUCONF%LBUDGET_SV ) then
    ...
    IF ( LIMAP%LSEDI .AND. LIMAP%NMOM_I.GE.2 ) THEN
      ...
      ELSE
        DO ISH = 1, LIMAP%NNB_CRYSTAL_SHAPE
          CALL BUDGET_STORE_END_PHY(D, TBUDGETS(NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_NI + ISH - 1), 'SEDI', &
                                    ZCIS_SHAPE( :, :, ISH) * PRHODJ( :, :) )
        END DO
      END IF
    END IF
            ...
            IF (LIMAP%LCRYSTAL_SHAPE) THEN
              DO ISH = 1, LIMAP%NNB_CRYSTAL_SHAPE
                ...
                ZTOT_SHCI_CORR2(I1(II),I3(II),ISH)  = ZTOT_SHCI_CORR2(I1(II),I3(II),ISH) + &
                                                             Z_SHCI_CORR2(II,ISH)
              END DO
              ZTOT_SHRI_ISCS(I1(II),I2(II)) = ZTOT_SHRI_ISCS(I1(II),I2(II)) + &    
...
!
IF ( LIMAP%NMOM_C.EQ.1 ) PCIT(:,:) = ZCIT(:,:)*PRHODREF(:,:)
!
!
! Call budgets
!
