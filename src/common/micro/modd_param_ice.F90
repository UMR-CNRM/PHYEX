...
!! the strucuture. A value can be accessed through the structure PARAM_ICE
!! or directly through these pointers.
  IMPLICIT NONE

  IF(.NOT. ASSOCIATED(LWARM)) THEN
    ...
    LCRIAUTI => PARAM_ICE%LCRIAUTI
    LOCND2 => PARAM_ICE%LOCND2
    !
    ...
    XCRIAUTI_NAM => PARAM_ICE%XCRIAUTI_NAM
    XT0CRIAUTI_NAM => PARAM_ICE%XT0CRIAUTI_NAM
    XBCRIAUTI_NAM => PARAM_ICE%XBCRIAUTI_NAM
    XACRIAUTI_NAM => PARAM_ICE%XACRIAUTI_NAM
    XCRIAUTC_NAM => PARAM_ICE%XCRIAUTC_NAM
    XRDEPSRED_NAM => PARAM_ICE%XRDEPSRED_NAM
    XRDEPGRED_NAM => PARAM_ICE%XRDEPGRED_NAM
    XFRMIN_NAM => PARAM_ICE%XFRMIN_NAM
    !
    NMAXITER_MICRO => PARAM_ICE%NMAXITER_MICRO
    NPROMICRO => PARAM_ICE%NPROMICRO
    !
...
!!*** *PARAM_ICE_INIT* - Code needed to initialize the MODD_PARAM_ICE module
!!
!!*   PURPOSE
!!    -------
...
!
IF(LLREADNAM) THEN
  CALL POSNAM_PHY(KUNITNML, 'NAM_PARAM_ICE', LDNEEDNAM, LLFOUND, KLUOUT)
  IF(LLFOUND) READ(UNIT=KUNITNML, NML=NAM_PARAM_ICE)
ENDIF
!
!*      3. CHECKS
!       ---------
...
IF(LLCHECK) THEN
  ...
  IF(HPROGRAM=='AROME') THEN
    IF(.NOT. (LADJ_BEFORE .AND. .NOT. LADJ_AFTER)) THEN
      CALL PRINT_MSG(NVERB_FATAL, 'GEN', 'MODD_PARAM_ICE', 'With AROME, LADJ_BEFORE must be .T. and LADJ_AFTER must be .F.')
    ENDIF
  ELSEIF(HPROGRAM=='MESONH') THEN
    IF(.NOT. LPACK_MICRO) THEN
      CALL PRINT_MSG(NVERB_FATAL, 'GEN', 'MODD_PARAM_ICE', 'With MESONH, LPACK_MICRO must be .T.')
    ENDIF
  ENDIF
ENDIF
!
