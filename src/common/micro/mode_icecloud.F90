...
DO JIJ = IIJB, IIJE
   ...
   ZRHW = ZA/ESATW(ICEP%TIWMX, PT(JIJ))  
   RSI(JIJ) = QSATMX_TAB(CST,ICEP%TIWMX,PP(JIJ),PT(JIJ),1.)
   ZRHI = ZA/ESATI(ICEP%TIWMX, PT(JIJ))
   ZI2W =  ESATW(ICEP%TIWMX, PT(JIJ))/ESATI(ICEP%TIWMX, PT(JIJ))

   SSIU(JIJ) = MIN(ZI2W,ZRHI)
   SSIO(JIJ) = SSIU(JIJ)
   W2D(JIJ) = 1.
   IF (PT(JIJ)>273.1 .OR. ZR<=0. .OR. ESATI(ICEP%TIWMX, PT(JIJ)) >= PP(JIJ)*0.5) THEN
      SSIU(JIJ) = SSIU(JIJ) - 1.
      SSIO(JIJ) = SSIU(JIJ)
