...
!
    ...
    IF (LTIW) ZTIW(JL)=TIWMX_TAB(CST, ICEP, ZPRES(JL),ZZT(JL), ZRVS(JL)*PTSTEP,0._JPRB,ZRSP,ZRSW,0.1_JPRB)
    ...
    IF (OCND2) THEN
       ...
       ZAA2(JL) = AA2(ICEP, ZZT(JL))
       ZBB3(JL) = BB3(ICEP, ZZT(JL))
       ZAA2W(JL)= AA2W(ICEP, ZZT(JL))
       ZBB3W(JL)= BB3W(ICEP, ZZT(JL))
...
! If so, adjust total ice and then moisture  and temperature.
    ...
    DO JL=1,KSIZE
      ...
      &  ZESI(JL) < ZPRES(JL)*0.5 )THEN
        ZTSP = TIWMX_TAB(CST, ICEP, ZPRES(JL),ZZT(JL), ZRVS(JL)*PTSTEP,1._JPRB,ZRSP,ZRSI,0.1_JPRB)
        ZRVSOLD =ZRVS(JL)
        ZRISOLD =ZRIS(JL)
        ZRSSOLD =ZRSS(JL)
        ZRGSOLD =ZRGS(JL)
