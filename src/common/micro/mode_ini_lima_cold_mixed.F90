...
INTEGER :: JSH, JSH2          ! Loop index for the number of crystal shapes
!
REAL, DIMENSION(8)  :: ZGAMI  ! parameters involving various moments
REAL, DIMENSION(2)  :: ZGAMS  ! of the generalized gamma law
REAL, DIMENSION(2,4) :: ZGAMI_SHAPE  ! replace ZGAMI(6) and ZGAMI(7)
!
...
REAL :: ZESR, ZESS, ZEII      ! Mean efficiency of rain-aggregate collection, aggregate-aggregate collection, (I+I) 
...
!
  ...
  SELECT CASE (HTYPE_CRYSTAL_SHAPE(JSH))
    CASE('CPLA')
      XAI_SHAPE(JSH)  = 0.82    ! relation masse maxdim
      XBI_SHAPE(JSH)  = 2.5     ! relation masse maxdim
      XC_I_SHAPE(JSH) = 747.0   ! vitesse de chute maxdim
      XDI_SHAPE(JSH)  = 1.0     ! vitesse de chute maxdim
      XC1I_SHAPE(JSH) = 0.339
      XGAMMAI_SHAPE(JSH) = 0.106 ! value from Yang et al. (2013)
      XDELTAI_SHAPE(JSH) = 1.84  ! value from Yang et al. (2013)
    CASE('CCOL')
      XAI_SHAPE(JSH)  = 110.798 ! Mitchell 1(d < 100 um) 1996
      XBI_SHAPE(JSH)  = 2.91    ! Mitchell 1(d < 100 um) 1996
      XC_I_SHAPE(JSH) = 1.96E5
      XDI_SHAPE(JSH)  = 1.585
      XC1I_SHAPE(JSH) = 0.24
      XGAMMAI_SHAPE(JSH) = 0.154 ! value from Yang et al. (2013)
      XDELTAI_SHAPE(JSH) = 1.84  ! value from Yang et al. (2013)
    CASE('POIR')
      XAI_SHAPE(JSH)  = 10.0    ! moyenne entre plates et colonnes (jpp)
      XBI_SHAPE(JSH)  = 2.7     ! relation masse diamètre des columns
      XC_I_SHAPE(JSH) = 7000.0
      XDI_SHAPE(JSH)  = 1.235
      XC1I_SHAPE(JSH) = 0.566
      XGAMMAI_SHAPE(JSH) = 0.057 ! value from Yang et al. (2013)
      XDELTAI_SHAPE(JSH) = 1.8  ! value from Yang et al. (2013)
    CASE('YPLA')
      XAI_SHAPE(JSH)  = 0.744      ! Plates_from Yang et al (2013)
      XBI_SHAPE(JSH)  = 2.47 
      XC_I_SHAPE(JSH) = 340.14    
      XDI_SHAPE(JSH)  = 0.87   
      XGAMMAI_SHAPE(JSH) = 0.23
      XDELTAI_SHAPE(JSH) = 1.94    
      XC1I_SHAPE(JSH) = 1./XPI   ! values from LIMA
    CASE('YCOL')
      XAI_SHAPE(JSH)  = 0.03   ! Columns_from Yang et al (2013)
      XBI_SHAPE(JSH)  = 2.0  
      XC_I_SHAPE(JSH) = 401.65
      XDI_SHAPE(JSH)  = 0.83
      XGAMMAI_SHAPE(JSH) = 0.004
      XDELTAI_SHAPE(JSH) = 1.45    
      XC1I_SHAPE(JSH) = 0.8      ! values from LIMA
    CASE('YBUR')
      XAI_SHAPE(JSH)  = 0.29      ! Bullet rosettes_from Yang et al (2013)
      XBI_SHAPE(JSH)  = 2.41 
      XC_I_SHAPE(JSH) = 677.43
      XDI_SHAPE(JSH)  = 0.96
      XGAMMAI_SHAPE(JSH) = 0.03
      XDELTAI_SHAPE(JSH) = 1.72   
      XC1I_SHAPE(JSH) = 0.5      !  values from LIMA      
    CASE('YDRO')
      XAI_SHAPE(JSH)  = 346.906      ! Droxtals_from Yang et al (2013)
      XBI_SHAPE(JSH)  = 3      
      XC_I_SHAPE(JSH) = 695.83 
      XDI_SHAPE(JSH)  = 0.61    
      XGAMMAI_SHAPE(JSH) = 0.673
      XDELTAI_SHAPE(JSH) = 2.0    
      XC1I_SHAPE(JSH) = 0.5      !  values from LIMA
    CASE('YHCO')
      XAI_SHAPE(JSH)  = 5.6   ! Hollow_Columns_from Yang et al (2013)
      XBI_SHAPE(JSH)  = 2.6   
      XC_I_SHAPE(JSH) = 97.7 
      XDI_SHAPE(JSH)  = 0.63  
      XGAMMAI_SHAPE(JSH) = 0.154
      XDELTAI_SHAPE(JSH) = 1.84    
      XC1I_SHAPE(JSH) = 0.8      ! values from LIMA
    CASE('YHBU')
      XAI_SHAPE(JSH)  = 1.033      ! Hollow_Bullet rosettes_from Yang et al (2013)
      XBI_SHAPE(JSH)  = 2.59  
      XC_I_SHAPE(JSH) = 112.5  
      XDI_SHAPE(JSH)  = 0.72   
      XGAMMAI_SHAPE(JSH) = 0.057
      XDELTAI_SHAPE(JSH) = 1.8    
      XC1I_SHAPE(JSH) = 0.5      ! values from LIMA            
  END SELECT
  ENDDO
!
...
IF (LCRYSTAL_SHAPE) THEN
  ...
  DO JSH = 1, NNB_CRYSTAL_SHAPE
    ...
    X2DEPI_SHAPE(JSH) = (4.0 * XPI) * XC1I_SHAPE(JSH) * XF2I * XC_I_SHAPE(JSH) * &
                        MOMG(XALPHAI,XNUI,XDI_SHAPE(JSH)+2.0)
  END DO
END IF
!
...
IF(LCRYSTAL_SHAPE) THEN
    ...
    DO JSH = 1, NNB_CRYSTAL_SHAPE
        XFISCS(JSH) = XAI_SHAPE(3)*(XPI/4.0)*(ZRHO00**XCEXVT)/(MOMG(XALPHAI,XNUI,XBI_SHAPE(JSH)))

        XLBISCS1(JSH) =    MOMG(XALPHAI,XNUI,2.)*MOMG(XALPHAI,XNUI,XBI_SHAPE(JSH))
        XLBISCS2(JSH) = 2.*MOMG(XALPHAI,XNUI,1.)*MOMG(XALPHAI,XNUI,XBI_SHAPE(JSH)+1.)
        XLBISCS3(JSH) =                          MOMG(XALPHAI,XNUI,XBI_SHAPE(JSH)+2.)   

        CALL RZCOLX  ( IND, XALPHAI, XNUI, XALPHAI, XNUI,                                              &
               ZEII, XBI_SHAPE(3), XC_I_SHAPE(JSH), XDI_SHAPE(JSH), 0., XC_I_SHAPE(3), XDI_SHAPE(3), 0., &
               XSCLBDAI_MAX, XSCLBDAI_MAX, XSCLBDAI_MIN, XSCLBDAI_MIN,                                 &
               ZFDINFTY, XKER_R_ISCI_SHAPE(JSH,:,:)                                                    )
        DO JSH2 = 1, NNB_CRYSTAL_SHAPE
            CALL NZCOLX  ( IND, XALPHAI, XNUI, XALPHAI, XNUI,                                  &
               ZEII, XC_I_SHAPE(JSH), XDI_SHAPE(JSH), 0., XC_I_SHAPE(JSH2), XDI_SHAPE(JSH2), 0., &
               XSCLBDAI_MAX, XSCLBDAI_MAX, XSCLBDAI_MIN, XSCLBDAI_MIN,                         &
               ZFDINFTY, XKER_N_ISCI_SHAPE(JSH,JSH2,:,:)                                       )
        ENDDO
    ENDDO
ENDIF
!
