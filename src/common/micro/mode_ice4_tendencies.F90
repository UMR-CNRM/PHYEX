...
USE MODE_TIWMX,      ONLY: ESATI, AA2, BB3, REDIN, AM3
!
...
!$acc kernels
    !$mnh_expand_where(JIJ=D%NIJB:D%NIJE)
     IF (PARAMI%LSNOW_T) THEN 
        ZLBDAS(D%NIJB:D%NIJE)=0.
        WHERE (ZVART(D%NIJB:D%NIJE,IRS)>0. .AND. ZT(D%NIJB:D%NIJE)>263.15)
           ZLBDAS(D%NIJB:D%NIJE) = MAX(MIN(ICED%XLBDAS_MAX, 10**(14.554-0.0423*ZT(D%NIJB:D%NIJE))),ICED%XLBDAS_MIN)*ICED%XTRANS_MP_GAMMAS
        END WHERE
        WHERE (ZVART(D%NIJB:D%NIJE,IRS)>0. .AND. ZT(D%NIJB:D%NIJE)<=263.15)
           ZLBDAS(D%NIJB:D%NIJE) = MAX(MIN(ICED%XLBDAS_MAX, 10**(6.226-0.0106*ZT(D%NIJB:D%NIJE))),ICED%XLBDAS_MIN)*ICED%XTRANS_MP_GAMMAS
        END WHERE
     ELSE
      WHERE(ZVART(D%NIJB:D%NIJE,IRS)>0.)
        ZLBDAS(D%NIJB:D%NIJE)  = MIN(ICED%XLBDAS_MAX, ICED%XLBS*(PRHODREF(D%NIJB:D%NIJE)*MAX(ZVART(D%NIJB:D%NIJE,IRS), ICED%XRTMIN(5)))**ICED%XLBEXS)
      ELSEWHERE
        ZLBDAS(D%NIJB:D%NIJE)=0.
      END WHERE
     END IF
