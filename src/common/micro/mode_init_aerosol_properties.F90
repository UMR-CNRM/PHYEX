...
!!!!!!!!!!!!!!!!
!
IF (LHOOK) CALL DR_HOOK('INIT_AEROSOL_PROPERTIES', 0, ZHOOK_HANDLE)
...
! sea-salt, sulfate, hydrophilic (GADS data)
   ...
   ENDSELECT
!
IF (PHYAERO%LORILAM) THEN   ! for sulphates and hydrophilic aerosols
  IF (.NOT.(ASSOCIATED(PHYAERO%XRHOI))) ALLOCATE(PHYAERO%XRHOI(PHYAERO%NSP+PHYAERO%NSOA+PHYAERO%NCARB))
  PHYAERO%XRHOI(:)          = 1.8e3
  PHYAERO%XRHOI(PHYAERO%JP_AER_H2O) = 1.0e3   ! water
  PHYAERO%XRHOI(PHYAERO%JP_AER_DST) = PHYAERO%XDENSITY_DUST   ! water

  ! assumption: we choose to put sulfates and hydrophilics compounds in mode J (accumulation)
  IF (PHYAERO%CRGUNIT=="MASS") THEN
  RCCN(2)   = PHYAERO%XINIRADIUSJ * EXP(-3.*(LOG(PHYAERO%XINISIGJ))**2) * 1E-6 ! Sulfates
  RCCN(3)   = PHYAERO%XINIRADIUSJ * EXP(-3.*(LOG(PHYAERO%XINISIGJ))**2) * 1E-6 ! Hydrophilic
  ELSE
  RCCN(2)   = PHYAERO%XINIRADIUSJ * 1E-6  ! Sulfates
  RCCN(3)   = PHYAERO%XINIRADIUSJ * 1E-6  ! Hydrophilic

  END IF
  LOGSIGCCN(2) = LOG(PHYAERO%XINISIGJ)
  LOGSIGCCN(3) = LOG(PHYAERO%XINISIGJ)
  RHOCCN(2)    = PHYAERO%XRHOI(PHYAERO%JP_AER_SO4) 
  RHOCCN(3)    = PHYAERO%XRHOI(PHYAERO%JP_AER_OC)
END IF
IF (PHYAERO%LSALT) THEN ! for sea salts
  JMOD = 1
  IF (PHYAERO%NMODE_SLT >= 5) JMOD = 4  ! choose mode 4 of Ovadnevaite 2014 (r = 0.115 µm, sigma = 1.53)
  IF (PHYAERO%NMODE_SLT == 3) JMOD = 1  ! choose mode 1 of Vig01 (r = 0.2 µm, sigma = 1.9) or Sch04 (r = 0.14 µm, sigma = 1.59)
  IF (PHYAERO%CRGUNITS=="MASS") THEN
  RCCN(1)   = PHYAERO%XINIRADIUS_SLT(JMOD) * EXP(-3.*(LOG(PHYAERO%XINISIG_SLT(JMOD)))**2) * 1E-6
  ELSE
  RCCN(1)   = PHYAERO%XINIRADIUS_SLT(JMOD) * 1E-6
  END IF
  LOGSIGCCN(1) = LOG(PHYAERO%XINISIG_SLT(JMOD))
  RHOCCN(1) = PHYAERO%XDENSITY_SALT
END IF
...
!
  ...
  IF (.NOT.(ASSOCIATED(XGMULTI)))  CALL PARAM_LIMA_ALLOCATE('XGMULTI', NMOD_CCN)
!
...
! no units relative to smax 
      ...
      XGMULTI(JMOD)=GAMMA_X0D( XKHEN_MULTI(JMOD)/2.0) * GAMMA_X0D(3.0/2.0) &
...
!!$
       ...
       XGMULTI(JMOD)=GAMMA_X0D(X2/2.0)*GAMMA_X0D(3.0/2.0)/GAMMA_X0D((X2+3.0)/2.0)
    ENDDO
!
...
IF (PHYAERO%LORILAM) THEN
  IDX=1
  IF ((IDX+1) .LE. NMOD_IFN) THEN
  IF (PHYAERO%CRGUNIT=="MASS") THEN
   XMDIAM_IFN(IDX+1)   = 2 * PHYAERO%XINIRADIUSJ * EXP(-3.*(LOG(PHYAERO%XINISIGJ))**2) * 1E-6
  ELSE
   XMDIAM_IFN(IDX+1)   = 2 * PHYAERO%XINIRADIUSJ * 1E-6
  END IF
  XRHO_IFN(IDX+1)    = PHYAERO%XRHOI(PHYAERO%JP_AER_BC)
  XSIGMA_IFN(IDX+1)  = PHYAERO%XINISIGI
  END IF
  IF ((IDX+2) .LE. NMOD_IFN) THEN
    IF (PHYAERO%CRGUNIT=="MASS") THEN
     XMDIAM_IFN(IDX+2)   = 2 * PHYAERO%XINIRADIUSJ * EXP(-3.*(LOG(PHYAERO%XINISIGJ))**2) * 1E-6
    ELSE
     XMDIAM_IFN(IDX+2)   = 2 * PHYAERO%XINIRADIUSJ * 1E-6
    END IF
  XRHO_IFN(IDX+2)    = PHYAERO%XRHOI(PHYAERO%JP_AER_OC)
  XSIGMA_IFN(IDX+2)  = PHYAERO%XINISIGJ
  END IF
END IF
IF (PHYAERO%LDUST) THEN
   IF (PHYAERO%CRGUNITD=="MASS") THEN
     XMDIAM_IFN(1) = 2.*PHYAERO%XINIRADIUS(PHYAERO%JPDUSTORDER(3))*EXP(-3.*(LOG(PHYAERO%XINISIG(PHYAERO%JPDUSTORDER(3))))**2)
    ELSE
     XMDIAM_IFN(1) = 2.*PHYAERO%XINIRADIUS(PHYAERO%JPDUSTORDER(3))
    END IF
  XSIGMA_IFN(1)  = PHYAERO%XINISIG(PHYAERO%JPDUSTORDER(3))
  XRHO_IFN(1) = PHYAERO%XDENSITY_DUST
END IF
...
END IF ! NMOD_IFN > 0
! 
IF (LHOOK) CALL DR_HOOK('INIT_AEROSOL_PROPERTIES', 1, ZHOOK_HANDLE)
