...
REAL, DIMENSION(D%NIJT,D%NKT),    INTENT(IN)           :: PPABST   ! Absolute pressure at time t
REAL, DIMENSION(D%NIJT,D%NKT),    INTENT(IN)           :: PTHT     ! Theta at time t
REAL, DIMENSION(D%NIJT,D%NKT,KRR), INTENT(IN)           :: PRT      ! Moist variables at time t
REAL, DIMENSION(D%NIJT,D%NKT),    INTENT(INOUT)        :: PRTHS    ! Source terms
REAL, DIMENSION(D%NIJT,D%NKT,KRR), INTENT(INOUT)        :: PRRS     ! Source terms
REAL, DIMENSION(D%NIJT,D%NKT,KSV), INTENT(INOUT)        :: PRSVS    ! Source terms
REAL, DIMENSION(D%NIJT,D%NKT),    INTENT(IN), OPTIONAL :: PRHODJ   ! Dry density * jacobian
