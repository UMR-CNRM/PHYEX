...
SUBROUTINE GETDATA_LIMA_ADJUST (NPROMA, NGPBLKS, NFLEVG, KRR, KSV, NSP, NCARB, NSOA, &
                               ...
                               &ZICE_CLD_WGT_B, PWEIGHT_MF_CLOUD_B,&
                               &ZDUM6_B, ZDUM7_B, ZDUM8_B, ZDUM9_B, &
                               &PHLC_HRC_B, PHLC_HCF_B, PHLI_HRI_B, PHLI_HCF_B, &
                               &PHLC_HRC_OUT_B, PHLC_HCF_OUT_B, PHLI_HRI_OUT_B, PHLI_HCF_OUT_B, &
                               &LDVERBOSE)
...
REAL, INTENT(OUT), ALLOCATABLE   :: ZDUM6_B        (:,:,:)
REAL, INTENT(OUT), ALLOCATABLE   :: ZDUM7_B        (:,:,:)
REAL, INTENT(OUT), ALLOCATABLE   :: ZDUM8_B        (:,:,:)
REAL, INTENT(OUT), ALLOCATABLE   :: ZDUM9_B        (:,:,:)
...
ALLOCATE (PSOLORG_B    (NPROMA,NFLEVG,NSOA,NGPBLKS))
...
ALLOCATE (ZDUM6_B         (NPROMA,NFLEVG,NGPBLKS))
ALLOCATE (ZDUM7_B         (NPROMA,NFLEVG,NGPBLKS))
ALLOCATE (ZDUM8_B         (NPROMA,NFLEVG,NGPBLKS))
ALLOCATE (ZDUM9_B         (NPROMA,NFLEVG,NGPBLKS))
...
PHLC_HRC_OUT(:,:,:)=PRT(:,:,2,:) 
PHLC_HCF_OUT(:,:,:)=PCLDFR_OUT(:,:,:)
PHLI_HRI_OUT(:,:,:)=PRT(:,:,4,:)
PHLI_HCF_OUT(:,:,:)=PICEFR_OUT(:,:,:)
