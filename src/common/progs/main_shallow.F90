...
CALL INIT_PHYEX(KRR, KRRL, KRRI, KSV, PHYEX, MISC)
...
ISTSZ(KIND(PRHODJ)/4) = ISTSZ(KIND(PRHODJ)/4) + NPROMA * 42 * KLEV
...
!$acc parallel loop gang vector present (PHYEX) private (YLSTACK, IBL, JLON, D) collapse (2)
  ...
  DO JLON = 1, NPROMA
    ...
    CALL SHALLOW_MF(D, PHYEX%CST, PHYEX%NEBN, PHYEX%PARAM_MFSHALLN, PHYEX%TURBN, PHYEX%CSTURB, PHYEX%RAIN_ICE_PARAMN,                   &
       &KRR=MISC%KRR, KRRL=MISC%KRRL, KRRI=MISC%KRRI, KSV=MISC%KSV,                                             &
       &ONOMIXLG=MISC%ONOMIXLG,KSV_LGBEG=MISC%KSV_LGBEG,KSV_LGEND=MISC%KSV_LGEND,      &
       &PTSTEP=MISC%PTSTEP, &
...
SUBROUTINE INIT_PHYEX(KRR, KRRL, KRRI, KSV, PHYEX, MISC)
