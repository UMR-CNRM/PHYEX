...
& YDCPG_MISC, YDCPG_GPAR, YDCPG_PHY0, YDMF_PHYS, YDHGRAD, YDCPG_DYN0, YDMF_PHYS_SURF, &
& YDCPG_SL1, YDVARS, YDGMV, YDSURF, YDCFU, YDXFU, YDMODEL, PGFL, PGP2DSDT, PGFLT1, YDDDH)
...
!   YDGMV:                t, t-dt, and t+dt fields for dynamics
...
!   PGFLT1:               Storage of some tendencies (should probably also be removed)
...
USE CPG_TYPE_MOD,                  ONLY: CPG_MISC_TYPE, CPG_GPAR_TYPE, CPG_DYN_TYPE, CPG_PHY_TYPE
...
USE SPP_MOD_TYPE,                  ONLY: UA_PHYS_SPP_VARS, SET_UA_PHYS_SPP, CLEAR_UA_PHYS_SPP
...
REAL(KIND=JPRB) :: ZRHODJM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZRHODREFM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZPABSM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)

REAL(KIND=JPRB) :: ZUM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZVM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZWM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZUS__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZVS__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZWS__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)

REAL(KIND=JPRB) :: ZTHM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)

REAL(KIND=JPRB) :: ZTKES_OUT__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZMF_UP__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZTHVREFM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)  ! thetav de l etat

REAL(KIND=JPRB) :: ZTENDU_TURB__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZTENDV_TURB__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)

REAL(KIND=JPRB) :: ZTKEM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZSRCS__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZSIGS__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)

! length scales for momentum and heat for mnh level definitions in case LHARATU=TRUE
REAL(KIND=JPRB) :: ZLENGTHM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZLENGTHH__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)

REAL(KIND=JPRB) :: ZFLXZTHVMF_SUM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZFLXZUMF_SUM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB) :: ZFLXZVMF_SUM__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)

! Subgrid autoconversions
REAL(KIND=JPRB) :: ZHLC_HRC_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZHLC_HCF_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZHLI_HRI_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZHLI_HCF_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)

! Updraft characteristics for Meso-NH world (input of ARO_SHALLOW_MF)
REAL(KIND=JPRB) :: ZTKES_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZZZ_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZDZZ_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZZZ_F_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)

REAL(KIND=JPRB) :: ZEXNREFM_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZNEBMNH_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)

! additions for future ice cloud fraction and precipitation fraction
REAL(KIND=JPRB) :: ZICEFR_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZPRCFR_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)

! additions for MF scheme (Pergaud et al)
REAL(KIND=JPRB) :: ZSIGMF_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZAERD_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)
...
REAL(KIND=JPRB), POINTER :: ZTHSIN_(:,:)
REAL(KIND=JPRB), TARGET  :: ZTHSWAP__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
REAL(KIND=JPRB), TARGET  :: ZTHSAVE__(YDCPG_BNDS%KFDIA,0:YDCPG_OPTS%KFLEVG+1)
...
REAL(KIND=JPRB), POINTER :: ZRS_(:,:,:)
REAL(KIND=JPRB), POINTER :: ZRSIN_(:,:,:)
...
REAL(KIND=JPRB), POINTER :: ZSVM_(:,:,:)
REAL(KIND=JPRB), POINTER :: ZSVMIN_(:,:,:)
...
REAL(KIND=JPRB), POINTER :: ZLIMAS_(:,:,:)
REAL(KIND=JPRB), POINTER :: ZLIMASIN_(:,:,:)
...
REAL(KIND=JPRB), POINTER :: ZSVSIN_(:,:,:)
REAL(KIND=JPRB), POINTER :: ZSVS_(:,:,:)
...
REAL(KIND=JPRB) :: ZAEROM_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, YDMODEL%YRML_GCONF%YGFL%NAERO)
! Cloud condensation nuclei, ice nuclei and cloud droplets
REAL(KIND=JPRB) :: ZIFN_NC_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZCLDROP_(YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG)
...
REAL(KIND=JPRB) :: ZSFTH_(YDCPG_BNDS%KFDIA)
REAL(KIND=JPRB) :: ZSFRV_(YDCPG_BNDS%KFDIA)

REAL(KIND=JPRB) :: ZACPRG_(YDCPG_BNDS%KFDIA)
REAL(KIND=JPRB) :: ZINPRG_NOTINCR_(YDCPG_BNDS%KFDIA)
REAL(KIND=JPRB) :: ZINPRG_(YDCPG_BNDS%KFDIA)

REAL(KIND=JPRB) :: ZACPRR_(YDCPG_BNDS%KFDIA)
REAL(KIND=JPRB) :: ZINPRR_NOTINCR_(YDCPG_BNDS%KFDIA)
REAL(KIND=JPRB) :: ZINPRR_(YDCPG_BNDS%KFDIA)

REAL(KIND=JPRB) :: ZACPRS_(YDCPG_BNDS%KFDIA)
REAL(KIND=JPRB) :: ZINPRS_NOTINCR_(YDCPG_BNDS%KFDIA)
REAL(KIND=JPRB) :: ZINPRS_(YDCPG_BNDS%KFDIA)

REAL(KIND=JPRB) :: ZINPRH_(YDCPG_BNDS%KFDIA)
REAL(KIND=JPRB) :: ZTOWNS_(YDCPG_BNDS%KFDIA)
REAL(KIND=JPRB) :: ZBUDTH_(YDCPG_BNDS%KFDIA)
...
REAL(KIND=JPRB) :: ZSFU_(YDCPG_BNDS%KFDIA)
REAL(KIND=JPRB) :: ZSFV_(YDCPG_BNDS%KFDIA)
REAL(KIND=JPRB) :: ZZS_(YDCPG_BNDS%KFDIA)
...
! Meso-NH world
REAL(KIND=JPRB) :: ZICLDFR_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZWCLDFR_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZSSIO_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZSSIU_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZIFR_(YDCPG_BNDS%KFDIA,YDCPG_OPTS%KFLEVG)
...
!       Boolean
LOGICAL :: LLMSE
LOGICAL :: LLSWAP_THS, LLSWAP_RS, LLSWAP_SVM, LLSWAP_LIMAS, LLSWAP_SVS ! logical to swap or not pointers in and out
...
!
TYPE(UA_PHYS_SPP_VARS) :: ZSPP_ALL

!     ------------------------------------------------------------------
INTEGER(KIND=JPIM) :: IPTREXT
INTEGER(KIND=JPIM) :: IPTR(YDMODEL%YRML_GCONF%YGFL%NUMFLDS)
INTEGER(KIND=JPIM) :: IPTRLIMA
INTEGER(KIND=JPIM) :: IPTRAERO ! pointer to 1st aerosol field in ZTENDGFLR
...
REAL(KIND=JPRB), POINTER, CONTIGUOUS :: ZTENDAERO (:,:,:) ! 3D N.R.T. AEROSOL MMR TENDENCIES
...
IF ( YDMODEL%YRML_GCONF%YRSPP_CONFIG%LSPP ) THEN
  ...
  DO JSPP=1,YDMODEL%YRML_GCONF%YRSPP_CONFIG%SM%NRFTOTAL
    ZGP2DSPP(:,JSPP) = YDMODEL%YRML_SPP%GP_ARP(JSPP)%GP2D(:,1,YDCPG_BNDS%KBL)
  ENDDO
...
IF (.NOT. YDMODEL%YRML_PHY_EC%YREPHY%LAGPHY) THEN
  ...
  CALL CPPHINP(YDGEOMETRY, YDMODEL, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, &
  & YDVARS%GEOMETRY%GEMU%T0, YDVARS%GEOMETRY%GELAM%T0,                  &
  & YDVARS%U%T0, YDVARS%V%T0, YDVARS%T%DL, YDVARS%T%DM,                 &
  & YDVARS%Q%T0, YDVARS%Q%DL, YDVARS%Q%DM,                              &
  & YDVARS%CVGQ%DL, YDVARS%CVGQ%DM,                                     &
  & YDCPG_PHY0%XYB%RDELP, YDCPG_DYN0%CTY%EVEL, YDVARS%CVGQ%T0,          &
  & ZRDG_MU0, ZRDG_SOLO, ZRDG_MU0LU,                                    &
  & ZRDG_MU0M, ZRDG_MU0N, ZRDG_CVGQ, ZRDG_CVGT)
...
CALL APL_AROME_CALC_IPTR(YDMODEL, IPTR, IPTREXT, IPTRLIMA, IPTRAERO, IPTRTKE, IRR)
...
ZTENDAERO => ZTENDGFLR (:, :, IPTRAERO:IPTRAERO + YDMODEL%YRML_GCONF%YGFL%NAERO - 1)
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LGCHECKMV) THEN
  CALL CHECKMV(YDCPG_OPTS%NINDAT, YDCST, &
             & YDMODEL%YRML_GCONF%YRRIP, YDMODEL%YRML_PHY_MF%YRPHY0, YDMODEL%YRML_PHY_MF%YRPHY2, &
             ...
             & YDVARS%GEOMETRY%GELAM%T0, YDVARS%GEOMETRY%GEMU%T0, ZRDG_MU0, &
             & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_BASE_STATE%T, YDMF_PHYS_BASE_STATE%Q, YDCPG_GPAR%VTS)
ENDIF
...
! 1.5 SPP settings
IF (YDMODEL%YRML_GCONF%YRSPP_CONFIG%LSPP) THEN
  CALL SET_UA_PHYS_SPP(YDCPG_OPTS%KLON,YDCPG_OPTS%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NGFL_EZDIAG, &
   &  YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDMODEL%YRML_GCONF%YRSPP_CONFIG%SM%NRFTOTAL, &
   &  ZGP2DSPP,ZP1EZDIAG, &
   &  YDMODEL%YRML_GCONF%YRSPP_CONFIG,ZSPP_ALL)
ENDIF
...
IF (YDMODEL%YRML_PHY_MF%YRARPHY%LMICRO .OR. YDMODEL%YRML_PHY_MF%YRARPHY%LTURB .OR. &
  ...
  CALL APL_AROME_INIT(YDCST, YDMODEL, YDCPG_BNDS, YDCPG_OPTS, YDCPG_GPAR, &
                    & YDMF_PHYS_BASE_STATE, &
                    & LLMSE, LSFORCS, &
...
CALL APL_AROME_ADJUST(YDCST, YDMODEL, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDGEOMETRY, YDVARS, &
                    ...
                    & ZTHS__(:, 1:YDCPG_OPTS%KFLEVG), ZTHSIN_, ZRS_, ZRSIN_, ZLIMAS_, ZLIMASIN_, &
                    & ZTENDLIMA, ZZZ_F_, ZTENDT, &
                    & ZP1EZDIAG(:,:,1), ZP1EZDIAG(:,:,2), &
                    & ZSRCS__, ZNEBMNH_, &
...
IF (YDMODEL%YRML_DIAG%YRLDDH%LFLEXDIA) THEN
  ...
  CALL ARO_STARTBU(YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, &
                 & YDMODEL%YRML_PHY_MF%YRPARAR%NRR, YDMODEL%YRML_GCONF%YGFL%NGFL_EXT, ZRHODJM__(:, 1:YDCPG_OPTS%KFLEVG), &
                 & ZUS__(:, 1:YDCPG_OPTS%KFLEVG), ZVS__(:, 1:YDCPG_OPTS%KFLEVG), &
                 & ZWS__(:, 1:YDCPG_OPTS%KFLEVG), ZTHS__(:, 1:YDCPG_OPTS%KFLEVG), &
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LAERONRT) THEN
  ...
  CALL ARO_CCN(YDMODEL%YRML_PHY_MF%YRPARAR%PHYEX, &
             ...
             & YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, ZDT,&
             & ZRHODREFM__(:, 1:YDCPG_OPTS%KFLEVG), &
             & ZEXNREFM_, &
             & ZPABSM__(:, 1:YDCPG_OPTS%KFLEVG), &
             & ZDZZ_, &
             & ZNEBMNH_, &
             & ZTHM__(:, 1:YDCPG_OPTS%KFLEVG), &
             & ZRM_, &
             & ZWM__(:, 1:YDCPG_OPTS%KFLEVG), &
...
IF (YDMODEL%YRML_PHY_MF%YRARPHY%LRDUST) THEN
  ...
  CALL APL_AROME_DUST(YDMODEL, YDCPG_BNDS, YDCPG_OPTS, &
                    & IOFF_MFSHAL, &
                    & ZZZ_, ZDZZ_, ZPABSM__, &
                    & ZTHM__, ZRHODREFM__, ZSVMIN_, &
                    & ZAERD_, ZTENDEXT, &
                    & ZP1EZDIAG, ZSVM_, &
...
IF (LSFORCS) THEN   ! <== Surface forcing for MUSC
  ...
  CALL APL_AROME_SURFACE_FORCING(YDCST, YDMODEL, YDMF_PHYS, YDMF_PHYS_BASE_STATE, YDMF_PHYS_SURF, &
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LRAYFM) THEN
  ...
  CALL APL_AROME_RADIATION(YDCST, YDMODEL, YDMF_PHYS_BASE_STATE, YDMF_PHYS_SURF, YDMF_PHYS, YDGEOMETRY, &
                         & YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDCPG_GPAR, YDVARS, YDSURF, &
                         & ZSPP_ALL, YDDDH, &
                         & LLMSE, LSFORCS, &
                         & RALB_FORC, REMIS_FORC, ZADTMS, &
                         & ZCPM, ZQVM, ZTM, &
...
IF(YDMODEL%YRML_PHY_MF%YRARPHY%LKFBCONV) THEN
  ...
  CALL APL_AROME_CONVECTION(YDCST, YDMODEL, YDMF_PHYS_BASE_STATE, &
                          ...
                          & ZPABSM__, ZZZ_F_, ZTM, ZRM_, ZRHODREFM__, ZUM__, ZVM__, ZWM__, &
                          & ZTENDT, ZTEND_Q, ZTEND_L, ZTEND_I, ZRS_, ZQDM, ZTHS__, &
...
CALL APL_AROME_SURFACE(YDCST, YDMODEL, YDMF_PHYS, YDMF_PHYS_BASE_STATE, YDMF_PHYS_SURF, &
                     & YDCPG_BNDS, YDCPG_OPTS, YDGEOMETRY, YDVARS, YDDDH, &
                     & ZZZ_F_, ZSVM_, ZUM__, ZVM__, ZTM, ZRM_, &
                     & ZRHODREFM__, ZPABSM__, ZRDG_MU0, ZRDG_MU0N, &
                     & ZINPRR_NOTINCR_, ZINPRS_NOTINCR_, ZINPRG_NOTINCR_, ZTKEM, &
                     & ZALBD, ZALBP, ZEMIS, ZTSURF, &
                     & ZZS_FSWDIR, ZZS_FSWDIF, &
                     & ZSFTH_, ZSFRV_, ZSFU_, ZSFV_, &
                     & YDCPG_OPTS%NINDAT, NSV_CO2, LLMSE, &
                     & ZQS, ZBUDTH_, ZTOWNS_, ZSFSV_, ZZS_)

IF (LLMSE) THEN

   CALL ACVISIH(YDCST, YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI, YDCPG_BNDS%KIDIA, &
   & YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, &
...
IF (YDMODEL%YRML_PHY_MF%YRARPHY%LMFSHAL) THEN
  ...
  CALL APL_AROME_SHALLOW(YDCST, YDMODEL, YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDMF_PHYS, &
                       & YDCPG_BNDS, YDCPG_OPTS, YDVARS, ZSPP_ALL, YDDDH, &
                       ...
                       & ZTHM__, ZUM__, ZVM__, &
                       & ZRS_, ZTHS__, ZTKES_, &
                       & ZTENDTKE, ZTENDT, ZTENDTT, ZTEND_Q, ZTEND_R, ZTEND_S, &
                       & ZUS__, ZVS__, ZTKEM, &
                       & ZMF_UP__, &
                       & ZSURFPREP, ZSURFSNOW, &
...
IF (YDMODEL%YRML_PHY_MF%YRARPHY%LTURB) THEN
  ...
  IF (YDMODEL%YRML_GCONF%YGFL%NGFL_EXT == 0) THEN
    CALL SWAP_SVS
  ELSE
    ZSVSIN_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDCPG_OPTS%KFLEVG,1:YDMODEL%YRML_GCONF%YGFL%NGFL_EXT) = &
    & ZSVS_(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDCPG_OPTS%KFLEVG,1:YDMODEL%YRML_GCONF%YGFL%NGFL_EXT)
  ENDIF

  CALL APL_AROME_TURBULENCE(YDCST, YDMODEL, YDGEOMETRY, YDMF_PHYS_BASE_STATE, YDMF_PHYS, &
                          & YDCPG_BNDS, YDCPG_OPTS, YDVARS, YDHGRAD, YDXFU, ZSPP_ALL, YDDDH, &
                          & ZAPHIM, ZAPHIFM, ZZZ_, ZZZ_F_, &
                          & ZLIMAM_, ZLIMASIN_, ZSVM_, ZSVSIN_, ZTKES_, ZSIGMF_, &
                          ...
                          & ZMF_UP__, ZTENDT, ZTEND_Q, &
                          & ZTENDTKE, ZTENDLIMA, ZSFSV_, &
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LWINDFARM) THEN
  ...
  JGFL = INT(GFL_WKA2(1, 6, 1))
  CALL APL_AROME_WINDFARM(YDCST, YDMODEL, YDGEOMETRY, &
                        & YDMF_PHYS_BASE_STATE, YDMF_PHYS, &
                        & YDCPG_BNDS, YDCPG_OPTS, YDVARS, &
                        & ZTKEM, &
                        & JGFL, GFL_WKA, GFL_WKA2, &
                        & ZTENDTKE, ZTKES_OUT__, &
                        & ZTENDU_TURB__, ZTENDV_TURB__, &
                        & ZP1EZDIAG(:,:,2))
...
IF (YDMODEL%YRML_PHY_MF%YRARPHY%LMICRO) THEN
  ...
  CALL APL_AROME_MICRO(YDMODEL, YDCPG_BNDS, YDCPG_OPTS, &
                     ...
                     & ZTHSIN_, ZRSIN_, ZLIMASIN_, ZSVMIN_, &
                     & ZDTHRAD_, ZSIGS__, &
                     & ZICLDFR_, ZWCLDFR_, &
                     & ZSSIO_, ZSSIU_, ZIFR_, &
                     & ZQDM, ZZZ_, &
                     ...
                     & ZTHS__(:, 1:YDCPG_OPTS%KFLEVG), ZRS_, ZLIMAS_, ZSVM_, &
                     & ZTENDAERO)
ENDIF ! LMICRO
...
CALL APL_AROME_FINAL(YDCST, YDMODEL, YDCPG_BNDS, YDCPG_OPTS, YDVARS, &
                   & YDCFU, YDXFU, YDMF_PHYS_BASE_STATE, YDMF_PHYS_SURF, &
                   & YDCPG_GPAR, YDMF_PHYS, &
                   & LSFORCS, &
                   & ZDT, &
...
IF (YDMODEL%YRML_DIAG%YRLDDH%LFLEXDIA) THEN
  ...
  CALL APL_AROME_DDH(YDCPG_BNDS, YDCPG_OPTS, YDMODEL, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDDDH, &
                   & ZTSURF)
...
CALL APL_AROME_TW_TENDENCY(YDCST, YDMODEL, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_DYN0, YDVARS, &
                         & YDMF_PHYS_BASE_STATE, YDGMV, YDMF_PHYS, &
                         & YDMF_PHYS_NEXT_STATE, YDCPG_SL1, &
                         & IPTR, &
                         & ZTENDT, ZTENDW, &
                         & ZTEND_Q, ZTEND_L, ZTEND_R, ZTEND_I, ZTEND_S, ZTEND_G, ZTEND_H, &
                         & ZTENDTKE, ZTENDGFLR, &
                         & PGFLT1, ZTENDD)
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  & YDSURF, YDMODEL%YRML_PHY_G%YRDPHY, YDMODEL%YRML_GCONF%YRRIP, YDMODEL%YRML_PHY_MF,                         &
  & YDCPG_BNDS%KFDIA, YDCPG_BNDS%KIDIA, YDCPG_OPTS%KGL1,                                                      &
  & YDCPG_OPTS%KGL2, YDCPG_OPTS%NSTEP,                                                     &
  & YDMODEL%YRML_PHY_G%YRDPHY%NTSSG, YDSURF%YSP_SBD%NLEVS, YDVARS%GEOMETRY%GELAM%T0,                          &
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LGCHECKNAN) THEN
  CALL CHECKNANARO(YDMODEL%YRML_GCONF%YRRIP, YDMODEL%YRML_PHY_MF%YRPHY0, YDMODEL%YRML_PHY_MF%YRPHY2, &
                 & YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, &
                 & YDMODEL%YRML_PHY_MF%YRPARAR%NRR, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%NSTEP, &
                 & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, &
                 & YDMF_PHYS_BASE_STATE%YCPG_PHY%PRE, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREF, &
                 & YDVARS%GEOMETRY%GELAM%T0, YDVARS%GEOMETRY%GEMU%T0, ZRDG_MU0, &
                 & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_BASE_STATE%T, YDMF_PHYS_BASE_STATE%Q, ZQSAT, &
                 & ZTSURF, ZTENDT, ZTENDRA, ZTENDW, ZTENDTKE, &
                 & YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRSO, &
                 & YDMF_PHYS%OUT%FRTHDS, YDMF_PHYS%OUT%FRSODS, YDMF_PHYS%OUT%CLCH, &
                 & YDMF_PHYS%OUT%CLCM, YDMF_PHYS%OUT%CLCL, YDCPG_MISC%CLCT, &
                 & YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPLSG, &
                 & YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV, YDMF_PHYS%OUT%FCS, &
                 & YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN)
ENDIF

! Clear SPP
IF (YDMODEL%YRML_GCONF%YRSPP_CONFIG%LSPP) THEN

  CALL CLEAR_UA_PHYS_SPP(ZSPP_ALL)
